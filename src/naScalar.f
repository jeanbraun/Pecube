...
c*************************************************************************
c
      Subroutine NA (ndin,rangein,run) ! Re-formating XR (2022/02/23)
c
c  MPI definitions
cifdef MPI
d       include "mpif.h"
...
c
c Set up all dimensions for arrays used by NA routines
      real*4misfit(nmod_max), ! Re-formating XR (2022/02/23)
     & misfitval,mfitmin, ! Re-formating XR (2022/02/23)
     &                  mfitmean,mfitminc

      real*4 sum(nsample_max) ! Re-formating XR (2022/02/23)
c
      real*4 na_models(nd_max*nmod_max), ! Re-formating XR (2022/02/23)
     & range(2,nd_max), ! Re-formating XR (2022/02/23)
     & ranget(2,nd_max), ! Re-formating XR (2022/02/23)
     & scales(nd_max+1), ! Re-formating XR (2022/02/23)
     & na_model(nd_max), ! Re-formating XR (2022/02/23)
     & xcur(nd_max) ! Re-formating XR (2022/02/23)
      ...
      integer mfitord(nsample_max) ! Re-formating XR (2022/02/23)

      real work_NA1(nmod_max), ! Re-formating XR (2022/02/23)
     &      work_NA2(nmod_max) ! Re-formating XR (2022/02/23)

      integer  iwork_NA1(nmod_max), ! Re-formating XR (2022/02/23)
     &         iwork_NA2(nsample_max) ! Re-formating XR (2022/02/23)

c
      logical monte, ! Re-formating XR (2022/02/23)
     &                  verbose,
     &                  summary,
     &                  timing,
     &                  debug,
     ...
     &                  calcmovement
      ...
      logical lroot ! Re-formating XR (2022/02/23)
c
c  Some bookeeping variables not used by NA routines
c     real acells(nit_max)
      real*4 model_opt(nd_max) ! Re-formating XR (2022/02/23)

c added by Jean for compatibility for Pecube

        character*255 run ! Re-formating XR (2022/02/23) ; Change 5 charact. limitation

        integer  nproc, iproc, ierr ! Re-formating XR (2022/02/23)
...
c
c  Set up logical units LU's for standard output
        lu_out = 6
c
c
...
c
      lu_na = 30 ! Re-formating XR (2022/02/23)
c  LU for input of quasi random number generator data (sobol sequence data)
      lu_sob = 31 ! Re-formating XR (2022/02/23)
c  LU for output of information summarising NA performance. 
        lu_sum = 32
c  LU for output of more detailed information to file na.det
        lu_det = 33
c  LU for output of sample density in best voronoi cell
c  (Only used if calcmovement is set to true and NA_PDF is not used)
c  
c  LU for NAD input/output file
        lu_nad = 34
c  LU for special info file
      lu_dis = 35 ! Re-formating XR (2022/02/23)

c  set up variables and start MPI
      iproc = 0 ! Re-formating XR (2022/02/23)
      nproc = 1 ! Re-formating XR (2022/02/23)
      lroot = .false. ! Re-formating XR (2022/02/23)
...
c  User specific setup for forward modelling
        nd=ndin
        range(:,1:nd)=rangein(:,1:nd)
      call user_init(nd,range,scales) ! Re-formating XR (2022/02/23)

c  Open NA input files
      open(lu_na, file=trim(run)//'/NA/na.in',status='old') ! Re-formating XR (2022/02/23); Change 5 charact limitation

c   Open NA output files
      if(lroot)open(lu_sum, file=trim(run)//'/NA/na.sum',  ! Re-formating XR (2022/02/23); Change 5 charact limitation
     & status='unknown')
c  write header to standard out
      if(lroot)write(*,100) ! Re-formating XR (2022/02/23)

c  Read in options for Neighbourhood algorithm.
      call NA_options ! Re-formating XR (2022/02/23)
...
c
      close(lu_na) ! Re-formating XR (2022/02/23)
c  set other info or debug options
      ...
      if(lroot)open(lu_dis, file='distances', status='unknown') ! Re-formating XR (2022/02/23)
        end if
c
c  Initialize NA routines.
c
      call NA_initialize ! Re-formating XR (2022/02/23)
     &       (range,ranget,scales,
     &        nd,nd_max,xcur,nsample,ncells,restartNA)

c  Generate or read in starting models 
c
      call NA_initial_sample ! Re-formating XR (2022/02/23)
...
c
      ...
      do 20 it = 1,itmax+1 ! Re-formating XR (2022/02/23)
c
c  Calculate misfit values for each model in the current population.
c
           if(summary.and.lroot)then
              write(*,*)
              write(*,*)' start forward modeling iteration: ',it-1
      end if ! Re-formating XR (2022/02/23)

c  skip forward modelling for starting models if 
c  they have been read in from a NAD file

      if(it.eq.1.and.noforward)go to 31 ! Re-formating XR (2022/02/23)
           t1 = cputime(t2,t3)
      do 30 i = iproc+1,ns,nproc ! Re-formating XR (2022/02/23)
c  Decode current model and put into array model.
c
              ii = 1+(i-1+ntot)*nd
     ...
     &             (na_models(ii),nd,range,scales,na_model)
      ...
      misfitval = 0.0 ! Re-formating XR (2022/02/23)
      call forward(nd, na_model, misfitval, run, iproc) ! Re-formating XR (2022/02/23)
c
              jj = ntot + i
      misfit(jj) = misfitval ! Re-formating XR (2022/02/23)
c
 30   continue ! Re-formating XR (2022/02/23)
           t1 = cputime(t2,t3)
           ttfor = ttfor + t2
...
c     write(*,*)iproc,' : ',(misfit(k),k=1+ntot,ntot+ns)
 ...
 31        if(summary.and.lroot)write(*,*)' finished forward modeling'
      ...
      if(nproc.gt.1)then ! Re-formating XR (2022/02/23)
       do i=1,ns ! Re-formating XR (2022/02/23)
         misfit(ntot+i) = sum(i) ! Re-formating XR (2022/02/23)
       end do ! Re-formating XR (2022/02/23)
      end if ! Re-formating XR (2022/02/23)
           ...
           tnat = tnat + t2
c
c  Calculate properties of current misfit distribution (Mean,min,best model etc.)

      call NA_misfits ! Re-formating XR (2022/02/23)
     ...
     &           iwork_NA1,iwork_NA2,mfitord)
           ...
           tmis = tmis + t2
           tnat = tnat + t2
c  copy optimum model from current population, to array model_opt.
c
           ii = 1 + (mopt-1)*nd
c
           call transform2raw
...
c
      ntot = ntot + ns ! Re-formating XR (2022/02/23)
           ns = nsample
c  write out summary info
      if(verbose.and.lroot)then ! Re-formating XR (2022/02/23)
        call NA_display ! Re-formating XR (2022/02/23)
     &             (lu_det,model_opt,it-1,nd,ntot,
     &              mfitmin,mfitminc,mfitmean,mopt)
      end if ! Re-formating XR (2022/02/23)

      if(it.eq.itmax+1)go to 99 ! Re-formating XR (2022/02/23)
c       F I/O
c
c  Call main NA routines
c
      if(monte)then ! Re-formating XR (2022/02/23)
              t1 = cputime(t2,t3)
c  Perform Monte Carlo search for comparison to NA.
      call NA_random ! Re-formating XR (2022/02/23)
     &             (na_models,nd,ranget,
     &              ntot,nsample,calcmovement)
      ...
      else  ! Re-formating XR (2022/02/23)
c  generate a new sample using Neighbourhood algorithm (resample version)

            call NA_sample ! Re-formating XR (2022/02/23)
...
c
 20   continue ! Re-formating XR (2022/02/23)
c
...
c  Some statistics
      ...
      if(summary.and.lroot)then ! Re-formating XR (2022/02/23)
...
c  transform all models back to scaled units
      do i=1,ntot ! Re-formating XR (2022/02/23)
           ii = 1 + (i-1)*nd
           call transform2raw
     &          (na_models(ii),nd,range,scales,na_models(ii))
      end do ! Re-formating XR (2022/02/23)

      nh = 1 ! Re-formating XR (2022/02/23)
c  call user supplied I/O routine
      if(lroot)call writemodels  ! Re-formating XR (2022/02/23)
     &       (nd, ntot, na_models, misfit,
     &        nsamplei, nsample, itmax, nh_max, nh, header, run)
c
c  write out ensemble of models as a direct access file
c  add NA-info to begining of header file
c
        fnme = trim(run)//'/NA/na.nad' ! Re-formating XR (2022/02/23) ; Remove 5 charact. limitation
      call NA_header ! Re-formating XR (2022/02/23)
...
c    write direct access nad file
        ...
        fnme = trim(run)//'/NA/na.nad' ! Re-formating XR (2022/02/23); Remove 5 characts limitation
...
c
      if(lroot)close(lu_sum) ! Re-formating XR (2022/02/23)
      if(lroot)close(lu_det) ! Re-formating XR (2022/02/23)
      close(lu_sob) ! Re-formating XR (2022/02/23)
        if(calcmovement)close(lu_dis)
c  Advise of files created
      if(lroot)then ! Re-formating XR (2022/02/23)

      write(*,*) ! Re-formating XR (2022/02/23)
      write(*,*)' Files created:' ! Re-formating XR (2022/02/23)
      write(*,*) ! Re-formating XR (2022/02/23)
      write(*,*) ! Re-formating XR (2022/02/23)
     &  ' na.nad : Direct access file containing ensemble of models ' 
      write(*,*) ! Re-formating XR (2022/02/23)
     &  ' na.sum : Short summary file of progress' 
      if(verbose)write(*,*) ! Re-formating XR (2022/02/23)
...
cendif
      ...
      return ! Re-formating XR (2022/02/23)
      end ! Re-formating XR (2022/02/23)
c
c ----------------------------------------------------------------------------
c
...
c
     ...
     &             (nsample_max, nit_max, nmod_max,
     &             nsleep_max, nd, nsamplei, nsample, istype,  ! Re-formating XR (2022/02/23)
     &             noforward, nsleep, ncells, itmax, monte, nclean, run) ! Re-formating XR (2022/02/23)

      logical monte ! Re-formating XR (2022/02/23)
      logical verbose ! Re-formating XR (2022/02/23)
      logical summary ! Re-formating XR (2022/02/23)
      logical timing ! Re-formating XR (2022/02/23)
      logical debug ! Re-formating XR (2022/02/23)
      logical sobol ! Re-formating XR (2022/02/23)
      logical noforward ! Re-formating XR (2022/02/23)
      logical lroot ! Re-formating XR (2022/02/23)

      character yesorno*1, run*255 ! Re-formating XR (2022/02/23); Change 5 charact limitation
      !character*1 yesorno
      !character*255 run
...
c                               to read in NA options.
      do i=1,3 ! Re-formating XR (2022/02/23)
         read(lu_na,*) ! Re-formating XR (2022/02/23)
      end do ! Re-formating XR (2022/02/23)
...
c          shrink = .false.
           ...
           write(*,*)
      call na_abort ! Re-formating XR (2022/02/23)
        end if
c
...
c    &                 ,' acceptance'
c       read(lu_na,*)nsleep
      nsleep = 1 ! Re-formating XR (2022/02/23)
...
c    &                 ' sequence'
      yesorno = 'Y' ! Re-formating XR (2022/02/23)
        read(lu_na,fmt='(a1,1x,i7)')yesorno,iseed
        sobol = .false.
      if(iseed.lt.0)then ! Re-formating XR (2022/02/23)
         iseed = -iseed ! Re-formating XR (2022/02/23)
      end if ! Re-formating XR (2022/02/23)
        if(yesorno.eq.'y'.or.yesorno.eq.'Y')sobol=.true.
c
c  read initial sample type
      read(lu_na,*)istype ! Re-formating XR (2022/02/23)
        noforward = .false.
      if(istype.lt.0)then ! Re-formating XR (2022/02/23)
         istype = -istype ! Re-formating XR (2022/02/23)
         noforward = .true. ! Re-formating XR (2022/02/23)
      end if
c
c  read in size of initial sample from nad file
        if(istype.eq.1)then

           len = 16
           open(lu_nad,file=trim(run)//'/NA/na.nad',status='old', ! Re-formating XR (2022/02/23); Remove 5 charact. limitation
     &          form='unformatted',access='direct',recl=len,err=20)
           read(lu_nad,rec=1)nd_nad,nsamplei,nh,nhu
           close(lu_nad)

      if(nd_nad.ne.nd)then ! Re-formating XR (2022/02/23)
         write(*,*) ! Re-formating XR (2022/02/23)
         write(*,*)' Error detected in reading starting models', ! Re-formating XR (2022/02/23)
     &                ' from NAD file' ! Re-formating XR (2022/02/23)
         write(*,*) ! Re-formating XR (2022/02/23)
         write(*,*)' Dimension of parameter space obtained'   ! Re-formating XR (2022/02/23)
         write(*,*)' from user provided subroutine user_init'   ! Re-formating XR (2022/02/23)
         write(*,*)' differs from that read in from NAD file'   ! Re-formating XR (2022/02/23)
         write(*,*) ! Re-formating XR (2022/02/23)
         write(*,*)' Perhaps this is the wrong NAD file ?' ! Re-formating XR (2022/02/23)
         write(*,*)' or user subroutine is in error ?' ! Re-formating XR (2022/02/23)
         write(*,*) ! Re-formating XR (2022/02/23)
         write(*,*)' Remedy: correct input NAD file or' ! Re-formating XR (2022/02/23)
         write(*,*)'         user_init and recompile' ! Re-formating XR (2022/02/23)
         write(*,*) ! Re-formating XR (2022/02/23)
         call na_abort ! Re-formating XR (2022/02/23)
      end if ! Re-formating XR (2022/02/23)
 ...
 20           write(*,*)
     ...
     &                ' from NAD file' ! Re-formating XR (2022/02/23)
         write(*,*) ! Re-formating XR (2022/02/23)
         write(*,*)' Cannot open file NA/na.nad' ! Re-formating XR (2022/02/23)
         write(*,*)' does it exist ?' ! Re-formating XR (2022/02/23)
         write(*,*) ! Re-formating XR (2022/02/23)
         write(*,*)' Remedy: ensure correct input NAD file' ! Re-formating XR (2022/02/23)
         write(*,*)'         is available for reading in' ! Re-formating XR (2022/02/23)
         write(*,*) ! Re-formating XR (2022/02/23)
         call na_abort ! Re-formating XR (2022/02/23)
 21        continue
...
c       read(lu_na,*)nclean
      nclean = 500 ! Re-formating XR (2022/02/23)
...
c         call na_abort 
     ...
     &                   ' greater than the maximum.'
      ...
      if(istype.gt.2.or.istype.lt.0)then ! Re-formating XR (2022/02/23)
          write(lu_out,200)
...
c       write(lu_sum,*)' Length of walk in cell        : ',nsleep
      ...
      if(istype.eq.1)then ! Re-formating XR (2022/02/23)
           write(lu_sum,*)' Starting models read in from NAD file'
      else if(istype.eq.2)then ! Re-formating XR (2022/02/23)
        ...
        write(lu_sum,100)
c
      if(summary)then ! Re-formating XR (2022/02/23)
...
c
      ...
      if(verbose.and.lroot)then ! Re-formating XR (2022/02/23)
         open(lu_det,file='na.det',status='unknown') ! Re-formating XR (2022/02/23)
           write(lu_det,100)
         write(lu_det,*)' Details of each iteration' ! Re-formating XR (2022/02/23)
           write(lu_det,100)
      end if ! Re-formating XR (2022/02/23)
...
d        include "mpif.h"
cendif
      real range(2,nd) ! Re-formating XR (2022/02/23)
      real ranget(2,nd) ! Re-formating XR (2022/02/23)
      real scales(*) ! Re-formating XR (2022/02/23)
        real x(nd) ! Re-formating XR (2022/02/23)
        real rval(2) ! Re-formating XR (2022/02/23)
c       real psums(nsel_max+1)
        real            dummy(1)

      logical restartNA ! Re-formating XR (2022/02/23)
      logical sobol ! Re-formating XR (2022/02/23)
      logical verbose ! Re-formating XR (2022/02/23)
      logical summary ! Re-formating XR (2022/02/23)
      logical timing ! Re-formating XR (2022/02/23)
      ...
      logical lroot ! Re-formating XR (2022/02/23)
        ...
        common /NA_IO/lu_na,lu_out,lu_sum,lu_det,lu_sob,lu_dis,
     &                lu_nad,verbose,debug,timing,summary
      common /NA_init/idnext,ic ! Re-formating XR (2022/02/23)
        common/randomtype/sobol,iseed

c  common /NA_pickdata/psums(nsel_max+1),nfmax

      if(nd.gt.nd_max)then ! Re-formating XR (2022/02/23)
         write(*,400)nd,nd_max ! Re-formating XR (2022/02/23)
         call na_abort ! Re-formating XR (2022/02/23)
      end if    ! Re-formating XR (2022/02/23)
          
c  set logical switch for first call to NA_sample 
c  (ensures distance list is initialized)
      restartNA = .true. ! Re-formating XR (2022/02/23)
c
c  set initial parameter for NA walk
c     idnext = 1
      ic = 1 ! Re-formating XR (2022/02/23)

      if(sobol)then ! Re-formating XR (2022/02/23)
c  Initialize 1-D Sobol sequence to control initial dimension in NA random walk
c          call sobseq(-2,rval)
           call sobseq(-1,rval)
c  Initialize pseudo-random number generator
      iseed0 = iseed ! Re-formating XR (2022/02/23)
      a = ran3(iseed0) ! Re-formating XR (2022/02/23)
c  open file for sas coefficients
c
      if(lroot)open(lu_sob,file='sobol.coeff',status='unknown') ! Re-formating XR (2022/02/23)

c  Generate coefficients for quasi-random multi-dimensional SAS sequence
c
      call NA_sas_table(nd*ncells,lu_sob) ! Re-formating XR (2022/02/23)

cifdef MPI
c    need to sync here so that all processes can read a file in NA_sobol
d          call MPI_BARRIER(MPI_COMM_WORLD)
cendif

      if(lroot)close (lu_sob) ! Re-formating XR (2022/02/23)
c  Initialize n-D Sobol sequence
      open(lu_sob,file='sobol.coeff',status='unknown') ! Re-formating XR (2022/02/23)
...
c  Initialize pseudo-random number generator
      iseed0 = iseed ! Re-formating XR (2022/02/23)
      a = ran3(iseed) ! Re-formating XR (2022/02/23)

      end if ! Re-formating XR (2022/02/23)
c    Normalize parameter ranges by a-priori model co-variances
        if(scales(1).eq.0.0)then
c  First option: No transform 
c  (All a priori model co-variances are equal to unity)
      do i=1,nd ! Re-formating XR (2022/02/23)
         ranget(1,i) = range(1,i) ! Re-formating XR (2022/02/23)
         ranget(2,i) = range(2,i) ! Re-formating XR (2022/02/23)
         scales(i+1) = 1.0 ! Re-formating XR (2022/02/23)
      end do ! Re-formating XR (2022/02/23)

        else if(scales(1).eq.-1.0)then
c  Second option: Use parameter range as a priori model co-variances 
      do i=1,nd ! Re-formating XR (2022/02/23)
              ranget(1,i) = 0.0
              ranget(2,i) = 1.0
         scales(i+1) = range(2,i)-range(1,i) ! Re-formating XR (2022/02/23)
      end do ! Re-formating XR (2022/02/23)

        else
c  Third option: Use scales array as a priori model co-variances 
      do i=1,nd ! Re-formating XR (2022/02/23)
              if(scales(i+1).eq.0.0)then ! Re-formating XR (2022/02/23)
                 write(*,200)i
                 call na_abort
              end if
              ranget(1,i)  = 0.0
...
c  calculate axis increments and initialize current point
c  (used by NA_sample) to mid-point of parameter space
...
c  write out information on parameter space to summary file
      if(lu_sum.gt.5.and.lroot)then ! Re-formating XR (2022/02/23)
   
c       write(lu_sum,300)
      write(lu_sum,fmt='("Parameter space details:"/)') ! Re-formating XR (2022/02/23)
...
c ----------------------------------------------------------------------------
c
      Subroutine NA_initial_sample ! Re-formating XR (2022/02/23)
     ...
     &              scales,misfit,run)
        ...
        character       header*(nh_max)
        character*256   fnme
        character*255 run ! Re-formating XR (2022/02/23); Remove 5 charact limitation
...
c  read in models from ascii input file
         open(lu_nad,file='models.in',status='old',err=33) ! Re-formating XR (2022/02/23)
         k = 1 ! Re-formating XR (2022/02/23)
 32      read(lu_nad,*,end=34,err=33) ! Re-formating XR (2022/02/23)
         read(lu_nad,*,err=33)misfit(k) ! Re-formating XR (2022/02/23)
           read(lu_nad,*)(na_models(i,k),i=1,nd)
         k = k + 1 ! Re-formating XR (2022/02/23)
           go to 32
 33        write(*,*)' Error reading input file models.in'
           call na_abort
 34        continue
         close(lu_nad) ! Re-formating XR (2022/02/23)
           nsample_r = k-1
...
c
         if(nsample_r.lt.nsample)then ! Re-formating XR (2022/02/23)
c  randomly generate the remaining samples 
            nrem = nsample - nsample_r ! Re-formating XR (2022/02/23)
            write(*,*)' generating remaining samples ',nrem ! Re-formating XR (2022/02/23)
c  Generate initial samples using quasi random sequences
            if(sobol)then
...
c  Use pseudo random number generator ran3 
                 do i=nsample_r+1,nsample
                    do j=1,nd
                       a = ran3(iseed)
                       b = 1-a
...
c  Generate initial uniform random sample using a uniform random distribution
...
c ----------------------------------------------------------------------------
c
      Subroutine NA_random ! Re-formating XR (2022/02/23)
     &             (na_models,nd,range,ntot,nsample,calcmovement)

        real*4          na_models(nd,*),
     &                  range(2,*)

      logical calcmovement ! Re-formating XR (2022/02/23)
      logical verbose ! Re-formating XR (2022/02/23)
      logical summary ! Re-formating XR (2022/02/23)
      logical timing ! Re-formating XR (2022/02/23)
      logical debug ! Re-formating XR (2022/02/23)
     ...
     &                lu_nad,verbose,debug,timing,summary
      ...
      dsum = 0.0 ! Re-formating XR (2022/02/23)
c     generate samples
        do i=ntot+1,ntot+nsample

         if(sobol)then ! Re-formating XR (2022/02/23)
c     use quasi-random number generator
              do j=1,nd
                 call NA_sobol(ldummy,j,na_models(j,i),1,0)
                 a = na_models(j,i)
                 b = 1-a
...
c      use pseudo-random number generator
              do j=1,nd
                 a = ran3(iseed)
                 b = 1-a
                 na_models(j,i) = b*range(1,j) + a*range(2,j)
...
c find distance to nearest model (for info purposes)

         if(calcmovement)then ! Re-formating XR (2022/02/23)
...
c  write out average distance of population from current best model
      if(calcmovement)then ! Re-formating XR (2022/02/23)
         write(lu_dis,*)ntot+nsample,dsum/real(nsample) ! Re-formating XR (2022/02/23)
      end if ! Re-formating XR (2022/02/23)
...
c ----------------------------------------------------------------------------
c
      Subroutine NA_sas_table(nt,lu) ! Re-formating XR (2022/02/23)

        parameter(maxdeg=10)

        integer         a(maxdeg)
        real*8          ntot(10)
        integer   nprim(10) ! Re-formating XR (2022/02/23)
      ...
      data ntot/1.,2.,8.,64.,1024.,32768.,2097152., ! Re-formating XR (2022/02/23)
     &                       268435456.,6.8719476E10,3.5184372E13/
      data nprim/1,1,2,2,6,6,18,16,48,60/ ! Re-formating XR (2022/02/23)
      data pporder/0,59*0, ! Re-formating XR (2022/02/23)
     & 1,59*0, ! Re-formating XR (2022/02/23)
     & 1,2,58*0, ! Re-formating XR (2022/02/23)
     & 1,4,58*0, ! Re-formating XR (2022/02/23)
     & 2,4,7,11,13,14,54*0, ! Re-formating XR (2022/02/23)
     & 1,13,16,19,22,25,54*0, ! Re-formating XR (2022/02/23)
     & 1,4,7,8,14,19,21,28,31,32, ! Re-formating XR (2022/02/23)
     & 37,41,42,50,55,56,59,62,42*0, ! Re-formating XR (2022/02/23)
     & 14,21,22,38,47,49,50,52,56,67, ! Re-formating XR (2022/02/23)
     & 70,84,97,103,115,122,44*0, ! Re-formating XR (2022/02/23)
     &                          8,13,16,22,25,44,47,52,55,59,
     & 62,67,74,81,82,87,91,94,103,104, ! Re-formating XR (2022/02/23)
     & 109,122,124,137,138,143,145,152, ! Re-formating XR (2022/02/23)
     & 157,167,173,176,181,182,185,191, ! Re-formating XR (2022/02/23)
     & 194,199,218,220,227,229,230,234, ! Re-formating XR (2022/02/23)
     & 236,241,244,253,12*0, ! Re-formating XR (2022/02/23)
     &  4, 13, 19, 22, 50, 55, 64, 69,  ! Re-formating XR (2022/02/23)
     & 98, 107, 115, 121, 127, 134, 140,  ! Re-formating XR (2022/02/23)
     & 145, 152, 158, 161, 171, 181, 194,  ! Re-formating XR (2022/02/23)
     & 199, 203, 208, 227, 242, 251, 253,  ! Re-formating XR (2022/02/23)
     & 265, 266, 274, 283, 289, 295, 301,  ! Re-formating XR (2022/02/23)
     & 316, 319, 324, 346, 352, 361, 367,  ! Re-formating XR (2022/02/23)
     & 382, 395, 398, 400, 412, 419, 422,  ! Re-formating XR (2022/02/23)
     & 426, 428, 433, 446, 454, 457, 472,  ! Re-formating XR (2022/02/23)
     & 493, 505, 508/ ! Re-formating XR (2022/02/23)

        common/randomtype/sobol,iseed

      open(lu,file='sobol.coeff',status='unknown') ! Re-formating XR (2022/02/23)
...
c     choose degree, order and number of independent sequences to be generated using 
c     malcolm's formula defined above
        ii = 0
c          read(10,*)mdeg,ip,num
      do mdeg=5,10 ! Re-formating XR (2022/02/23)
           if(mdeg.gt.maxdeg)stop 'too many degrees' ! Re-formating XR (2022/02/23)
         numt = int(ntot(mdeg)/100) ! Re-formating XR (2022/02/23)
         do num=1,numt ! Re-formating XR (2022/02/23)
            is = mod(num,nprim(mdeg))+1 ! Re-formating XR (2022/02/23)
            ip = pporder(is,mdeg)  ! Re-formating XR (2022/02/23)
            ii = ii + 1 ! Re-formating XR (2022/02/23)
              do j=2,mdeg
                 rval = ran3(iseed)
                 m = 2**(j-1)
     ...
     &        write(lu,*)' ',mdeg,'   ',ip,'    ',(a(j),j=1,mdeg)
            if(ii.eq.nt)go to 99 ! Re-formating XR (2022/02/23)
           end do
        end do
  99    continue
c
 ...
 102    format('%',15x,'random seed used =',i10/
     &         '% deg   poly    Initializing values')
      ...
      close(lu) ! Re-formating XR (2022/02/23)
...
c
      ...
      REAL x(*)
c
      include 'na_param.inc' ! Re-formating XR (2022/02/23)
c
...
c
      ...
      logical debug ! Re-formating XR (2022/02/23)
      logical verbose ! Re-formating XR (2022/02/23)
      logical silent ! Re-formating XR (2022/02/23)
...
c                                               read in data
      ...
      if(debug)then ! Re-formating XR (2022/02/23)
         write(*,*)' NA_sobol read in initializing data for', ! Re-formating XR (2022/02/23)
...
c ----------------------------------------------------------------------------
c
      Subroutine NA_misfits ! Re-formating XR (2022/02/23)
     &             (misfit,nsample,it,ntot,mfitmin,
     &              mfitminc,mfitmean,mopt,
     &              ncells,work,ind,iwork,mfitord) ! Re-formating XR (2022/02/23)

      real misfit(*) ! Re-formating XR (2022/02/23)
      real mfitmin ! Re-formating XR (2022/02/23)
      real mfitmean ! Re-formating XR (2022/02/23)
      real mfitminc ! Re-formating XR (2022/02/23)
      real work(*) ! Re-formating XR (2022/02/23)

      integer ind(*) ! Re-formating XR (2022/02/23)
      integer mfitord(*) ! Re-formating XR (2022/02/23)
      integer iwork(*) ! Re-formating XR (2022/02/23)
     ...
     &                lu_nad,verbose,debug,timing,summary
      ...
      do i=ntot+2,ntot+nsample ! Re-formating XR (2022/02/23)
           mfitmean = mfitmean + misfit(i)
           if(misfit(i).lt.mfitminc)then
              mfitminc = misfit(i)
            iopt = i ! Re-formating XR (2022/02/23)
           end if 
      end do ! Re-formating XR (2022/02/23)
...
c                                       find models with lowest ncells misfit values
         if(ncells.eq.1)then ! Re-formating XR (2022/02/23)

               mfitord(1) = mopt

           else 

            ntotal = ntot+nsample ! Re-formating XR (2022/02/23)
              do i=1,ntotal
               ind(i) = i ! Re-formating XR (2022/02/23)
                 work(i) = misfit(i)
              end do
   
c     jumble initial indices to randomize order of models when misfits are equal
            call jumble(ind,work,ntotal) ! Re-formating XR (2022/02/23)
c
...
c             end do
         ...
         do j=1,ncells ! Re-formating XR (2022/02/23)
                iwork(j) = ind(j)
              end do 
...
c   write(50,*)' n ',n,' i',i,' j',j,' dev ',dev(i,j)
         ...
         ran = ran3(iseed) ! Re-formating XR (2022/02/23)
           deviate = x1 + (x2-x1)*ran(1)
...
c
     ...
     &         (nd,nc,nsp,nsl,nsample,dev) ! Re-formating XR (2022/02/23)
      ...
      nrem = mod(nsample,nc) ! Re-formating XR (2022/02/23)

      n = nd*nc ! Re-formating XR (2022/02/23)
      m = nsp*nsl ! Re-formating XR (2022/02/23)
      n1 = nd*nrem ! Re-formating XR (2022/02/23)
      n2 = nd*(nc-nrem) ! Re-formating XR (2022/02/23)
      m1 = nsp*nsl ! Re-formating XR (2022/02/23)
      m2 = (nsp-1)*nsl ! Re-formating XR (2022/02/23)

      if(sobol)then ! Re-formating XR (2022/02/23)

        if(nrem.eq.0)then ! Re-formating XR (2022/02/23)
           do i=1,m ! Re-formating XR (2022/02/23)
              do j=1,n ! Re-formating XR (2022/02/23)
                   call NA_sobol(ldummy,j,dev(j,i),1,0)
                end do
             end do
        else ! Re-formating XR (2022/02/23)
           do i=1,m1 ! Re-formating XR (2022/02/23)
              do j=1,n1 ! Re-formating XR (2022/02/23)
                   call NA_sobol(ldummy,j,dev(j,i),1,0)
                end do
             end do
           do i=1,m2 ! Re-formating XR (2022/02/23)
              do j=n1+1,n ! Re-formating XR (2022/02/23)
                   call NA_sobol(ldummy,j,dev(j,i),1,0)
                end do
             end do
        end if ! Re-formating XR (2022/02/23)
...
c
     ...
     &              calcmovement, nclean, dlist)
      ...
      real na_models(nd,*) ! Re-formating XR (2022/02/23)
      real range(2,*) ! Re-formating XR (2022/02/23)
      real misfit(*) ! Re-formating XR (2022/02/23)

      real xcur(*) ! Re-formating XR (2022/02/23)
      real dlist(*) ! Re-formating XR (2022/02/23)
      real xdum(100) ! Re-formating XR (2022/02/23)
c
...
c
      ...
      integer mfitord(*) ! Re-formating XR (2022/02/23)
      integer cell ! Re-formating XR (2022/02/23)

      logical verbose ! Re-formating XR (2022/02/23)
      logical summary ! Re-formating XR (2022/02/23)
      logical timing ! Re-formating XR (2022/02/23)
      logical debug ! Re-formating XR (2022/02/23)
      logical resetlist ! Re-formating XR (2022/02/23)
      logical check ! Re-formating XR (2022/02/23)
      logical restartNA ! Re-formating XR (2022/02/23)
      logical info ! Re-formating XR (2022/02/23)
      logical x_on_left ! Re-formating XR (2022/02/23)
c     logical x_on_right ! Re-formating XR (2022/02/23)
      logical calcmovement ! Re-formating XR (2022/02/23)

        common /NA_info/nxsave,ndsave,ndc,nerr,ncald,nupd,
     &                  cells,torder,taxis,tup,tcd,tdev,tna,tres

      common /NA_info_special/taxis2 ! Re-formating XR (2022/02/23)
     ...
     &                lu_nad,verbose,debug,timing,summary
      ...
      save id ! Re-formating XR (2022/02/23)
...
c     initialize some variables
      ic = ic + 1 ! Re-formating XR (2022/02/23)
        info = .false.
        if(mod(ic,nclean).eq.0)resetlist = .true.
      if(mod(ic,nclean).eq.0.and.verbose)info = .true. ! Re-formating XR (2022/02/23)
        ...
        ind_celllast = 0
      dsum = 0.0 ! Re-formating XR (2022/02/23)
      dcount = 0.0 ! Re-formating XR (2022/02/23)
      nrem = mod(nsample,ncells) ! Re-formating XR (2022/02/23)
        if(nrem.eq.0)then
        ...
        icount = 0
      if(debug)write(*,*)' nsample     = ',nsample ! Re-formating XR (2022/02/23)
      if(debug)write(*,*)' nsampercell = ',nsampercell ! Re-formating XR (2022/02/23)
      if(debug)write(*,*)' ncells      = ',ncells ! Re-formating XR (2022/02/23)
c       write(lu_sum,*)
c       write(lu_sum,*)
c
c     loop over samples
        do is = 1,nsample
c     choose Voronoi cell for sampling
      ind_cell = ind_cellnext  ! Re-formating XR (2022/02/23)
           icount = icount + 1
      if(debug)write(*,*)cell,' cell = ',ind_cell ! Re-formating XR (2022/02/23)
...
c
c     update dlist and nodex for new axis
...
c     Calculate intersection of current Voronoi cell with current 1-D axis
         call NNaxis_intersect ! Re-formating XR (2022/02/23)
...
c
c     Generate new node in Voronoi cell of input point
              if(timing)then
                 t1 = cputime(t2,t3)
                 tna = tna + t2
              end if
...
c     check Voronoi boundaries 
         ...
         if(check)then ! Re-formating XR (2022/02/23)
                 do i=1,nd
                   xdum(i) = xcur(i)
                 end do
                 xdum(id) = x1
            call findnearest(xdum,na_models,ntot,nd,nnode) ! Re-formating XR (2022/02/23)
            write(*,*)' Nearest node to x1 = ',nnode ! Re-formating XR (2022/02/23)
                 xdum(id) = x2
            call findnearest(xdum,na_models,ntot,nd,nnode) ! Re-formating XR (2022/02/23)
            write(lu_sum,*)' Nearest node to x2 = ',nnode ! Re-formating XR (2022/02/23)
            write(*,*)' Nearest node to x2 = ',nnode ! Re-formating XR (2022/02/23)
         end if ! Re-formating XR (2022/02/23)
c
c     increment axis 
              idnext = idnext + 1
...
c     check nearest node
      ...
      if(check)then ! Re-formating XR (2022/02/23)
         call findnearest(xcur,na_models,ntot,nd,nnode) ! Re-formating XR (2022/02/23)

         write(lu_sum,*)' Nearest node to new model',is+ntot, ! Re-formating XR (2022/02/23)
     &                       ' = ',nnode
         write(*,*)' Nearest node to new model',is+ntot, ! Re-formating XR (2022/02/23)
     &                  ' = ',nnode
...
c     find distance moved from Vcell node 

      if(calcmovement.and.ind_cell.eq.mopt)then ! Re-formating XR (2022/02/23)
              dist = 0.0
              do i=1,nd
         ...
         ind_celllast = ind_cell ! Re-formating XR (2022/02/23)

           if(icount.eq.nsampercell)then
              icount = 0 
              cell = cell + 1
         ind_cellnext = mfitord(cell) ! Re-formating XR (2022/02/23)
         if(cell.eq.nrem+1)nsampercell = nsampercell - 1 ! Re-formating XR (2022/02/23)
           end if
c
...
c     write average distance of samples from node in optimum Vcell 
      if(calcmovement)then ! Re-formating XR (2022/02/23)
           write(lu_dis,*)ntot+nsample,dsum/dcount,ind_cell
      end if ! Re-formating XR (2022/02/23)
c  write out average distance of samples from current Vcell node
...
c-----------------------------------------------------------------------
c
cNNaxis_intersect - find intersections of current Voronoi cell 
...
c-----------------------------------------------------------------------
c
      Subroutine NNaxis_intersect ! Re-formating XR (2022/02/23)
     &             (x,dim,dlist,bp,nd,nb,
     &              nodex,xmin,xmax,x1,x2)

      real*4 x(nd) ! Re-formating XR (2022/02/23)
      real*4 bp(nd,nb) ! Re-formating XR (2022/02/23)
      real*4 dlist(nb) ! Re-formating XR (2022/02/23)
        integer dim ! Re-formating XR (2022/02/23)
c  search through nodes
      x1 = xmin ! Re-formating XR (2022/02/23)
      x2 = xmax ! Re-formating XR (2022/02/23)
      dp0   = dlist(nodex) ! Re-formating XR (2022/02/23)
      x0    = bp(dim,nodex) ! Re-formating XR (2022/02/23)
...
c     write(*,*)'left = ',left
c     write(*,*)'right = ',right
c find intersection of current Voronoi cell with 1-D axis
           do j=1,nodex-1
              xc    = bp(dim,j)
              dpc   = dlist(j)
c calculate intersection of interface (between nodes nodex and j) and 1-D axis.
         dx = x0 - xc ! Re-formating XR (2022/02/23)
         if(dx.ne.0.0)then ! Re-formating XR (2022/02/23)
            xi = 0.5*(x0+xc+(dp0-dpc)/dx) ! Re-formating XR (2022/02/23)
...
c
     ...
     &             (na_models,nd,mreset,x,debug,restartNA)
      ...
      real na_models(nd,*) ! Re-formating XR (2022/02/23)
      real x(*) ! Re-formating XR (2022/02/23)
...
c-----------------------------------------------------------------------
c
      Subroutine NNupdate_dlist ! Re-formating XR (2022/02/23)
     &             (dim,dimlast,dlist,bp,nd,nb,x,node,dmin)

      real*4 bp(nd,*) ! Re-formating XR (2022/02/23)
      real*4 x(nd) ! Re-formating XR (2022/02/23)
      real*4 dlist(*) ! Re-formating XR (2022/02/23)
        integer dim,dimlast ! Re-formating XR (2022/02/23)
        ...
        d2 = d2*d2
        dlist(1) = dmin-d2
      do i=2,nb ! Re-formating XR (2022/02/23)
...
c ----------------------------------------------------------------------------
c
      Subroutine findnearest(smodelt,na_models,ntot,moddim,mopts) ! Re-formating XR (2022/02/23)

      real  na_models(moddim,*) ! Re-formating XR (2022/02/23)
      real  smodelt(*) ! Re-formating XR (2022/02/23)

      common /findnearestcom/dmin ! Re-formating XR (2022/02/23)
        ...
        mopts = 1
      do j=2,ntot ! Re-formating XR (2022/02/23)
           d = 0
           do i=1,moddim
...
c ---------------------------------------------------------------------
c
      Subroutine read_da(lu,fnme,nd,ne,nh,mul,header,models,data) ! Re-formating XR (2022/02/23)
c
        real*4            models(nd,ne)
        real*4            data(ne)
        character         header(nh)
...
c
      ...
      real*4 tarray(2) ! Re-formating XR (2022/02/23)

      cputime = dtime(tarray) ! Re-formating XR (2022/02/23)
        t1 = tarray(1)
        t2 = tarray(2)
...
c ----------------------------------------------------------------------------
c
      Subroutine NA_header ! Re-formating XR (2022/02/23)
     &             (lu,fnme,header,nh_max,nh,nd,
     &              range,scales,n1,n2,n3,nhu)

      real*4    range(2,nd) ! Re-formating XR (2022/02/23)
      real*4  scales(nd+1) ! Re-formating XR (2022/02/23)
        character*(*)     header
        character*256   fnme ! Re-formating XR (2022/02/23)

c                                               calculate total header length
      rlen = 3*nd + 4 ! Re-formating XR (2022/02/23)
        len = 4*rlen+nh
      nhu = nh ! Re-formating XR (2022/02/23)
      nh_na   = 4*rlen ! Re-formating XR (2022/02/23)
      nh_tot  = len ! Re-formating XR (2022/02/23)
c  write(*,*)' nh_user = ',nhu
c  write(*,*)' nh_na   = ',nh_na  
...
c  write out header information
      call write_header ! Re-formating XR (2022/02/23)
     &       (lu,fnme,len,nd,nh,range,scales,n1,n2,n3,header)

      nh = nh_tot ! Re-formating XR (2022/02/23)
c  read header information
c  into character string
      call read_header ! Re-formating XR (2022/02/23)
...
c        temporary file
      Subroutine read_header ! Re-formating XR (2022/02/23)
     &   (lu,fnme,nh,len,header) ! Re-formating XR (2022/02/23)
...
c                                               open direct access 
c  temporary file
      Subroutine write_header ! Re-formating XR (2022/02/23)
     &     (lu,fnme,len,nd,nh, ! Re-formating XR (2022/02/23)
     &              range,scales,n1,n2,n3,header) ! Re-formating XR (2022/02/23)

      real*4  range(2,nd) ! Re-formating XR (2022/02/23)
      real*4  scales(nd+1) ! Re-formating XR (2022/02/23)
      character         header(nh) ! Re-formating XR (2022/02/23)
      character*256  fnme ! Re-formating XR (2022/02/23)
...
c
     ...
     &              mfitmin,mfitminc,mfitmean,mopt)
      ...
      real*4  model_opt(*) ! Re-formating XR (2022/02/23)
      real*4  mfitmin,mfitmean,mfitminc ! Re-formating XR (2022/02/23)
...
c write out optimum model
      ...
      write(lu_det,*)' Parameter best fitting model' ! Re-formating XR (2022/02/23)
      do i=1,nd ! Re-formating XR (2022/02/23)
         write(lu_det,*)' ',i,' : ',model_opt(i) ! Re-formating XR (2022/02/23)
      end do ! Re-formating XR (2022/02/23)
        write(lu_det,100)
...
c ----------------------------------------------------------------------------
c
      Subroutine jumble(iarr,arr,n) ! Re-formating XR (2022/02/23)

      integer iarr(n) ! Re-formating XR (2022/02/23)
      real*4 arr(n) ! Re-formating XR (2022/02/23)
...
c write(*,*)(iarr(k),k=1,n)
        ...
        do j=1,n
           ...
           k = 1 + int(val*rn)
           if(k.eq.n+1)then
         k = n ! Re-formating XR (2022/02/23)
           else if(k.gt.n)then
...
c ----------------------------------------------------------------------------
c
      Subroutine na_abort ! Re-formating XR (2022/02/23)
...
cendif
      stop ! Re-formating XR (2022/02/23)
      end ! Re-formating XR (2022/02/23)
