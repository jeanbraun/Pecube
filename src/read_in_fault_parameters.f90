...
double precision dx,dy,xlon,xlat,xl0,yl0 !Maxime
character line*1024
logical logvelo
...
xl = p%lon0+(p%nx-1)*dx 
yl = p%lat0+(p%ny-1)*dy
if (p%lon0.lt.0) xlon1 = p%lon0 + 360
!if (p%lat0.lt.0) xlat = p%lat0 + 90
!Retrieve coordinate of the fault
p%x1 = xlon1 + abs(xl - p%lon0) * ((p%x1 - p%lon0) / (xl - p%lon0)) 
p%x2 = xlon1 + abs(xl - p%lon0) * ((p%x2 - p%lon0) / (xl - p%lon0))
