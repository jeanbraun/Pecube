...
! this is where the FE matrices and RHS vectors are built
      ...
      integer icon(mpe),kfix(nnode),nx,ny,nz,heatprod_flag
      ...
      integer mpe,nint,k,i,iint,j,nnode,istatic,nfault,check
      double precision diffusivity,heatproduction,x1f,y1f,x2f,y2f,def,dif,alpha0,dt0,time,D_heat!MB
...
!           endif
      ...
      enddo
          ...
          heat=heat_production(xinit,yinit,zinit,tint,tfinal-time,heatproduction,D_heat)
          endif
