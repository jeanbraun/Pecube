...
c check with Pecube for any inverted parameter value update, for instance
c a parameter value updates if t1 < t2
       counter = 1
c       iterate through number of models
       do n = 1,nsamplei 
   	     nn = counter
c       transform scaled parameter value into absolute value
   	     call transform2raw 
     &        (na_models(nn),nd,range,scales,na_model)
c          read parameter value into `Pecube and update accordingly
           call read_ifile(nd, na_model, misfitval, run, iproc)
           
c          transform back to scaled value
           call transform2sca(na_model,nd,range,scales,na_models(nn))
...
c			 if(summary.and.lroot)write(*,*)'na_model1 : ', na_model
              call transform2raw
     &             (na_models(ii),nd,range,scales,na_model)

	      misfitval = 9999.0
	      call forward(nd, na_model, misfitval, run, iproc)
c In the new version of pecube, an inverted parameter value can change according to another.
c For instance, the user might want to limit the relief amplitude value below the value from 
c the previous time step, inverted as well. Changing the value of the inverted parameter chosen
c by the NA algorithm change the scale stored of that parameter (in 'na_models' above). It is therefore
c important to rescale according to the new parameter values in 'na_model' returned by Pecube.
c We thus add an additionnal call to transform2sca below
...
c       iterate through number of models
	   do n = 1,ns
	     nn = nsamplei*nd + (it-1)*(ns*nd) + counter
c       transform scaled parameter value into absolute value
	     call transform2raw
     &        (na_models(nn),nd,range,scales,na_model)
c          read parameter value into `Pecube and update accordingly
...
c						ascii input file
c
	   open(lu_nad,file=run//'/NA/models.in',status='old',err=33)
	   k = 1
