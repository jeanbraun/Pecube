...
integer ires,vocal, num
integer,intent(inout)::nd
real*4,intent(inout)::range(2,1024),par(1024)
character*10 text2
character*10 text_temp

character line*256,record*256,cres0*256
integer ios,icolon,istar,itime,ihashtag, i1, i2
...
if (ieq/=0) then
   if (trim(adjustl(line(1:ieq-1))).eq.text(1:len_trim(text))) then
      nrec=len_trim(adjustl(line(ieq+1:len_trim(line))))
      record(1:nrec)=trim(adjustl(line(ieq+1:len_trim(line))))
      itime=index(record(1:nrec),'time_topo')
      if (itime.ne.0) then
...
991     read(record(itime+9:itime+9),'(i1)',err=999) i
      ...
      ! ! Check for tectonic time in time topo
      ! itime=index(record(1:nrec),'time_end')
      ! print *, 'itime = ', itime, record(1:nrec)
      ! if (itime.ne.0) then
      !   read(record(itime+8:itime+8),'(i1)',err=999) i1
      !   read(record(itime+10:itime+10),'(i1)',err=999) i2
      !   text2=""
      !   write(text_temp,'(I1)') i1
      !   text2 = trim(text2) //trim(text_temp)
      !   write(text_temp,'(I1)') i2
      !   text2 = trim(text2) // trim(text_temp)
      !   read(text2, '(I2)'), num
      !   print *, 'num = ', num
      !   ires=-num
...
889     continue
        ...
        istar=scan(record(1:nrec),'*')
            ihashtag=scan(record(1:nrec),'#')
        if (icolon.eq.0 .and. istar.eq.0 .and. ihashtag.eq.0 .and. itime.eq.0) then
          read (record(1:nrec),*,err=999) res
          ires=1
        elseif (icolon.ne.0 .and. istar.eq.0 .and. ihashtag.eq.0) then
              nd=nd+1
              read (record(1:icolon-1),*,err=999) range(1,nd)
              read (record(icolon+1:nrec),*,err=999) range(2,nd)
              res=par(nd)
              ires=1
            elseif (icolon.ne.0 .and. istar.ne.0 .and. ihashtag.eq.0) then !case for inversion
                nd=nd+1
                if (istar.gt.1) then
                    read (record(1:icolon-1),*,err=999) range(1,nd) ! Record first number
                    ires=996
                  ! print *, 'range = ', range(2,nd)
        ...
        elseif (icolon.eq.0 .and. istar.ne.0) then
              nrec=1
              record(1:1)='*'
              ires=999
              ! Maxime
            elseif (icolon.eq.0 .and. ihashtag.ne.0) then
              nrec=1
              record(1:1)='#'
              ires=998
        endif
