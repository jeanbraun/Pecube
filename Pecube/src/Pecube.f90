!--------------------------------------------------------------------------------------------
!This version of Pecube is compatible with PecubeGUI

program Main

 use Pecube
 type (version) :: vers
 type (parameters) :: p, p0
 integer :: nd, ninter, ierr
 real*4, dimension(1024) :: par
 real*4, dimension(2,1024) :: range
 real*4 :: misfit, val1, val2, dval
 real*4, dimension(:), allocatable :: param_array
 character*5 run
 character*15 value_string

 character*9 arg
 integer :: numarg

 integer k
 character*9 cparam(1024)
 character*7 cAges(1024)
 logical is_unix ! Maxime


 numarg=command_argument_count()
 if (numarg.eq.0) then
  write (*,*) 'You need to specify a run directory (i.e. RUN00 for example)'
  stop 'End of run'
 else
   call getarg (1,arg)
   if (arg.eq.'--version') then
      write (*,*) vers%str
      stop
   else
      run = arg
   endif
 endif
 ! Check Os system
 call GetOperatingSystem(is_unix)

 ! checks that the run directory exists, otherwise exits

 open (7,file=run//'/input/Pecube.in',status='unknown',err=999)
 goto 998
 999 write (*,*) 'Cant open/create '//run//'/input/Pecube.in'
 write (*,*) 'Check that the directory '//run//'/input exists'
 stop
 998 continue
 close (7)

 if (is_unix) then
    call system ('rm '//run//'/output/*')
    ! For NA do not delete na.nad, can be reused...
    call system ('rm '//run//'/NA/NA_int_res.csv')
    call system ('rm '//run//'/NA/NA_results.csv')
    call system ('rm '//run//'/NA/na.in')
    call system ('rm '//run//'/NA/models.in')
    call system ('mkdir "'//run//'/output"')
    call system ('mkdir "'//run//'/data"')
    call system ('mkdir "'//run//'/NA"')
    call system ('mkdir "'//run//'/LOG"')
 else
    call system ('del /Q '//run//'\output\*')
    call system ('del /Q '//run//'\NA\NA_int_res.csv')
    call system ('del /Q '//run//'\NA\NA_results.csv')
    call system ('del /Q '//run//'\NA\na.in')
    call system ('del /Q '//run//'\NA\models.in')
  call system ('del /Q '//run//'\NA\Age*')
  call system ('del /Q '//run//'\NA\Elevation*')
    call system ('mkdir "'//run//'/output"')
    call system ('mkdir "'//run//'/data"')
    call system ('mkdir "'//run//'/NA"')
    call system ('mkdir "'//run//'/LOG"')
 endif
 nd = 0
 par = 0.

 call read_input_file (run//'/input/Pecube.in', 0, p0, nd, range, par)
 if (p0%echo_input_file.gt.0.and.nd.eq.0) then
  write (*,*) '---------------------------------------------------------------------------------'
  write (*,*) '-------------------------------Pecube--------------------------------------------'
  write (*,*) '---------------------------------------------------------------------------------'
  write (*,*) 'version:',vers%str
  write (*,*) '---------------------------------------------------------------------------------'
  write (*,*) '-----------------------Echoing input parameters----------------------------------'
  write (*,*) '---------------------------------------------------------------------------------'
 endif

 nd = 0
 par = 0.
 range = 0.
 if (p0%echo_input_file.eq.4) write (*,*) 'Complete list of model parameters'
 call read_input_file (run//'/input/Pecube.in', p0%echo_input_file, p, nd, range, par)
 if (p0%echo_input_file.eq.4) stop

 if (p0%echo_input_file.gt.0.and.nd.eq.0) then
  write (*,*) '---------------------------------------------------------------------------------'
 endif

 if (nd.ne.0) then
    do k=1,1024
       ! Parameters and Age label
     write (cparam(k),'(a5,i4)') 'Param',k
       write (cAges(k),'(a3,i4)') 'Age',k
     if (k.lt.10) then
          cparam(k)(6:8)='000'
          cAges(k)(4:6) = '000'
     elseif (k.lt.100) then
          cparam(k)(6:7)='00'
          cAges(k)(4:5) = '00'
     elseif (k.lt.1000) then
          cparam(k)(6:6)='0'
          cAges(k)(4:4) = '0'
       endif
    enddo
    open (71,file=run//'/NA/NA_int_res.csv',status='unknown')
    ! write (71,'(1025a)') 'Misfit',(','//cparam(k),k=1,nd),',','datascore'
    write (71,'(1025a)') 'Misfit',(','//cparam(k),k=1,nd)
    close (71)
    open  (71,file=run//'/NA/na.in',status='unknown')
    if (p%inversion_mode.eq.2) then ! batch mode
       write (71,'(a)') '#'
       write (71,'(a)') '#       Uniform MC input options file'
       write (71,'(a)') '#'
       write (71,'(a)') '0         : Algorithm type (NA or Uniform MC: 1=MC,0=NA)'
    else
     write (71,'(a)') '#'
       write (71,'(a)') '#       Neighbourhood Algorithm input options file'
       write (71,'(a)') '#'
       write (71,'(a)') '0         : Algorithm type (NA or Uniform MC: 1=MC,0=NA)'
    endif
    write (71,'(i5,a)') p%maximum_number_of_iterations,'         : Maximum number of iterations'
    write (71,'(i5,a)') p%sample_size_for_first_iteration,'        : Sample size for first iteration'
    write (71,'(i5,a)') p%sample_size_for_all_other_iterations,'         : Sample size for all other iterations'
    write (71,'(i5,a)') p%number_of_cells_to_resample,'         : Number of cells to re-sample'
    if (p%inversion_mode.eq.2) then !Batch mode
       write (71,'(a)') 'y,210728  : Use Quasi random number generator ? (y/n);random seed'
       write (71,'(a)') '2         : Type of initial sample (0=random;1=read in a NAD file)'
    else
       write (71,'(a)') 'n,210728  : Use Quasi random number generator ? (y/n);random seed'
       if (p%inversion_mode.eq.1) then ! from previous inversion file (na.nad)
          write (71,'(a)') '1         : Type of initial sample (0=random;1=read in a NAD file)'
       else
          write (71,'(a)') '0         : Type of initial sample (0=random;1=read in a NAD file)'
       endif
    endif
    write (71,'(a)') '2         : Output information level (0=silent,1=summary info,2=1+models)'
    write (71,'(a)') 'y         : Turn timing mode on ? (y/n)'
    write (71,'(a)') 'n         : Turn debug mode on ? (y/n)'
    close (71)

    ! open (71,file=run//'/NA/He43.csv',status='new',action='write',position='rewind')
    ! close (71)
    

    if (p%inversion_mode.eq.2) then
    
       ! First write the list of parameter values in misfit.in
       open (unit=72,file=run//'/NA/models.in',status='unknown',iostat=ierr,position='rewind')
       ! Check if the file was opened successfully
       if (ierr /= 0) then
            print *, 'Error opening misfit.in file'
            stop
       endif
       val1 = range(1,1) ! min value for parameter to batch
       val2 = range(2,1) ! max value for parameter to batch
       ninter = p%sample_size_for_first_iteration-1 ! number of intervals
       allocate (param_array(ninter+2))
       dval = (val2 - val1) / ninter
       param_array = (/((val1+dval*(i-1)), i=1,ninter+2)/)
       do k=1,ninter+2
        ! Write empty line
        write (72,'(g2.0)') '  '
          ! write random misfit
          write (72,'(A)') '0.5' 
          ! write parameter value
          write (value_string,'(g15.5)') param_array(k)
          write (72,'(A)') adjustl(value_string)
       enddo
       close(72)
      
       ! Initiate file to write ages
       if (p%age_ESR_flag.ne.0.) then
      open (71,file=run//'/NA/AgeESR.csv',status='new',action='write',position='rewind')
      close (71)
     endif
     if (p%age_AHe_flag.ne.0.) then
      open (71,file=run//'/NA/AgeApatiteHelium.csv',status='new',action='write',position='rewind')
      
        ! write (71,'(1025a)') 'Misfit',(','//cAges(k),k=1,nd)
      ! write (71,'("Predicted Apatite Helium Ages")')
      close (71)
     endif
     if (p%age_AFT_flag.ne.0.) then
      open (71,file=run//'/NA/AgeApatiteFT.csv',status='new',action='write',position='rewind')
        ! write (71,'(1025a)') 'Misfit',(','//cAges(k),k=1,nd)
      ! write (71,'("Predicted Apatite FT Ages")')
      close (71)
     endif
     if (p%age_ZHe_flag.ne.0.) then
      open (71,file=run//'/NA/AgeZirconHelium.csv',status='new',action='write',position='rewind')
        ! write (71,'(1025a)') 'Misfit',(','//cAges(k),k=1,nd)
      ! write (71,'("Predicted Zircon Helium Ages")')
      close (71)
     endif
     if (p%age_ZFT_flag.ne.0.) then
      open (71,file=run//'/NA/AgeZirconFT.csv',status='new',action='write',position='rewind')
        ! write (71,'(1025a)') 'Misfit',(','//cAges(k),k=1,nd)
      ! write (71,'("Predicted Zircon FT Ages")')
      close (71)
     endif
     if (p%age_KAr_flag.ne.0.) then
      open (71,file=run//'/NA/AgeKSparArgon.csv',status='new',action='write',position='rewind')
        ! write (71,'(1025a)') 'Misfit',(','//cAges(k),k=1,nd)
      ! write (71,'("Predicted K-spar Argon Ages")')
      close (71)
     endif
     if (p%age_BAr_flag.ne.0.) then
      open (71,file=run//'/NA/AgeBiotiteArgon.csv',status='new',action='write',position='rewind')
        ! write (71,'(1025a)') 'Misfit',(','//cAges(k),k=1,nd)
      ! write (71,'("Predicted Biotite Argon Ages")')
      close (71)
     endif
     if (p%age_MAr_flag.ne.0.) then
      open (71,file=run//'/NA/AgeMuscoviteArgon.csv',status='new',action='write',position='rewind')
        ! write (71,'(1025a)') 'Misfit',(','//cAges(k),k=1,nd)
      ! write (71,'("Predicted Muscovite Argon Ages")')
      close (71)
     endif
     if (p%age_HAr_flag.ne.0.) then
      open (71,file=run//'/NA/AgeHornblendeArgon.csv',status='new',action='write',position='rewind')
        ! write (71,'(1025a)') 'Misfit',(','//cAges(k),k=1,nd)
      ! write (71,'("Predicted Hornblende Argon Ages")')
      close (71)
     endif
      
      !   ! Initiate file to write elevation
      ! open (71,file=run//'/NA/ElevationPredicted.csv',status='new',action='write',position='rewind')
      ! close (71)
    endif
  endif

  if (nd.eq.0) then
    call forward (nd, par, misfit, run, 0)
  else
    call NA (nd, range, run)
  endif

end program Main

!--------------------------------------------------------------------------------------------

subroutine user_init (nd,range,scales)

implicit none

real*4 range(2,*)
real*4 scales(*)
integer nd

scales(1:nd)=-1.

end subroutine user_init

!--------------------------------------------------------------------------------------------

subroutine writemodels (nd,ntot,models,misfit,ns1,ns2,itmax,nh_max,nh,header,run)

implicit none

integer :: nd,ntot,ns1,ns2,itmax,nh_max,nh
real*4 :: models (nd,*)
real*4 :: misfit (ntot)
character*(*) :: header
character*5 run
character*9 cparam(1024)

integer :: i,k

  do k=1,1024
  write (cparam(k),'(a5,i4)') 'param',k
  if (k.lt.10) cparam(k)(6:8)='000'
  if (k.lt.100) cparam(k)(6:7)='00'
  if (k.lt.1000) cparam(k)(6:6)='0'
  enddo

open (87,file=run//'/NA/NA_results.csv',status='unknown')
write (87,'(1025a)') 'Misfit',(','//cparam(k),k=1,nd)
  do i=1,ntot
  write (87,'(g15.9,1024(a,g15.9))') misfit(i),(',',models(k,i),k=1,nd)
  enddo
close (87)

end subroutine writemodels

!--------------------------------------------------------------------------------------------

subroutine read_ifile (nd,param,misfit,run, iproc)

    use Pecube
    
    !      implicit real*8 (a-h,o-z)
    implicit none
    
    type (version) :: vers
    type (parameters) p
    type (faulttype),dimension(:),allocatable::fault,faultp
    
    integer nd, iproc
    real*4 param(nd)
    real*4 misfit
    real*4 range(2,1024)
    real*4 times(1000)
    
    character run*5
    
    integer i
    double precision eps
    
    integer check
    
    call cpu_time (times(1))
    
    eps=tiny(eps)
    
    nd=0
    check = 0
    p%run_name = run
    call read_input_file (run//'/input/Pecube.in', 0, p, nd, range, param)

end subroutine read_ifile
      
!--------------------------------------------------------------------------------------------

!subroutine forwardp (nd, par, misfit)

!use Pecube

!implicit none

!type (parameters) :: p
!integer nd
!real*4 par(nd), misfit
!real*4 range(2,1024)

!nd=0
!call read_input_file ('Pecube.in', 0, p, nd, range, par)

! here I should be able to run Pecube using a single line such as:
! call Pecube (p, nd, misfit)
! that would take the input parameters stored in p and return a misfit

!return
!end

!--------------------------------------------------------------------------------------------

subroutine forward (nd,param,misfit,run, iproc)

  use Pecube

 !      implicit real*8 (a-h,o-z)
  implicit none

  type (version) :: vers
  type (parameters) p
  type (faulttype),dimension(:),allocatable::fault,faultp

  double precision,dimension(:),allocatable :: x,y,z,xp,yp,zp
  double precision,dimension(:),allocatable :: u,up !by VKP
  double precision,dimension(:),allocatable :: t,tp,f
  integer,dimension(:,:),allocatable :: icon
  double precision,dimension(:,:,:),allocatable :: ael
  double precision,dimension(:,:),allocatable :: bel
  integer, dimension(:),allocatable :: kfix,ielsurf,ieo
  double precision,dimension(:),allocatable :: xsurf,ysurf,zsurf,zsurfp
  double precision,dimension(:),allocatable :: usurf,usurfp,tempsurf,tempsurfp !by VKP
  double precision,dimension(:),allocatable :: topoa,topob,rsurf,rebound
  integer,dimension(:,:),allocatable :: iconsurf,neighbour
  double precision,dimension(:,:),allocatable :: xdepth,ydepth,zdepth
  double precision,dimension(:,:),allocatable::ftdist,ftdisto
  double precision,dimension(:),allocatable :: xdeptho,ydeptho,zdeptho,zsurfo,zsurfpo
  double precision,dimension(:,:),allocatable :: tprev
  double precision,dimension(:),allocatable :: tprevo, fmean,dfmean, fmeanp,dfmeanp
  double precision, dimension(:,:),allocatable:: xexhumation,yexhumation,zexhumation
  double precision, dimension(:),allocatable:: xexhumationo,yexhumationo,zexhumationo
  double precision,dimension(:),allocatable::we1,we2,we3,we4,het,diff,zinit,xinit,yinit
  integer,dimension(:),allocatable :: proc,ice
  real*4,dimension(:),allocatable :: t2,t2p,t3,t4,t5
  real*4,dimension(:),allocatable::vxx,vyy,vzz,exhumation
  integer, dimension(:), allocatable :: nhist,nheating_array
  real *4, dimension(:,:), allocatable :: thist,temphist,errortemphist,temphistp
      
  double precision, dimension(:), allocatable :: doser,d0,radius,et,logs,b,logrho,eu,sigmaet,heighto,a_coef
  double precision, dimension(:), allocatable :: GOK_a, GOK_b
  real*4, dimension(:), allocatable :: nn, dnn
  double precision, dimension(:,:), allocatable :: timeTSL,temperatureTSL
  double precision, dimension(:,:), allocatable :: timeOSL,temperatureOSL
  double precision, dimension(:,:), allocatable :: timeESR,temperatureESR
  real*4,dimension(:),allocatable::ztimep,ztempp !Maxime
  double precision,dimension(:,:),allocatable::memp !Maxime
  real*4 zjunkp,timenowp,age43Hep
  real*4, dimension(:),allocatable:: ageESRf,ageOSLf !Maxime
  real*4 error_pred, nnf
  integer ESRmisfitTarget, OSLmisfitTarget
  double precision, dimension(:), allocatable :: imax

  double precision, dimension(:,:),allocatable::lsf,lsfn,lsfx,lsfy,lsfxn,lsfyn

  character run*5,cstep*3

  real*4 times(1000)
  real*4,dimension(:),allocatable :: age1,age2,age3,age4,age5,age6,age7,age8,grainsize,grainobs
  real*4,dimension(:),allocatable :: EdgeAge !Maxime
  real*4 t1
  integer, parameter :: sp = selected_real_kind(6) ! ~6 digits single precision
  integer, parameter :: dp = selected_real_kind(15)
  integer nproc,iproc,nfault,npe,mpe,nsurf,nz,nelemsurf,nstep,ilog,iterative,interpol
  integer isoflag,nx,ny,nxiso,nyiso,niteradvec,istep,kstep
  integer i1,i2,i3,i4,iout,istatic,ntime,nnode,nelem,ie,iesurf,i,j,k,kk,nelemloc,je
  integer irec,jrec,in,itime,ij,iu,ic,ieobs,inp,iobs,krec,ktime,niter,nobs,nobs1,nobs2,nobs3,nobs4,nobs5,nobs6 !VKP
  integer ftlflag,mftflag,fltflag, heatprod_flag
  double precision x1,x2,x3,x4,x5,dt,time,alpha,zsurfmax,fact,tsurf,zh,ftime,ftimep,tprevious,tfake
  double precision eps,zl,diffusivity,heatproduction,zsurf1,zsurf2
  double precision tempsurf1,tempsurf2 !by VKP
  double precision tmax,tmsl,tlapse,tau,rhoc,rhom,xstep,ystep,young,poisson,thickness
  double precision D_heat !Maxime
  double precision xlonmin,xlonmax,xlatmin,xlatmax,xxx,yyy,zzz,vx,vy,vz,dxxx,dyyy,dzzz
  double precision xmin,xmax,ymin,ymax,timesurf,timesurfp,tfinal,dtimesurf
  double precision xlonobs,xlatobs,tnow,fmeano,dfmeano
  real *4 ageheobs,dageheobs
  real *4 ageftobs,dageftobs
  real *4 ageheZobs,dageheZobs
  real *4 ageftZobs,dageftZobs
  real *4 agearKobs,dagearKobs
  real *4 agearBobs,dagearBobs
  real *4 agearMobs,dagearMobs
  real *4 agearHobs,dagearHobs
  double precision tlobs(20),length(20)
  real*4,dimension(:),allocatable::ageft,agehe,ageheZ,ageftZ,agearK,agearB,agearM,agearH,hei,nNtl,nNosl,nNesr
  real*4,dimension(:),allocatable::ageESR, ageOSL
  real*4,dimension(:),allocatable::lonobs,latobs
  double precision wobs1,wobs2,wobs3,wobs4,x1f,x2f,y1f,y2f
  double precision def,dif,heightobs,friction
  double precision,dimension(:),allocatable::aheoreg,hheoreg,ahepreg
  double precision,dimension(:),allocatable::aftoreg,hftoreg,aftpreg
  double precision,dimension(:),allocatable::aheZoreg,hheZoreg,aheZpreg
  double precision,dimension(:),allocatable::aftZoreg,hftZoreg,aftZpreg
  double precision,dimension(:),allocatable::aarKoreg,harKoreg,aarKpreg
  double precision,dimension(:),allocatable::aarBoreg,harBoreg,aarBpreg
  double precision,dimension(:),allocatable::aarMoreg,harMoreg,aarMpreg
  double precision,dimension(:),allocatable::aarHoreg,harHoreg,aarHpreg
  double precision,dimension(:),allocatable::hpreg
  integer nhe,nft,nheZ,nftZ,narK,narB,narM,narH,ageflag(9),ageflagp(9)
  double precision aheom,hheom,heoi,heos
  double precision aftom,hftom,ftoi,ftos
  double precision aheZom,hheZom,heZoi,heZos
  double precision aftZom,hftZom,ftZoi,ftZos
  double precision aarKom,harKom,arKoi,arKos
  double precision aarBom,harBom,arBoi,arBos
  double precision aarMom,harMom,arMoi,arMos
  double precision aarHom,harHom,arHoi,arHos
  double precision ahepm,hhepm,hepi,heps
  double precision aftpm,hftpm,ftpi,ftps
  double precision aheZpm,hheZpm,heZpi,heZps
  double precision aftZpm,hftZpm,ftZpi,ftZps
  double precision aarKpm,harKpm,arKpi,arKps
  double precision aarBpm,harBpm,arBpi,arBps
  double precision aarMpm,harMpm,arMpi,arMps
  double precision aarHpm,harHpm,arHpi,arHps
  double precision volume_eroded(10000),surf_element,time_eroded(10000)
  integer nvolume
  double precision element_surface,volume_total
  integer check
 !       external element_surface
  integer agecomput!Maxime
  ! Maxime Helium thermochronometry
  real*4,dimension(:),allocatable :: AUppm, AThppm, ASize !Maxime
  real*4,dimension(:),allocatable :: ZUppm, ZThppm, ZSize !Maxime
  double precision rhoST !Maxime
  real *8 D0_spec, Ea_spec, D0z, Eaz !Maxime
  real *8 D0k,Eak,D0b,Eab,D0m,Eam,D0h,Eah
  integer Init_FTL_Model, kinFTLID
  integer RDmodel,RDmodelz, Alpha_Ejec_Flag !Maxime
  double precision Init_FTL_value
  double precision, dimension(:), allocatable :: kinFTL_AFT, kinFTL_AHe 
  ! Maxime - 4He/3He thermochronometer parameters
  real *4, dimension(:), allocatable :: age43He,dage43He, Uppm43_array, Thppm43_array, dummy43
  double precision, dimension(:), allocatable :: rmr043_array
  real *4, dimension(:), allocatable :: size43He_array
  real*4 size43He, Thppm43, Uppm43
  double precision  rmr043
  double precision, dimension(:,:), allocatable :: theating,duration
  double precision, dimension(:,:), allocatable :: released,dreleased,agereleased,dagereleased
  double precision, dimension(:,:), allocatable :: time43He,temperature43He
  double precision, dimension(:), allocatable :: releasedp,agereleasedp,He43_Ejec
  double precision, dimension(:), allocatable :: StepAgeObs,StepAgePred,relError
  double precision :: maxStepAgeObs,maxStepAgePred
  integer He43_flag, He43_inv_flag, nheating
  logical is_unix
      
  logical interpol_success,vivi,saveTt,saveCr !Maxime (saveCr)
  double precision,dimension(3,1000000)::data_ADAM_array ! Maxime
  character*50 :: sampleID !Maxime
  character*50,dimension(:),allocatable :: sampleID_array !Maxime
  double precision durationHeating(20),temp_heat(20)

  integer nd,nmisfit,nmisfit1,nmisfit2,nmisfit3,nmisfit4,nmisfit5,nmisfit6,nmisfit1a,nmisfit3a
  real*4 param(nd)
  real*4 misfit, datascore
  real*4 range(2,1024)
  real*4 misfit1, misfit2, misfit3, misfit4,misfit5,misfit6,misfit1a,misfit3a,datafit,rand_num
  real*4 misfitAHE, misfitAFT, misfitZHE, misfitZFT,misfitKAR,misfitBAR,misfitMAR,misfitHAR,misfitres
  real*4 misfitMFTL, misfitDMFTL
  real*4 LL_results
  real*4 LL_total,LL_AHE, LL_AFT, LL_ZHE, LL_ZFT,LL_KAR,LL_BAR,LL_MAR,LL_HAR,LL_ThL,LL_OSL,LL_ESR
  real*4 LL_MFTL,LL_DMFTL
  integer nmisfitAHE,nmisfitAFT,nmisfitZHE,nmisfitZFT,nmisfitKAR,nmisfitBAR,nmisfitMAR,nmisfitHAR,misfitflag
  integer nmisfitMFTL

  external hinterpolate,thermal_diffusivity,heat_production,Luminescence
  double precision hinterpolate,zmax,thermal_diffusivity,heat_production,dummy
  double precision, dimension(:,:), allocatable :: nnf_array,ageESR_array,ageOSL_array
  character*10000 line    
  integer counter

  call cpu_time (times(1))
  nproc=1

  eps=tiny(eps)

  do i=1,17
    length(i)=i
  enddo

 ! Pecube is a Finite Element solver of the 3D, transient heat transfer
 ! equation that allows for conduction, vertical advection and production of
 ! heat. Pecube also allows for the surface geometry to vary with time.

 ! This version is an improvement (we hope) of the original version
 ! available by download from the author's website in that it reads an
 ! input (topography) file as well as a set of cooling ages (He and FT in
 ! apatite of known location and calculates synthetic ages at the same
 ! locations for comparison.

 ! A flexural isostatic model has also been included to
 ! calculate the effect of isostasy in amplifying the exhumation caused
 ! by erosional unloading.

 ! Two dimensional faults have also been included; they can be orientated
 ! in any direction; more than one can be active at any given time and
 ! they advect each other (this only works for simple scenario)

 ! To understand how this version works, read the information in the file
 ! README

 ! comment this line out if you want Pecube to read your own input file
 ! (named Pecube.in)

  nd=0
  check = 0
  p%run_name = run
  call read_input_file (run//'/input/Pecube.in', 0, p, nd, range, param)

  if (nd.ne.0 .and. iproc.eq.0 .and. p%echo_input_file.gt.0 .and. p%inversion_mode.lt.1) then
   write (*,*) '---------------------------------------------------------------------------------'
   write (*,*) '------------------------Pecube inversion mode------------------------------------'
   write (*,*) '---------------------------------------------------------------------------------'
   write (*,*) 'Number of model parameters:',nd
      
    elseif (nd.ne.0 .and. iproc.eq.0 .and. p%echo_input_file.gt.0 .and. p%inversion_mode.eq.2) then
     write (*,*) '---------------------------------------------------------------------------------'
     write (*,*) '------------------------Pecube batch mode----------------------------------------'
     write (*,*) '---------------------------------------------------------------------------------'
  endif

  nfault = p%nfault
  if (nfault.eq.0) nfault=1

  if (nfault.gt.0) allocate (fault(nfault),faultp(nfault))
    ! Check operating system
    call GetOperatingSystem(is_unix)
  ! Store Pecube input parameters
  call create_pecube_in (fault,nfault,nd,range,param,run,iproc)

  call cpu_time (times(2))

 ! opens input files 

  rewind (7)

 ! read in general information

 ! first line:
 ! run: 5 character string that will determine the name of the folder where the input file
 !      will be copied to and where the output files (Pecube.ou and Pecube.ptt) will be stored.

  read (7,'(a)') run

 ! second line
 ! npe: number of nodes per surface (2D) elements (3 = triangular elements - 4 = rectangular elements)
 ! nsurf: number of nodes defining the surface geometry
 ! nz: number of nodes in the vertical direction
 ! nelemsurf: number of surface 2D elements
 ! zl: thickness of crustal layer (= depth at which t is fixed) (in km)
 ! diffusivity: thermal diffusivity (in km^2/Myr)
 ! heatproduction: heat production (in degC/Myr)

  read (7,*) npe,nsurf,nz,nelemsurf,zl,diffusivity,heatproduction,friction,heatprod_flag
  vivi=.FALSE.
  if (nsurf.lt.0) then
    nsurf=-nsurf
    vivi=.TRUE.
  endif
  mpe=2*npe

 ! second line:
 ! tmax: basal temperature (at z=-zl) (in degC)
 ! tmsl: temperature at mean sea level (z=0) (in degC)
 ! tlapse: lapse rate (indegC/km)
 ! nstep: number of stages (the surface topo and exhumatoin rate can be specified at each time stage)
 ! ilog: redundant (dont use)
 ! iterative: iterative solver method (1 = Gauss Siedel with overrelaxation - 2 = Conjugate gradient)
 ! interpol: interpolation used (1 = linear - 2 = quadratic, not recommended)
 ! D_heat: e-folding depth

  read (7,*) tmax,tmsl,tlapse,nstep,ilog,iterative,interpol,tprevious,ftlflag,mftflag,fltflag,D_heat
  read (7,*) isoflag,tau,rhoc,rhom
  read (7,*) nx,ny,nxiso,nyiso
  read (7,*) xstep,ystep,young,poisson,thickness

  allocate (iconsurf(npe,nelemsurf),ielsurf(nsurf))
  allocate (neighbour(npe,nelemsurf))
  allocate (xsurf(nsurf),ysurf(nsurf),zsurf(nsurf),zsurfp(nsurf))
  allocate (usurf(nsurf),usurfp(nsurf),tempsurf(nsurf),tempsurfp(nsurf)) !VKP
  allocate (topoa(nsurf),topob(nsurf),rsurf(nsurf))
  allocate (tprev(nsurf,nstep))
  allocate (xdepth(nsurf,nstep),ydepth(nsurf,nstep),zdepth(nsurf,nstep))
  allocate (xexhumation(nsurf,nstep),yexhumation(nsurf,nstep),zexhumation(nsurf,nstep))
  allocate (lsf(nsurf,nz),lsfx(nsurf,nz),lsfy(nsurf,nz))

  tprev=0.d0
  usurf=0.d0 !VKP
  usurfp=0.d0 !VKP
  tempsurf=0.d0 !VKP
  tempsurfp=0.d0 !VKP

  if (nd.eq.0.and.ilog.eq.1) open (9,file=run//'/LOG/Pecube.log',status='unknown')
  if (nd.eq.0.and.ilog.eq.1) write (9,*) 'This is Pecube version ',vers%str
  if (nd.eq.0.and.ilog.eq.1) write (9,*) 'Reading Geometry'

 ! read in nodal geometry

  read (7,*) xlonmin,xlonmax,xlatmin,xlatmax

 ! xsurf, ysurf: x- and y-locations of the surface nodes
 ! iconsurf: connectivity matrix describing the 2D surface elements

  read (7,*) (xsurf(i),ysurf(i),i=1,nsurf)
  read (7,*) ((iconsurf(i,j),i=1,npe),j=1,nelemsurf)

    ! ! read new parameters for sample-specific predictions - Maxime Bernard 2021
    read (7,*) agecomput,saveCr
    read (7,*) D0_spec,Ea_spec,RDmodel,Alpha_Ejec_Flag,D0z,Eaz,RDmodelz
        
  zmax=0.d0
  do kstep=0,nstep
    read (7,*) timesurfp,iout
    read (7,*) (zsurfp(i),i=1,nsurf)
    zmax=max(maxval(zsurfp),zmax)
    if (vivi) read (7,*) (usurfp(i),i=1,nsurf)  !VKP
    if (vivi) read (7,*) (tempsurfp(i),i=1,nsurf)  !VKP
  enddo
      
  if (nd.eq.0.and.ilog.eq.1) write (9,*) 'Advecting rocks'

  xmin=minval(xsurf)
  xmax=maxval(xsurf)
  ymin=minval(ysurf)
  ymax=maxval(ysurf)

  do k=1,nsurf
    lsfx(k,:)=xsurf(k)
    lsfy(k,:)=ysurf(k)
  enddo

  do k=1,nz
    lsf(:,k)=zl-(zl+zmax)*float(k-1)/(nz-1)
  enddo

  nvolume=0
        
 ! go through the input file a first time to determine the depth of the
 ! points that will end up at the surface at the end of the model run
  if (p%echo_input_file.gt.0.and.nd.eq.0) then !Maxime
   write (*,*) '---------------------------------------------------------------------------------'
   write (*,*) '-------------------Backward advecting surface nodes------------------------------'
   write (*,*) '---------------------------------------------------------------------------------'
  endif
  niteradvec=3

  do istep=1,nstep
    xexhumation(1:nsurf,istep)=xsurf
    yexhumation(1:nsurf,istep)=ysurf
    zexhumation(1:nsurf,istep)=zl
  enddo

  ! Loop backward in time
  do istep=nstep,1,-1
    if (iproc.eq.0.and.nd.eq.0.and.p%echo_input_file.gt.0) write (6,*) 'Time interval ',istep
    timesurfp=0.
    rewind (7)
    read (7,*)
    read (7,*) i1,i2,i3,i4,x1,x2,x3,x4
    read (7,*) x1,x2,x3,i1,i2,i3,i4,x1,i2,i3,i4
    read (7,*) i1,x1,x2,x3
    read (7,*) i1,i2,i3,i4
    read (7,*) x1,x2,x3,x4,x5
    read (7,*) x1,x2,x3,x4
    read (7,*) (xsurf(i),ysurf(i),i=1,nsurf)
    read (7,*) ((iconsurf(i,j),i=1,npe),j=1,nelemsurf)
    read (7,*) agecomput,saveCr!Maxime
    read (7,*) D0_spec,Ea_spec,RDmodel,Alpha_Ejec_Flag,D0z,Eaz,RDmodelz
    ! For each topography evolution step
    do kstep=0,istep-1
      read (7,*) timesurfp,iout
      read (7,*) (zsurfp(i),i=1,nsurf)
      if (vivi) read (7,*) (usurfp(i),i=1,nsurf)  !VKP
      if (vivi) read (7,*) (tempsurfp(i),i=1,nsurf)  !VKP
    enddo
    ! read last topography (present-day)
    read (7,*) timesurf,iout
    read (7,*) (zsurf(i),i=1,nsurf)
    if (vivi) read (7,*) (usurf(i),i=1,nsurf) !VKP
    if (vivi) read (7,*) (tempsurf(i),i=1,nsurf) !VKP
    if (istep.eq.nstep) then
      read (7,*) nobs1,nobs2,nobs3,nobs4,nobs5,nobs6
      saveTt=.FALSE.
      if (nobs1.lt.0.and.nd.eq.0) then
        saveTt=.TRUE.
      endif
      if (nobs1.lt.0) nobs1=-nobs1
      if (nobs2.lt.0) nobs2=-nobs2
      if (nobs3.lt.0) nobs3=-nobs3
      if (nobs4.lt.0) nobs4=-nobs4
      nobs=nobs1+nobs2+nobs3+nobs4+nobs5+nobs6

      allocate (xdeptho(nobs),ydeptho(nobs),zdeptho(nobs))
      allocate (zsurfo(nobs),zsurfpo(nobs))
      allocate (xexhumationo(nobs),yexhumationo(nobs),zexhumationo(nobs),grainobs(nobs))
      ! allocate (ASize(nsurf),AUppm(nsurf),AThppm(nsurf),rmr0(nsurf),ZUppm(nsurf),ZThppm(nsurf),ZSize(nsurf))
      allocate (tprevo(nobs),ieo(nobs),we1(nobs),we2(nobs),we3(nobs),we4(nobs))
      allocate (nhist(nobs),thist(1024,nobs),temphist(1024,nobs),errortemphist(1024,nobs))
      allocate (nheating_array(nobs),theating(1024,nobs),released(1024,nobs),dreleased(1024,nobs),duration(1024,nobs))
      allocate (agereleased(1024,nobs),dagereleased(1024,nobs))
      allocate (size43He_array(nobs),age43He(nobs),dage43He(nobs),Uppm43_array(nobs),Thppm43_array(nobs),&
         dummy43(nobs),rmr043_array(nobs))
      allocate (doser(nobs),d0(nobs),radius(nobs),et(nobs),logs(nobs),b(nobs))
      allocate (logrho(nobs),nn(nobs),dnn(nobs),eu(nobs),sigmaet(nobs),heighto(nobs),sampleID_array(nsurf),imax(nobs),&
            a_coef(nobs),GOK_a(nobs), GOK_b(nobs))

      ! Initialize arrays
      size43He_array = 0.d0;age43He = 0.d0;dage43He = 0.d0;Uppm43_array = 0.d0;Thppm43_array = 0.d0
      dummy43 = 0.d0;rmr043_array = 0.d0
      xexhumationo = 0.d0;yexhumationo = 0.d0;zexhumationo = 0.d0
      we1 = 0.d0;we2 = 0.d0;we3 = 0.d0;we4 = 0.d0
      eu = 0.d0; et=0.d0;
      errortemphist = 0.0; duration=0.d0
      ! Get predicted position and elevation of samples from surface topography (input are elevation of samples, even from tunnel of tunnel sample)
      do i=1,nobs1
        read (7,*) sampleID,xlonobs,xlatobs,ageheobs,dageheobs,ageftobs,dageftobs, & ! By Xav
                    ageheZobs,dageheZobs,ageftZobs,dageftZobs,agearKobs,dagearKobs, &
                    agearBobs,dagearBobs,agearMobs,dagearMobs,agearHobs,dagearHobs, &
                    tlobs,heightobs,fmeano,dfmeano,grainobs(i),ieo(i),we1(i),we2(i),we3(i),we4(i)
        xexhumationo(i)=we1(i)*xsurf(iconsurf(1,ieo(i))) &
                        +we2(i)*xsurf(iconsurf(2,ieo(i))) &
                        +we3(i)*xsurf(iconsurf(3,ieo(i))) &
                        +we4(i)*xsurf(iconsurf(npe,ieo(i)))
        yexhumationo(i)=we1(i)*ysurf(iconsurf(1,ieo(i))) &
                        +we2(i)*ysurf(iconsurf(2,ieo(i))) &
                        +we3(i)*ysurf(iconsurf(3,ieo(i))) &
                        +we4(i)*ysurf(iconsurf(npe,ieo(i)))
        zexhumationo(i)=we1(i)*zsurf(iconsurf(1,ieo(i))) &
                        +we2(i)*zsurf(iconsurf(2,ieo(i))) &
                        +we3(i)*zsurf(iconsurf(3,ieo(i))) &
                        +we4(i)*zsurf(iconsurf(npe,ieo(i)))
                if (heightobs.lt.0.d0) then
                zexhumationo(i)= zexhumationo(i) -&
                        (min(0.d0,heightobs/1.e3) + (we1(i)*zsurf(iconsurf(1,ieo(i))) &
                +we2(i)*zsurf(iconsurf(2,ieo(i))) &
                +we3(i)*zsurf(iconsurf(3,ieo(i))) &
                +we4(i)*zsurf(iconsurf(npe,ieo(i)))))
              endif
               zexhumationo(i)= zexhumationo(i) + zl
      enddo

      do i=nobs1+1,nobs1+nobs2
        read (7,*) xlonobs,xlatobs,nhist(i),(thist(j,i),temphist(j,i),errortemphist(j,i),j=1,nhist(i)), &
                       heightobs,ieo(i),we1(i),we2(i),we3(i),we4(i)   ! By Xav
        xexhumationo(i)=we1(i)*xsurf(iconsurf(1,ieo(i))) &
                        +we2(i)*xsurf(iconsurf(2,ieo(i))) &
                        +we3(i)*xsurf(iconsurf(3,ieo(i))) &
                        +we4(i)*xsurf(iconsurf(npe,ieo(i)))
        yexhumationo(i)=we1(i)*ysurf(iconsurf(1,ieo(i))) &
                        +we2(i)*ysurf(iconsurf(2,ieo(i))) &
                        +we3(i)*ysurf(iconsurf(3,ieo(i))) &
                        +we4(i)*ysurf(iconsurf(npe,ieo(i)))
        zexhumationo(i)=we1(i)*zsurf(iconsurf(1,ieo(i))) &
                        +we2(i)*zsurf(iconsurf(2,ieo(i))) &
                        +we3(i)*zsurf(iconsurf(3,ieo(i))) &
                        +we4(i)*zsurf(iconsurf(npe,ieo(i)))
              if (heightobs.lt.0.d0) then
                  zexhumationo(i)= zexhumationo(i) -&
                           (min(0.d0,heightobs/1.e3) + (we1(i)*zsurf(iconsurf(1,ieo(i))) &
                      +we2(i)*zsurf(iconsurf(2,ieo(i))) &
                      +we3(i)*zsurf(iconsurf(3,ieo(i))) &
                      +we4(i)*zsurf(iconsurf(npe,ieo(i)))))
              endif
              zexhumationo(i)= zexhumationo(i) + zl
      enddo

      do i=nobs1+nobs2+1,nobs1+nobs2+nobs3
        read (7,*) sampleID_array(i),xlonobs,xlatobs,size43He_array(i),age43He(i),dage43He(i),nheating_array(i), &
                  Uppm43_array(i),Thppm43_array(i),rmr043_array(i),(theating(j,i),duration(j,i),&
                  released(j,i),dreleased(j,i), &
                  agereleased(j,i),dagereleased(j,i),j=1,nheating_array(i)), &
                  heightobs,ieo(i),we1(i),we2(i),we3(i),we4(i)   ! By Xav and Maxime
        xexhumationo(i)=we1(i)*xsurf(iconsurf(1,ieo(i))) &
                    +we2(i)*xsurf(iconsurf(2,ieo(i))) &
                    +we3(i)*xsurf(iconsurf(3,ieo(i))) &
                    +we4(i)*xsurf(iconsurf(npe,ieo(i)))
        yexhumationo(i)=we1(i)*ysurf(iconsurf(1,ieo(i))) &
                    +we2(i)*ysurf(iconsurf(2,ieo(i))) &
                    +we3(i)*ysurf(iconsurf(3,ieo(i))) &
                    +we4(i)*ysurf(iconsurf(npe,ieo(i)))
        zexhumationo(i)=we1(i)*zsurf(iconsurf(1,ieo(i))) &
                    +we2(i)*zsurf(iconsurf(2,ieo(i))) &
                    +we3(i)*zsurf(iconsurf(3,ieo(i))) &
                    +we4(i)*zsurf(iconsurf(npe,ieo(i))) 
              if (heightobs.lt.0.d0) then
                  zexhumationo(i)= zexhumationo(i) -&
                          (min(0.d0,heightobs/1.e3) + (we1(i)*zsurf(iconsurf(1,ieo(i))) &
                    +we2(i)*zsurf(iconsurf(2,ieo(i))) &
                    +we3(i)*zsurf(iconsurf(3,ieo(i))) &
                    +we4(i)*zsurf(iconsurf(npe,ieo(i)))))
        endif
              zexhumationo(i)= zexhumationo(i) + zl
      enddo

            !TL
      do i=nobs1+nobs2+nobs3+1,nobs1+nobs2+nobs3+nobs4
        read (7,*) sampleID_array(i),xlonobs,xlatobs, &
                  doser(i),d0(i),radius(i),et(i), &
                  logs(i),b(i),logrho(i),nn(i),dnn(i), &
                  heightobs,ieo(i),we1(i),we2(i),we3(i),we4(i)   ! By Xav
        xexhumationo(i)=we1(i)*xsurf(iconsurf(1,ieo(i))) &
                    +we2(i)*xsurf(iconsurf(2,ieo(i))) &
                    +we3(i)*xsurf(iconsurf(3,ieo(i))) &
                    +we4(i)*xsurf(iconsurf(npe,ieo(i)))
        yexhumationo(i)=we1(i)*ysurf(iconsurf(1,ieo(i))) &
                    +we2(i)*ysurf(iconsurf(2,ieo(i))) &
                    +we3(i)*ysurf(iconsurf(3,ieo(i))) &
                    +we4(i)*ysurf(iconsurf(npe,ieo(i)))
        zexhumationo(i)=we1(i)*zsurf(iconsurf(1,ieo(i))) &
                    +we2(i)*zsurf(iconsurf(2,ieo(i))) &
                    +we3(i)*zsurf(iconsurf(3,ieo(i))) &
                    +we4(i)*zsurf(iconsurf(npe,ieo(i)))
              if (heightobs.lt.0.d0) then
                  zexhumationo(i)= zexhumationo(i) -&
                           (min(0.d0,heightobs/1.e3) + (we1(i)*zsurf(iconsurf(1,ieo(i))) &
                  +we2(i)*zsurf(iconsurf(2,ieo(i))) &
                  +we3(i)*zsurf(iconsurf(3,ieo(i))) &
                  +we4(i)*zsurf(iconsurf(npe,ieo(i)))))
              endif
              zexhumationo(i)= zexhumationo(i) + zl
      enddo
            !OSL
            do i=nobs1+nobs2+nobs3+nobs4+1,nobs1+nobs2+nobs3+nobs4+nobs5
        read (7,*) sampleID_array(i),xlonobs,xlatobs, &
                  doser(i),d0(i),et(i), &
                  logs(i),logrho(i),eu(i),nn(i),dnn(i),a_coef(i),imax(i), &
                  heightobs,ieo(i),we1(i),we2(i),we3(i),we4(i)   ! By Xav
        xexhumationo(i)=we1(i)*xsurf(iconsurf(1,ieo(i))) &
                    +we2(i)*xsurf(iconsurf(2,ieo(i))) &
                    +we3(i)*xsurf(iconsurf(3,ieo(i))) &
                    +we4(i)*xsurf(iconsurf(npe,ieo(i)))
        yexhumationo(i)=we1(i)*ysurf(iconsurf(1,ieo(i))) &
                    +we2(i)*ysurf(iconsurf(2,ieo(i))) &
                    +we3(i)*ysurf(iconsurf(3,ieo(i))) &
                    +we4(i)*ysurf(iconsurf(npe,ieo(i)))
        zexhumationo(i)=we1(i)*zsurf(iconsurf(1,ieo(i))) &
                    +we2(i)*zsurf(iconsurf(2,ieo(i))) &
                    +we3(i)*zsurf(iconsurf(3,ieo(i))) &
                    +we4(i)*zsurf(iconsurf(npe,ieo(i)))
                if (heightobs.lt.0.d0) then
                  zexhumationo(i)= zexhumationo(i) - &
                           (min(0.d0,heightobs/1.e3) + (we1(i)*zsurf(iconsurf(1,ieo(i))) &
                    +we2(i)*zsurf(iconsurf(2,ieo(i))) &
                    +we3(i)*zsurf(iconsurf(3,ieo(i))) &
                    +we4(i)*zsurf(iconsurf(npe,ieo(i)))))
              endif
              zexhumationo(i)= zexhumationo(i) + zl
      enddo
            !ESR
      do i=nobs1+nobs2+nobs3+nobs4+nobs5+1,nobs1+nobs2+nobs3+nobs4+nobs5+nobs6
        read (7,*) sampleID_array(i),xlonobs,xlatobs, &
                  doser(i),d0(i), &
                  logs(i),et(i),sigmaet(i),nn(i),dnn(i),GOK_a(i),GOK_b(i),&
                  imax(i),heightobs,ieo(i),we1(i),we2(i),we3(i),we4(i) 
        xexhumationo(i)=we1(i)*xsurf(iconsurf(1,ieo(i))) &
                      +we2(i)*xsurf(iconsurf(2,ieo(i))) &
                      +we3(i)*xsurf(iconsurf(3,ieo(i))) &
                      +we4(i)*xsurf(iconsurf(npe,ieo(i)))
        yexhumationo(i)=we1(i)*ysurf(iconsurf(1,ieo(i))) &
                      +we2(i)*ysurf(iconsurf(2,ieo(i))) &
                      +we3(i)*ysurf(iconsurf(3,ieo(i))) &
                      +we4(i)*ysurf(iconsurf(npe,ieo(i)))
        zexhumationo(i)=we1(i)*zsurf(iconsurf(1,ieo(i))) &
                      +we2(i)*zsurf(iconsurf(2,ieo(i))) &
                      +we3(i)*zsurf(iconsurf(3,ieo(i))) &
                      +we4(i)*zsurf(iconsurf(npe,ieo(i)))
                if (heightobs.lt.0.d0) then
                  zexhumationo(i)= zexhumationo(i) -&
                           (min(0.d0,heightobs/1.e3) + (we1(i)*zsurf(iconsurf(1,ieo(i))) &
                    +we2(i)*zsurf(iconsurf(2,ieo(i))) &
                    +we3(i)*zsurf(iconsurf(3,ieo(i))) &
                    +we4(i)*zsurf(iconsurf(npe,ieo(i)))))
              endif
              zexhumationo(i)= zexhumationo(i) + zl
      enddo  

      read (7,*) ageflag
          ! print *, 'zsurf = ', zexhumationo
    endif
          
        
   !isostatic rebound
    topoa=zsurfp
    topob=zsurf
    if (isoflag.eq.1) then
      call isostatic_rebound (topoa,topob,xsurf,ysurf,nsurf,rsurf, &
                              rhoc,rhom,nx,ny,nxiso,nyiso, &
                              xstep,ystep,young,poisson,thickness)
    else
      rsurf=0.
    endif
    zexhumation(1:nsurf,istep)=zexhumation(1:nsurf,istep)+zsurf
    if (istep.eq.nstep) tfinal=timesurf
    call find_dt (zl,diffusivity,nsurf,zsurf,zsurfp, &
                  nz,timesurf,timesurfp,istep,eps,ilog,nd, &
                  dt,ntime,istatic,fault,nfault,usurf,p%min_dt)

    if (nd.eq.0 .and. p%echo_input_file.gt.0) write (*,*) ntime,'time steps required'

    do ktime=1,ntime
      time=timesurf-dt*ktime
      !time=max(time,1.d-6)
      do kstep=istep,nstep
        do i=1,nsurf
          xxx=xexhumation(i,kstep)
          yyy=yexhumation(i,kstep)
          zzz=zexhumation(i,kstep)
          do k=1,niteradvec
            call find_velo (xxx,yyy,zzz,time,-dt,fault,nfault,vx,vy,vz,xmin,xmax,ymin,ymax,.true.)
            dxxx=xexhumation(i,kstep)-dt*vx/2.-xxx
            dyyy=yexhumation(i,kstep)-dt*vy/2.-yyy
            dzzz=zexhumation(i,kstep)-dt*vz/2.-zzz
            xxx=xxx+dxxx
            yyy=yyy+dyyy
            zzz=zzz+dzzz
          enddo
          xexhumation(i,kstep)=xexhumation(i,kstep)-dt*vx
          yexhumation(i,kstep)=yexhumation(i,kstep)-dt*vy
          zexhumation(i,kstep)=zexhumation(i,kstep)-dt*vz-rsurf(i)/ntime-dt*usurf(i) !VKP
        enddo
      enddo

       ! lines to compute volume eroded
        ! nvolume=nvolume+1
        ! if (nvolume.gt.10000) stop 'nvolume > 10000'
        ! volume_eroded(nvolume)=0.d0
          ! do i=1,nsurf
          ! surf_element=element_surface(npe,iconsurf,xsurf,ysurf,nsurf)
          ! dtimesurf=timesurf-timesurfp
          ! if (dtimesurf.gt.0.d0) volume_eroded=volume_eroded-(zsurf(i)-zsurfp(i))/dtimesurf*surf_element
          ! call find_velo (xsurf(i),ysurf(i),zl+zsurf(i),time,dt,fault,nfault,vx,vy,vz,xmin,xmax,ymin,ymax,.true.)
          ! volume_eroded(nvolume)=volume_eroded(nvolume)+(vz+rsurf(i)/dtimesurf+usurf(i))*surf_element
          ! enddo
        ! time_eroded(nvolume)=tfinal-time
       ! end lines to compute volume eroded

      do i=1,nobs
        xxx=xexhumationo(i)
        yyy=yexhumationo(i)
        zzz=zexhumationo(i)
        do k=1,niteradvec
          call find_velo (xxx,yyy,zzz,time,-dt,fault,nfault,vx,vy,vz,xmin,xmax,ymin,ymax,.true.)
          dxxx=xexhumationo(i)-dt*vx/2.-xxx
          dyyy=yexhumationo(i)-dt*vy/2.-yyy
          dzzz=zexhumationo(i)-dt*vz/2.-zzz
          xxx=xxx+dxxx
          yyy=yyy+dyyy
          zzz=zzz+dzzz
        enddo
        xexhumationo(i)=xexhumationo(i)-dt*vx
        yexhumationo(i)=yexhumationo(i)-dt*vy
        zexhumationo(i)=zexhumationo(i)-dt*vz-we1(i)*(rsurf(iconsurf(1,ieo(i)))/ntime+dt*usurf(iconsurf(1,ieo(i)))) &
                        -we2(i)*(rsurf(iconsurf(2,ieo(i)))/ntime+dt*usurf(iconsurf(2,ieo(i)))) &
                        -we3(i)*(rsurf(iconsurf(3,ieo(i)))/ntime+dt*usurf(iconsurf(3,ieo(i)))) &
                        -we4(i)*(rsurf(iconsurf(npe,ieo(i)))/ntime+dt*usurf(iconsurf(npe,ieo(i))))
      enddo

      if (fltflag.eq.1) call move_fault (fault,faultp,nfault,-dt,time,xmin,xmax,ymin,ymax)
    enddo
        ! print *, 'zsample = ', zexhumationo, vz, dt, nobs
    zsurfp=zsurf
    timesurfp=timesurf
  enddo
         

  if (p%save_eroded_volume.eq.1) then
    open (43,file=run//'/output/VolumeEroded.csv',status='unknown')
    write (43,'(a)') 'Interval_start,Interval_End,Flux,Accumulated_Volume'
    volume_total=0.d0
    do i=nvolume,2,-1
      volume_total=volume_total+volume_eroded(i)*(time_eroded(i)-time_eroded(i-1))
      write (43,'(g12.6,",",g12.6,",",g12.6,",",g12.6)') time_eroded(i),time_eroded(i-1),volume_eroded(i),volume_total
    enddo
    volume_total=volume_total+volume_eroded(1)*time_eroded(1)
    write (43,'(g12.6,",",g12.6,",",g12.6,",",g12.6)') time_eroded(1),0.d0,volume_eroded(1),volume_total
    close (43)
  endif
    
  ! depth is the initial depth of the rocks that end up at the location of the
  ! surface nodes at the end of the model experiment

  xdepth=xexhumation
  ydepth=yexhumation
  zdepth=zexhumation
  xdeptho=xexhumationo
  ydeptho=yexhumationo
  zdeptho=zexhumationo
      ! print *, 'initial depth = ',zexhumationo(1:nobs)

  if (nd.eq.0 .and. p%echo_input_file.gt.0) then
    write (*,*) '---------------------------------------------------------------------------------'
  endif

  ! reset the input file to its proper position
  ! read new parameters for sample-specific predictions - Maxime Bernard 2021

  if (nd.eq.0 .and. p%echo_input_file.gt.0) write (*,*) ''

  rewind (7)
  read (7,'(a)') run
  read (7,*) npe,nsurf,nz,nelemsurf,zl,diffusivity,heatproduction,friction,heatprod_flag
  nsurf=iabs(nsurf)
  read (7,*) tmax,tmsl,tlapse,nstep,ilog,iterative,interpol,tprevious,ftlflag,mftflag,fltflag,D_heat
  nstep=abs(nstep)
  read (7,*) isoflag,tau,rhoc,rhom
  read (7,*) nx,ny,nxiso,nyiso
  read (7,*) xstep,ystep,young,poisson,thickness
  read (7,*) xlonmin,xlonmax,xlatmin,xlatmax
  read (7,*) (xsurf(i),ysurf(i),i=1,nsurf)
  read (7,*) ((iconsurf(i,j),i=1,npe),j=1,nelemsurf)
      
  ! ! read new parameters for sample-specific predictions - Maxime Bernard 2021
  read (7,*) agecomput,saveCr
  read (7,*) D0_spec,Ea_spec,RDmodel,Alpha_Ejec_Flag,D0z,Eaz,RDmodelz

  nnode=nsurf*nz
  nelem=nelemsurf*(nz-1)
  if (nd.eq.0.and.ilog.eq.1) write (9,*) 'nnode/nelem= ',nnode,nelem

  ! opens output files

  ! Pecube.out contains the temperature field at the end of each stage

  if (iproc.eq.0.and.nd.eq.0) open (8,file=run//'/output/Pecube.out',status='unknown',access='direct', &
            recl=4*(4+12*nnode+mpe*nelem))

  ! Ages.out contains the ages at the end of each stage

  if (iproc.eq.0.and.nd.eq.0) open (11,file=run//'/output/Ages.out',status='unknown',access='direct', &
        recl=4*(4+17*nsurf+npe*nelemsurf))

  ! Pecube.ptt contains the depth-temperture-paths of all surface nodes
    if (iproc.eq.0.and.saveCr) open (1192,status='scratch',access='direct', &
                 recl=4*(1+nsurf*4))
            
  if (nd.eq.0.and.ilog.eq.1) write (9,*) 'Start of time stepping'
      
  do istep=1,nstep
    open (100+istep,status='scratch',access='direct', &
            recl=4*(1+nsurf*4))
  enddo

  open (500,status='scratch',access='direct', &
        recl=4*(1+nobs*4))

  allocate (x(nnode),y(nnode),z(nnode),t(nnode))
  allocate (u(nnode)) !VKP
  allocate (xp(nnode),yp(nnode),zp(nnode),tp(nnode))
  allocate (up(nnode)) !VKP
  allocate (icon(mpe,nelem))
  allocate (kfix(nnode))
  allocate (f(nnode),rebound(nnode))

  u=0.d0
  up=0.d0

  ! build 3D element connectivity

  ie=0
  do iesurf=1,nelemsurf
    do k=1,(nz-1)
      ie=ie+1
      do kk=1,npe
        icon(kk,ie)=(iconsurf(kk,iesurf)-1)*nz+k
        icon(kk+npe,ie)=icon(kk,ie)+1
      enddo
    enddo
  enddo

  if (ie.ne.nelem) then
    stop 'nelem mismatch'
  endif

  ! finds processor topology

  allocate (proc(nnode))
  proc=0
  nelemloc = nelem

  allocate (ael(mpe,mpe,nelemloc),bel(mpe,nelemloc),ice(nelemloc))

  je=0
  do ie=1,nelem
    je=je+1
    ice(je)=ie
  enddo

  if (nd.eq.0.and.ilog.eq.1) then
    write (9,*) 'icon'
    do ie=1,nelem
      write (9,*) (icon(k,ie),k=1,mpe)
      enddo
  endif

  ! finds neighbour conectivity matrix

  call find_neighbours (iconsurf,neighbour,npe,nelemsurf,nsurf)
      ielsurf=1

  ! initialize global parameters
  ! alpha is the time integration parameter

  alpha=0.5
  time=0.
  timesurf=0.
  irec=0
  jrec=0

  ! If use of ADAM Radiation damage model, load data. Maxime
  data_ADAM_array = 0.d0
  if (RDModel.eq.4) then
      open (17, file='src/Willett_data.txt',status='old',action='read')
      !First row = EDD; Second row = Ea, third row = ln(D0/a²)
      read (17,*) data_ADAM_array
      close (17)
  endif

  if (p%Echo_input_file.gt.0.and.nd.eq.0) then
    write (*,*) '-----------------------------Time stepping---------------------------------------'
    write (*,*) '---------------------------------------------------------------------------------'
  endif
  ! begining of surface stepping (stageing)
  do istep=0,nstep

    if (nd.eq.0.and.ilog.eq.1) write (9,*) 'istep= ',istep

    if (nd.eq.0.and.p%echo_input_file.gt.0) write (6,*) 'Interval : ',istep,' of ',nstep

    if (istep.ne.0) then
       zsurfp=zsurf
       usurfp=usurf !VKP
       tempsurfp=tempsurf !VKP
    endif
    timesurfp=timesurf

    ! read the step information

    if (nd.eq.0.and.ilog.eq.1) write (9,*) 'Reading step information'

    ! timesurf is the time since the begining of the experiment
    ! Peclet is the exhumation velocity since the end of the last stage
    ! iout indicates if the T information is to be saved at the end of the stage

    read (7,*) timesurf,iout
    if (istep.eq.0) then
      if (timesurf.gt.eps.and.iproc.eq.0.and.p%echo_input_file.gt.0) then
        write (6,*) 'timesurf= ',timesurf
        write (6,*) 'first topography record must be at time zero ...'
        stop
      endif
    endif

    read (7,*) (zsurf(i),i=1,nsurf)

    if (vivi) then
      read (7,*) (usurf(i),i=1,nsurf) !VKP
      read (7,*) (tempsurf(i),i=1,nsurf) !VKP
    endif
      
    ! initial (or zeroth) step

    if (istep.eq.0) then
      zsurfp=zsurf
      usurfp=usurf !VKP
      tempsurfp=tempsurf !VKP

      zsurfmax=maxval(zsurf)
      in=0
      do i=1,nsurf
        do k=1,nz
          in=in+1
          fact=float(k-1)/float(nz-1)
          if (interpol.eq.2.and.fact.gt.eps) fact=sqrt(fact)
          !  Elevation from the base of the crust
          zh=zsurf(i)+zl
          zp(in)=zh*fact ! Depth from the surface

          xp(in)=xsurf(i)
          x(in)=xp(in)
          yp(in)=ysurf(i)
          y(in)=yp(in)
          up(in)=usurf(i)
          u(in)=up(in)
          kfix(in)=0
          if (k.eq.1 .or. k.eq.nz) kfix(in)=1
        enddo
      enddo

      ! calculates initial temperature

      if (nd.eq.0.and.ilog.eq.1) write (9,*) 'Calculating initial temperature'

      do i=1,nsurf
        tsurf=-zsurf(i)*tlapse+tmsl
        ! If from spm 
        if (vivi) tsurf = tempsurf(i) ! get surface temperature
        do k=1,nz
          in=(i-1)*nz+k
          zh=zsurf(i)+zl
          ! interploate temperature up to the base of the crust
          tp(in)=tsurf+(tmax-tsurf)*(zh-zp(in))/zh
        enddo
      enddo
      ! profile of temperature at depth for point i
      t=tp

      if (nd.eq.0.and.ilog.eq.1) then
        write (9,*) 'Nodal geometry'
        do i=1,nnode
          write (9,*) i,xp(i),yp(i),zp(i),tp(i),kfix(i)
        enddo
      endif

    endif

    if (nd.eq.0) then
      print *, ''
      print '(A18,F6.3,A2,F6.3,A5)', '  time interval = ', tfinal-timesurfp,&
          ' -',tfinal-timesurf, ' Myrs'
    endif

      ! print *, 'timesurf = ', timesurf, timesurfp
    call find_dt (zl,diffusivity,nsurf,zsurf,zsurfp, &
                  nz,timesurf,timesurfp,istep,eps,ilog,nd, &
                  dt,ntime,istatic,fault,nfault,usurf,p%min_dt)
        ! print *, 'dt = ', dt

    ! beginning of time stepping ---------------------------------------------------------------------
    if (nd.eq.0.and.ilog.eq.1) write (9,*) 'Begining of time stepping'

    do itime=1,ntime
      call cpu_time (times(3))
      time=time+dt
      ftime=float(itime)/float(ntime)
      ftimep=float(itime-1)/float(ntime)
      ! Mod made by Jean on 15/3/2010
      ! now one should use tau=0 to indicate a linear change in topography
      ! a positive value for a change late in the time step
      ! a negative value for a change early in the time step (most meaningful geomorphologically)
      if (tau.ne.0.d0) then
        ftime=(1.-exp(-ftime*tau/(timesurf-timesurfp)))/(1.-exp(-tau/(timesurf-timesurfp)))
        ftimep=(1.-exp(-ftimep*tau/(timesurf-timesurfp)))/(1.-exp(-tau/(timesurf-timesurfp)))
      endif

      if (nd.eq.0.and.ilog.eq.1) write (9,*) 'itime,time= ',itime,time

      ! build new node geometry

      if (nd.eq.0.and.ilog.eq.1) write (9,*) 'Build new node geometry'

      z=zp
      ! Get temperature below surface
      do i=1,nsurf
        in=i*nz
        inp=i*nz-1
        zsurf1=zsurfp(i)+(zsurf(i)-zsurfp(i))*ftime+zl
        zsurf2=zsurfp(i)+(zsurf(i)-zsurfp(i))*ftimep+zl
        topoa(i)=zsurf2
        topob(i)=zsurf1
        z(in)=z(in)+zsurf1-zsurf2
        z(inp)=z(inp)+(zsurf1-zsurf2)/2.
        ! Mod made by Jean on 23/3/2010
        ! This insures that the surface temperature follows the lapse rate at every time step
        ! not just at t=0
        tp(in)=-(zsurfp(i)+(zsurf(i)-zsurfp(i))*ftimep)*tlapse+tmsl
        t(in)=-(zsurfp(i)+(zsurf(i)-zsurfp(i))*ftime)*tlapse+tmsl

        if (vivi) then !VKP
          tempsurf1 = tempsurfp(i) + (tempsurf(i)-tempsurfp(i))*ftime !VKP
          tempsurf2 = tempsurfp(i) + (tempsurf(i)-tempsurfp(i))*ftimep !VKP
          ! t(in) = t(in) + tempsurf1 - tempsurf2 !VKP
          tp(in)=tempsurf2
          t(in)=tempsurf1
        endif !VKP
      enddo

      in=0
      do i=1,nsurf !VKP
        do k=1,nz !VKP
          in=in+1 !VKP
          u(in)=usurf(i) !VKP
        enddo !VKP
      enddo !VKP

      if (nd.eq.0.and.ilog.eq.1) write (9,*) 'Isostatic rebound'
      ! compute isostasy
      if (isoflag.eq.1) then
        call isostatic_rebound (topoa,topob,xsurf,ysurf,nsurf,rsurf, &
                          rhoc,rhom,nx,ny,nxiso,nyiso, &
                          xstep,ystep,young,poisson,thickness)
      else
        rsurf=0.
      endif

      do i=1,nsurf
        do k=1,nz
          rebound(k+(i-1)*nz)=rsurf(i)
        enddo
      enddo

      if (in.ne.nnode) then
        stop 'nnode mismatch'
      endif

      ! build local FE matrices

      if (nd.eq.0.and.ilog.eq.1) write (9,*) 'Building matrix'

      call cpu_time (times(7))
      je=0
      do je=1,nelemloc
        ie=ice(je)
        call make_matrix (mpe,ael(1,1,je),bel(1,je),icon(1,ie), &
                          x,y,z,u,xp,yp,zp,up, & !VKP
                          kfix,diffusivity,heatproduction,heatprod_flag,D_heat, &
                          x1f,y1f,x2f,y2f,def,dif, &
                          alpha,dt,time,tp,nnode,istatic, &
                          fault,nfault,rebound, &
                          xmin,xmax,ymin,ymax,0.d0,zl+zmax,friction, &
                          lsf,lsfx,lsfy,nx,ny,nz,tfinal,check)
        if (check.eq.1) then 
            misfit = 9999
            print *, "check make_matrix"
            goto 333
        endif
      enddo
      call cpu_time (times(8))

      ! build global RHS vector

      if (nd.eq.0.and.ilog.eq.1) write (9,*) 'Building RHS vector'

      f=0.d0
      do je=1,nelemloc
        ie=ice(je)
        do k=1,mpe
          ic=icon(k,ie)
          f(ic)=f(ic)+bel(k,je)
        enddo
      enddo

      ! solve global FE equations


      if (nd.eq.0.and.ilog.eq.1) write (9,*) 'Solving matrix'

      call cpu_time (times(5))
      if (iterative.eq.1) then
        call solve_iterative (mpe,1,ael,f,t,kfix,icon, &
                              nnode,nelemloc,niter,proc,ice)
        if (nd.eq.0.and.ilog.eq.1) write (9,*) niter,' iterations'
      elseif (iterative.eq.2) then
        stop 'solution strategy not implemented'
      else
        stop 'solution strategy not implemented'
      endif
      
      call cpu_time (times(6))

      if (iproc.eq.0.and.nd.eq.0) call screen_counter (itime,ntime,niter)

      ! stretch old grid

      if (nd.eq.0.and.ilog.eq.1) write (9,*) 'Stretch old grid'

      do i=1,nsurf
        do k=1,nz
          in=(i-1)*nz+k
          fact=float(k-1)/float(nz-1)
          if (interpol.eq.2.and.fact.gt.eps) fact=sqrt(fact)
          zsurf1=zsurfp(i)+(zsurf(i)-zsurfp(i))*ftime
          zh=zsurf1+zl
          zp(in)=zh*fact
        enddo
      enddo

      ! update fault geometry

      if (nd.eq.0.and.ilog.eq.1) write (9,*) 'Update fault geometry'

      if (fltflag.eq.1) call move_fault (fault,faultp,nfault,dt,time,xmin,xmax,ymin,ymax)

      ! interpolate result onto undeformed mesh

      if (nd.eq.0.and.ilog.eq.1) write (9,*) 'Interpolate onto undeformed mesh'

      do i=1,nsurf
        ij=(i-1)*nz+1
        call interpolate (t(ij),tp(ij),z(ij),zp(ij),nz)
      enddo

      ! update ptt

      if (nd.eq.0.and.ilog.eq.1) write (9,*) 'Update PTT'

      zsurfo=we1*zsurf(iconsurf(1,ieo))+we2*zsurf(iconsurf(2,ieo))+ &
            we3*zsurf(iconsurf(4,ieo))+we4*zsurf(iconsurf(4,ieo))
      zsurfpo=we1*zsurfp(iconsurf(1,ieo))+we2*zsurfp(iconsurf(2,ieo))+ &
            we3*zsurfp(iconsurf(4,ieo))+we4*zsurfp(iconsurf(4,ieo))

      ! For prediction at all surface nodes
      do kstep=max(1,istep),nstep
        do i=1,nsurf
        ! Jean unchanged the position of the following lines after the computations for the Temperature
        ! as it appeared that the depth (and thus ages) where offset by one time step
          xxx=xdepth(i,kstep)
          yyy=ydepth(i,kstep)
          zzz=zdepth(i,kstep)
          do k=1,niteradvec
            call find_velo (xxx,yyy,zzz,time,dt,fault,nfault,vx,vy,vz,xmin,xmax,ymin,ymax,.true.)
            dxxx=xdepth(i,kstep)+dt*vx/2.-xxx
            dyyy=ydepth(i,kstep)+dt*vy/2.-yyy
            dzzz=zdepth(i,kstep)+dt*vz/2.-zzz
            xxx=xxx+dxxx
            yyy=yyy+dyyy
            zzz=zzz+dzzz
          enddo
          xdepth(i,kstep)=xdepth(i,kstep)+vx*dt
          ydepth(i,kstep)=ydepth(i,kstep)+vy*dt
          zdepth(i,kstep)=zdepth(i,kstep)+vz*dt+rsurf(i)+dt*usurf(i) ! VKP
          call find_element (xdepth(i,kstep),ydepth(i,kstep),zdepth(i,kstep),tnow,x,y,z,t, &
                    xsurf,ysurf,zsurf,ielsurf(i),neighbour, &
                    iconsurf,icon,nelemsurf,nelem, &
                    nsurf,nz,nnode,npe,mpe, &
                    interpol_success)
          if (interpol_success) then
              tprev(i,kstep)=tnow
          else
            if (zdepth(i,kstep).ge.zl) then
              tprev(i,kstep)=(zl-zdepth(i,kstep))*tlapse+tmsl
            elseif (zdepth(i,kstep).lt.0.d0) then
              tprev(i,kstep)=tmax
            else
            ! tprev(i,kstep)=tmax-tmax*zdepth(i,kstep)/zl
              tprev(i,kstep)=tmax-tmax*zdepth(i,kstep)/(zl+zsurfp(i)+(zsurf(i)-zsurfp(i))*ftime)
            endif
          endif
        enddo
      enddo

      ! If prediction at sample-specific locations
      do i=1,nobs
        ! Jean unchanged the position of the following lines after the computations for the Temperature
        ! as it appeared that the depth (and thus ages) where offset by one time step
        xxx=xdeptho(i)
        yyy=ydeptho(i)
        zzz=zdeptho(i)
        do k=1,niteradvec
          call find_velo (xxx,yyy,zzz,time,dt,fault,nfault,vx,vy,vz,xmin,xmax,ymin,ymax,.true.)
          dxxx=xdeptho(i)+dt*vx/2.-xxx
          dyyy=ydeptho(i)+dt*vy/2.-yyy
          dzzz=zdeptho(i)+dt*vz/2.-zzz
          xxx=xxx+dxxx
          yyy=yyy+dyyy
          zzz=zzz+dzzz
        enddo
        xdeptho(i)=xdeptho(i)+vx*dt
        ydeptho(i)=ydeptho(i)+vy*dt
        zdeptho(i)=zdeptho(i)+vz*dt+we1(i)*(rsurf(iconsurf(1,ieo(i)))+dt*usurf(iconsurf(1,ieo(i)))) &
                        +we2(i)*(rsurf(iconsurf(2,ieo(i)))+dt*usurf(iconsurf(2,ieo(i)))) &
                        +we3(i)*(rsurf(iconsurf(3,ieo(i)))+dt*usurf(iconsurf(3,ieo(i)))) &
                        +we4(i)*(rsurf(iconsurf(npe,ieo(i)))+dt*usurf(iconsurf(npe,ieo(i))))
        call find_element (xdeptho(i),ydeptho(i),zdeptho(i),tnow,x,y,z,t, &
                        xsurf,ysurf,zsurf,ielsurf(i),neighbour, &
                        iconsurf,icon,nelemsurf,nelem, &
                        nsurf,nz,nnode,npe,mpe, &
                        interpol_success)
        if (interpol_success) then
            tprevo(i)=tnow
        else
          if (zdeptho(i).ge.zl) then ! Means that sample is at the surface ? assume it is (Maxime)
            tprevo(i)=(zl-zdeptho(i))*tlapse+tmsl
          elseif (zdeptho(i).lt.0.d0) then
            tprevo(i)=tmax
          else
            !  tprevo(i)=tmax-tmax*zdeptho(i)/zl
            tprevo(i)=tmax-tmax*zdeptho(i)/(zl+zsurfpo(i)+(zsurfo(i)-zsurfpo(i))*ftime)
          endif
        endif
      enddo

      if (nd.eq.0.and.ilog.eq.1) write (9,*) 'Update Level Set Functions'

      allocate (lsfn(nsurf,nz),lsfxn(nsurf,nz),lsfyn(nsurf,nz))
      do k=1,nz
        do i=1,nsurf
          xxx=xsurf(i)
          yyy=ysurf(i)
          zzz=(zl+zmax)*float(k-1)/(nz-1)
          call find_velo (xxx,yyy,zzz,time,dt,fault,nfault,vx,vy,vz,xmin,xmax,ymin,ymax,.false.)
          xxx=xxx-vx*dt
          yyy=yyy-vy*dt
          zzz=zzz-vz*dt-rsurf(i)-dt*usurf(i)
          lsfxn(i,k)=hinterpolate(lsfx,xxx,yyy,zzz,xmin,xmax,ymin,ymax,0.d0,zl+zmax,nx,ny,nz)
          lsfyn(i,k)=hinterpolate(lsfy,xxx,yyy,zzz,xmin,xmax,ymin,ymax,0.d0,zl+zmax,nx,ny,nz)
          lsfn(i,k)=hinterpolate(lsf,xxx,yyy,zzz,xmin,xmax,ymin,ymax,0.d0,zl+zmax,nx,ny,nz)
        enddo
      enddo
      lsf=lsfn
      lsfx=lsfxn
      lsfy=lsfyn
      deallocate (lsfn,lsfxn,lsfyn)

      ! saves tt and pt

      if (nd.eq.0.and.ilog.eq.1) write (9,*) 'Save Time-Temperature paths'

      jrec=jrec+1
      tfake=tfinal-time
      if (jrec.eq.1) tfake=tprevious-time
      do kstep=max(1,istep),nstep
        
        write (100+kstep,rec=jrec) sngl(tfake),sngl(tprev(1:nsurf,kstep)), &
          sngl(zl+zsurfp(1:nsurf)+(zsurf(1:nsurf)-zsurfp(1:nsurf))*ftime-zdepth(1:nsurf,kstep)), &
         sngl(xdepth(1:nsurf,kstep)),sngl(ydepth(1:nsurf,kstep))
            if (saveCr) then
              write (1192,rec=jrec) sngl(tfake),sngl(tprev(1:nsurf,kstep)), &
              sngl(zl+zsurfp(1:nsurf)+(zsurf(1:nsurf)-zsurfp(1:nsurf))*ftime-zdepth(1:nsurf,kstep)) !Maxime
        endif
          enddo

      write (500,rec=jrec) sngl(tfake),sngl(tprevo(1:nobs)), &
            sngl(zl+zsurfpo(1:nobs)+(zsurfo(1:nobs)-zsurfpo(1:nobs))*ftime-zdeptho(1:nobs)), &
            sngl(xdeptho(1:nobs)),sngl(ydeptho(1:nobs))
        
      call cpu_time (times(4))

    enddo ! end of time stepping

    ! clean up

    if (istep.ne.0) then

      allocate(t2(nsurf),t2p(nsurf),t3(nsurf),t4(nsurf),t5(nsurf))
      t2p=0.
      do krec=jrec,1,-1
        read (100+istep,rec=krec) t1,t2,t3,t4,t5
        do i=1,nsurf
          if (t2(i).lt.0.) t2(i)=t2p(i)
          t2p(i)=t2(i)
        enddo
        write (100+istep,rec=krec) t1,t2,t3,t4,t5
      enddo
      deallocate (t2,t2p,t3,t4,t5)

      write (100+istep,rec=jrec+1) sngl(-1.d0),sngl(tprev(1:nsurf,istep)), &
           sngl(zl+zsurfp+(zsurf-zsurfp)*ftime-zdepth(1:nsurf,istep)), &
           sngl(xdepth(1:nsurf,istep)),sngl(ydepth(1:nsurf,istep))
    endif

    ! calculates ages

    if (nd.eq.0.and.ilog.eq.1.and.agecomput.eq.1) write (9,*) 'Calculate ages' !Maxime

    allocate (age1(nsurf),age2(nsurf),age3(nsurf),age4(nsurf),age5(nsurf),nNtl(nsurf),nNosl(nsurf),nNesr(nsurf))
        allocate (ageESR(nsurf), ageOSL(nsurf))
    allocate (age6(nsurf),age7(nsurf),age8(nsurf),ftdist(20,nsurf),grainsize(nsurf),fmean(nsurf),dfmean(nsurf))
        allocate (AUppm(nsurf),AThppm(nsurf),kinFTL_AFT(nsurf),kinFTL_AHe(nsurf)) !Maxime
        allocate (ASize(nsurf),ZUppm(nsurf),ZThppm(nsurf),ZSize(nsurf),EdgeAge(nsurf)) !Maxime

    if (istep.eq.0.or.iout.eq.0) then 

      age1=real(tprevious, kind=sp);age2=real(tprevious, kind=sp);age3=real(tprevious, kind=sp);
      age4=real(tprevious, kind=sp);age5=real(tprevious, kind=sp);age6=real(tprevious, kind=sp);
      age7=real(tprevious, kind=sp);age8=real(tprevious, kind=sp);
      ftdist=0.;nNtl=0.;nNosl=0.;nNesr=0.;ageESR=0.;fmean=0.
      ageOSL=0.
          dfmean = 0.
      EdgeAge = 0. !Maxime

    elseif (agecomput.eq.1) then !Compute the ages for all nodes in the model - Maxime
      if (nd.eq.0.and.p%echo_input_file.gt.0) then
        write (6,*) ''
        write (6,*) 'Calculating ages'
      endif
      grainsize = real(p%grainradius, kind=sp) !Maxime
      AUppm = real(p%AUPPM, kind=sp) !Maxime
      AThppm = real(p%AThPPM, kind=sp) !Maxime
      kinFTL_AFT = p%Kinetic_FTL_Parameter_value_AFT !Maxime
      kinFTL_AHe = p%Kinetic_FTL_Parameter_value_AHe !Maxime
      kinFTLID = p%Kinetic_FTL_Parameter !Maxime
      Init_FTL_Model = p%Init_FTL_Model !Maxime
      Init_FTL_value = p%Init_FTL_value !Maxime
      rhoST = p%rhoST ! Maxime
      D0_spec = p%D0_AHe!Maxime
      Ea_spec = p%Ea_AHe    !Maxime
      RDmodel = p%RDmodel !Maxime
      ASize = real(p%ASize, kind=sp) 
      D0z = p%D0_ZHe
      Eaz = p%Ea_ZHe
      RDmodelz = p%RDmodelz
      ZSize = real(p%ZSize, kind=sp)
      ZThppm = real(p%ZThppm, kind=sp)
      ZUppm = real(p%ZUppm, kind=sp)
      D0k = p%D0_KAr
      Eak = p%Ea_KAr
      D0b = p%D0_BAr
      Eab = p%Ea_BAr
      D0m = p%D0_MAr
      Eam = p%Ea_MAr
      D0h = p%D0_HAr
      Eah = p%Ea_HAr
      Alpha_Ejec_Flag = p%Alpha_Ejec_Flag !Maxime
      durationHeating = p%duration
      temp_heat = p%theating
      He43_flag = p%He43_flag
      ! Calculate thermochon ages for all surface nodes
      call calculate_ages (jrec,nsurf,nz,istep,age1,age2,age3,age4,age5,age6,age7,age8, &
              ftdist,fmean,dfmean,tprevious,ftlflag,ageflag,iproc,nd,.FALSE.,grainsize,&
              ASize,AUppm,AThppm,kinFTL_AFT,kinFTL_AHe,Init_FTL_Model,Init_FTL_value,kinFTLID,rhoST,&
              D0_spec,Ea_spec,RDmodel,Alpha_Ejec_Flag,data_ADAM_array,&
              D0z,Eaz,RDmodelz,ZUppm,ZThppm,ZSize,sampleID,EdgeAge,durationHeating,&
              temp_heat,He43_flag,D0k,Eak,D0b,Eab,D0m,Eam,D0h,Eah)
                             
          ! Thermoluminescence calculation
      if (nd.eq.0.and.p%age_TL_flag.ne.0) then
        write (6,*) ''
        write (6,*) 'Calculating TL'
        call calculate_TL (jrec,nsurf,nz,istep,nNtl,p%TL_doser,p%TL_D0,p%TL_a,p%TL_b,p%TL_Et,p%TL_logs,p%TL_logrho,p%TL_Model)
      endif
          ! OSL calculation
          if (nd.eq.0.and.p%age_OSL_flag.ne.0) then
        write (6,*) ''
        write (6,*) 'Calculating OSL'
        call calculate_OSL (jrec,nsurf,nz,istep,nNosl,ageOSL,p%OSL_doser,p%OSL_D0,p%OSL_Et,p%OSL_Eu,p%OSL_logs,&
                            p%OSL_logrho,p%OSL_Lmax,p%OSL_a,p%OSL_Model)
        if (p%OSL_misfit_Target.eq.0) then
            nNosl = ageOSL
        endif
      endif
          ! ESR calculation
          if (nd.eq.0.and.p%age_ESR_flag.ne.0) then
        write (6,*) ''
        write (6,*) 'Calculating ESR'
        call calculate_ESR (jrec,nsurf,nz,istep,nNesr,ageESR,p%ESR_doser,p%ESR_D0,p%ESR_logs,p%ESR_Et,&
            p%ESR_sigmaEt,p%ESR_Lmax,p%ESR_a,p%ESR_b,p%ESR_Model)

        if (p%ESR_misfit_Target.eq.0) then
            nNesr = ageESR
        endif
      endif                                      
    endif

    ! write output

    if (iout.eq.1) then

      if (iproc.eq.0.and.nd.eq.0.and.p%echo_input_file.gt.0) then
       write (6,*) 'Saving at step ',istep
      endif
      irec=irec+1

      allocate (vxx(nnode),vyy(nnode),vzz(nnode),diff(nnode),het(nnode),zinit(nnode),xinit(nnode),yinit(nnode))
      ij=0
      iu=0 !VKP
      do i=1,nsurf
        iu=iu+1 !VKP
        do k=1,nz
          ij=ij+1
          call find_velo (x(ij),y(ij),z(ij),time,dt,fault,nfault,vx,vy,vz,xmin,xmax,ymin,ymax,.true.)
          vxx(ij)=real(vx, kind=sp); vyy(ij)=real(vy, kind=sp); vzz(ij)=real(vz+usurf(iu), kind=sp)

          if (dt.gt.tiny(dt)) vzz(ij)= real(vzz(ij)+rebound(ij)/dt, kind=sp) !VKP

          xinit(ij)=hinterpolate (lsfx,x(ij),y(ij),z(ij),xmin,xmax,ymin,ymax,0.d0,zl+zmax,nx,ny,nz)
          yinit(ij)=hinterpolate (lsfy,x(ij),y(ij),z(ij),xmin,xmax,ymin,ymax,0.d0,zl+zmax,nx,ny,nz)
          zinit(ij)=hinterpolate (lsf,x(ij),y(ij),z(ij),xmin,xmax,ymin,ymax,0.d0,zl+zmax,nx,ny,nz)

          if (heatprod_flag.eq.1) then ! e-folding
            het(ij)=heat_production(xinit(ij),yinit(ij),zinit(ij),t(ij),tfinal-time,heatproduction,D_heat)
          elseif (heatprod_flag.eq.0) then ! vertically and spatially uniform
            het(ij)=heatproduction
          endif
          if (diffusivity.lt.0.d0) then
            diff(ij)=thermal_diffusivity(xinit(ij),yinit(ij),zinit(ij),t(ij),tfinal-time)
          else
            diff(ij)=diffusivity
          endif
        enddo
      enddo

      if (nd.eq.0) write (8,rec=irec) nnode,nelem,mpe,sngl(tfinal-time),sngl(x),sngl(y),sngl(z),sngl(t), &
            vxx,vyy,vzz,sngl(xinit),sngl(yinit),sngl(zinit),sngl(diff),sngl(het),((icon(j,i),j=1,mpe),i=1,nelem)

      deallocate (vxx,vyy,vzz,diff,het,xinit,yinit,zinit)

      ! Retrieve and store exhumation rates
      allocate (exhumation(nsurf))

      exhumation=0.d0
      do i=1,nsurf ! Loop on topographic nodes
        dtimesurf=timesurf-timesurfp
        if (dtimesurf.gt.0.d0) exhumation(i)= real(-(zsurf(i)-zsurfp(i))/dtimesurf, kind=sp)

        call find_velo (xsurf(i),ysurf(i),zl+zsurf(i),time,dt,fault,nfault,vx,vy,vz,xmin,xmax,ymin,ymax,.true.)
        
          exhumation(i)= real(exhumation(i)+vz+usurf(i)+rsurf(i)/dtimesurf, kind=sp) !VKP - Maxime added isostatic rebound
      enddo

      if (nd.eq.0) write (11,rec=irec) nsurf,nelemsurf,npe,sngl(tfinal-time),sngl(xsurf),sngl(ysurf),sngl(zl+zsurf), &
                      exhumation,age1,age2,age3,age4,age5,age6,age7,age8,EdgeAge, &
                      (sngl(sum(length*ftdist(:,j))),j=1,nsurf),nNtl,nNosl,nNesr,iconsurf

      deallocate (exhumation)

      ! end store exhumation rates

      write (cstep,'(i3)') istep
      if (istep.lt.10) cstep(1:2)='00'
      if (istep.lt.100) cstep(1:1)='0'
      if (nd.eq.0) then
        open (12,file=run//'/output/Ages'//cstep//'.csv',status='unknown')
        write (12,'(a)') 'Longitude,Latitude,Height,HeApatite,HeZircon,FTApatite,' &
              //'FTZircon,ArKFeldspar,ArBiotite,ArMuscovite,ArHornblend,' &
              //'EdgeAge,FTApMeanTL,nNtl,nNosl,nNesr,ESRAge'
        do i=1,nsurf
          xxx=xlonmin+(xlonmax-xlonmin)*(xsurf(i)-xmin)/(xmax-xmin)
          yyy=xlatmin+(xlatmax-xlatmin)*(ysurf(i)-ymin)/(ymax-ymin)
          write (12,'(g12.6,16(",",g12.6))') xxx,yyy,zsurf(i)*1000,age1(i),age2(i),age3(i),age4(i),age5(i), &
                  age6(i),age7(i),age8(i),EdgeAge(i),sngl(sum(length*ftdist(:,i))),nNtl(i),nNosl(i),nNesr(i)
        enddo
        close (12)
        !call vtkp (lsf,nx,ny,nz,xmax-xmin,ymax-ymin,zl+zmax,istep)
      endif
    endif

    if (istep.ne.nstep) then
      deallocate (age1,age2,age3,age4,age5,age6,age7,age8,ftdist,grainsize,AUppm,AThppm,kinFTL_AFT,&
                  kinFTL_AHe,nNtl,nNosl,nNesr)
          deallocate (ageESR,ageOSL,fmean,dfmean)
      deallocate (ASize,ZUppm,ZThppm,ZSize,EdgeAge)
    endif

    ! end of surface stepping

    if (istep.ne.0) close (100+istep)
      
  enddo


  !-----------------------------------------------------------

  if (p%echo_input_file.gt.0.and.nd.eq.0) then
    write (*,*) '---------------------------------------------------------------------------------'
    write (*,*) '------------------Computing misfit and writing output----------------------------'
    write (*,*) '---------------------------------------------------------------------------------'
  endif

  zsurfo=we1*zsurf(iconsurf(1,ieo))+we2*zsurf(iconsurf(2,ieo))+ &
      we3*zsurf(iconsurf(4,ieo))+we4*zsurf(iconsurf(4,ieo))
  zsurfpo=we1*zsurfp(iconsurf(1,ieo))+we2*zsurfp(iconsurf(2,ieo))+ &
      we3*zsurfp(iconsurf(4,ieo))+we4*zsurfp(iconsurf(4,ieo))
  write (500,rec=jrec+1) sngl(-1.d0),sngl(tprevo(1:nobs)), &
      sngl(zl+zsurfpo(1:nobs)+(zsurfo(1:nobs)-zsurfpo(1:nobs))*ftime-zdeptho(1:nobs)), &
      sngl(xdeptho(1:nobs)),sngl(ydeptho(1:nobs))

  if (iproc.eq.0.and.nd.eq.0) then
    close (8)
    open (8,file=run//'/output/Pecube.out',status='unknown',access='direct', recl=4)
    write (8,rec=irec*(4+12*nnode+mpe*nelem)+1) -1
    close (8)
  endif

  if (iproc.eq.0.and.nd.eq.0) then
    close (11)
    open (11,file=run//'/output/Ages.out',status='unknown',access='direct', recl=4)
    write (11,rec=irec*(4+17*nsurf+npe*nelemsurf)+1) -1
    close (11)
  endif

  if (nd.eq.0.and.ilog.eq.1) close (9)

  ! calculate misfit
  ! added by Jean to change misfit to slope rather than exact ages
  ! (4/6/2008)
  if (nd.eq.0.and.nobs1.gt.0) open (13,file=run//'/output/CompareAGE.csv',status='unknown')
  misfit=9999.d0
  nmisfit1=0
  misfit1 = 0.d0
  nhe=0
  nft=0
  nheZ=0
  nftZ=0
  narK=0
  narB=0
  narM=0
  narH=0
  ! Read observations
  read (7,*) nobs1,nobs2,nobs3,nobs4,nobs5,nobs6
  if (nobs1.lt.0) nobs1=-nobs1
  if (nobs2.lt.0) nobs2=-nobs2
  if (nobs3.lt.0) nobs3=-nobs3
  if (nobs4.lt.0) nobs4=-nobs4
  ! Total number of observations
  ! nobs1: ages, nobs2: Tt, nobs3: 43He, nobs4: TSL
  nobs=nobs1+nobs2+nobs3+nobs4+nobs5+nobs6
  allocate (lonobs(nobs),latobs(nobs))
  allocate (aheoreg(nobs),ahepreg(nobs),hheoreg(nobs))
  allocate (aftoreg(nobs),aftpreg(nobs),hftoreg(nobs))
  allocate (aheZoreg(nobs),aheZpreg(nobs),hheZoreg(nobs))
  allocate (aftZoreg(nobs),aftZpreg(nobs),hftZoreg(nobs))
  allocate (aarKoreg(nobs),aarKpreg(nobs),harKoreg(nobs))
  allocate (aarBoreg(nobs),aarBpreg(nobs),harBoreg(nobs))
  allocate (aarMoreg(nobs),aarMpreg(nobs),harMoreg(nobs))
  allocate (aarHoreg(nobs),aarHpreg(nobs),harHoreg(nobs))
  allocate (hpreg(nobs))
  allocate (hei(nobs),agehe(nobs),ageheZ(nobs),ageft(nobs),ageftZ(nobs), &
        agearB(nobs),agearK(nobs),agearM(nobs),agearH(nobs),ftdisto(20,nobs), &
        temphistp(1024,nobs),fmeanp(nobs),dfmeanp(nobs))
  agehe = -9999
  ageheZ = -9999
  ageft = -9999
  ageftZ = -9999
  agearB = -9999
  agearK = -9999
  agearM = -9999
  agearH = -9999
  fmeanp = -9999
  dfmeanp = -9999
  if (nd.eq.0.and.nobs1.gt.0) write (13,'(a,64(",",a))') 'SAMPLE','LON','LAT','HEIGHTOBS','HEIGHTPRED', &
        'AHEOBS','AHEPRED','AFTOBS','AFTPRED','ZHEOBS','ZHEPRED','ZFTOBS','ZFTPRED', &
        'KAROBS','KARPRED','BAROBS','BARPRED','MAROBS','MARPRED','HAROBS','HARPRED',& 
        'MFTLOBS','MFTLPRED','DMFTLOBS','DMFTLPRED',&
        'FT01OBS','FT01PRED','FT02OBS','FT02PRED','FT03OBS','FT03PRED','FT04OBS','FT04PRED', &
        'FT05OBS','FT05PRED','FT06OBS','FT06PRED','FT07OBS','FT07PRED','FT08OBS','FT08PRED', &
        'FT09OBS','FT09PRED','FT10OBS','FT10PRED','FT11OBS','FT11PRED','FT12OBS','FT12PRED', &
        'FT13OBS','FT13PRED','FT14OBS','FT14PRED','FT15OBS','FT15PRED','FT16OBS','FT16PRED', &
        'FT17OBS','FT17PRED','FT18OBS','FT18PRED','FT19OBS','FT19PRED','FT20OBS','FT20PRED'

  if (saveTt.and.nd.eq.0) open (82,file=run//'/output/TimeTemperaturePaths.csv',status='unknown')
    ageflagp=1

    do iobs=1,nobs1
      read (7,*) sampleID_array(iobs),dummy,dummy,dummy,dummy,dummy,dummy, &
        dummy,dummy,dummy,dummy,dummy,dummy, &
        dummy,dummy,dummy,dummy,dummy,dummy, &
        (dummy,i=1,20),dummy,fmeano,dfmeano,grainsize(iobs),ieobs,wobs1,wobs2,wobs3,wobs4,ASize(iobs),AUppm(iobs),& 
        AThppm(iobs),kinFTL_AFT(iobs),kinFTL_AHe(iobs),ZUppm(iobs),ZThppm(iobs),ZSize(iobs)!Maxime
    enddo

    do iobs=1,nobs1
      backspace (7)
    enddo

    rhoST = p%rhoST
    ftlflag = p%FT_code_flag
    kinFTLID = p%Kinetic_FTL_Parameter !Maxime
    Init_FTL_Model = p%Init_FTL_Model !Maxime
    Init_FTL_value = p%Init_FTL_value !Maxime
    D0k = p%D0_KAr
    Eak = p%Ea_KAr
    D0b = p%D0_BAr
    Eab = p%Ea_BAr
    D0m = p%D0_MAr
    Eam = p%Ea_MAr
    D0h = p%D0_HAr
    Eah = p%Ea_HAr
    ! Predict the ages
    call calculate_ages (jrec,nobs1,nz,400,agehe,ageheZ,ageft,ageftZ,agearK,agearB,agearM,agearH, &
              ftdisto,fmeanp,dfmeanp,tprevious,ftlflag,ageflag,iproc,nd,saveTt,grainsize,&
              ASize,AUppm,AThppm,kinFTL_AFT,kinFTL_AHe,Init_FTL_Model,Init_FTL_value,kinFTLID,rhoST,&
              D0_spec,Ea_spec,RDmodel,Alpha_Ejec_Flag,data_ADAM_array,&
              D0z,Eaz,RDmodelz,ZUppm,ZThppm,ZSize,sampleID_array,EdgeAge,durationHeating,&
              temp_heat,0,D0k,Eak,D0b,Eab,D0m,Eam,D0h,Eah)  
                       
    if (saveTt) close (82)
        
    ! Save PTT path for all surface nodes, to compute cooling rates - Maxime
      if (saveCr) then
          open (15,file=run//'/output/CoolingRates.csv',status='unknown')
          allocate (ztimep(jrec),ztempp(jrec))
          allocate (memp(jrec,0:nsurf)) 
      do i=1,nsurf
              if (iproc.eq.0.and.nd.eq.0) call screen_counter (i,nsurf,0)
              ij=(i-1)*nz+1
        do irec=1,jrec
                  read (1192,rec=irec) ztimep(irec), (zjunkp, k=1, i-1), ztempp(irec)
        enddo
              timenowp=ztimep(jrec)
              ztimep=ztimep-timenowp
              memp(:,i)=ztempp
              memp(:,0)=ztimep
            enddo
             
            do ij=1,jrec
              write (15,'(g12.6,100000(",",g12.6))') (memp(ij,i),i=0,nsurf)
            enddo
      deallocate (ztimep,ztempp)
      close (15)
      close (1192) !Maxime
        endif

    deallocate (ASize)
       
    ! compute misfit for ages

    nmisfit1 = 0
    misfit1 = 0.
    misfitAHE = 0.
    nmisfitAHE = 0
    misfitAFT = 0.
    nmisfitAFT = 0
    misfitZHE = 0.
    nmisfitZHE = 0
    misfitZFT = 0.
    nmisfitZFT = 0
    misfitKAR = 0.
    nmisfitKAR = 0
    misfitBAR = 0.
    nmisfitBAR = 0
    misfitMAR = 0.
    nmisfitMAR = 0
    misfitHAR = 0.
    nmisfitHAR = 0
      misfitMFTL = 0.
      nmisfitMFTL = 0
      misfitDMFTL = 0.
    misfitres = 0.
      LL_ThL = 0.
      LL_OSL = 0.
      LL_ESR = 0.
    LL_AHE = 0.
    LL_AFT = 0.
    LL_ZHE = 0.
    LL_ZFT = 0.
    LL_KAR = 0.
    LL_BAR = 0.
    LL_MAR = 0.
    LL_HAR = 0.
      LL_MFTL = 0.
      LL_DMFTL = 0.
    LL_total = 0.
    
    do iobs=1,nobs1
      ! Read observed ages with errors
      read (7,*) sampleID,xlonobs,xlatobs,ageheobs,dageheobs,ageftobs,dageftobs, & ! By Xav
          ageheZobs,dageheZobs,ageftZobs,dageftZobs,agearKobs,dagearKobs, &
          agearBobs,dagearBobs,agearMobs,dagearMobs,agearHobs,dagearHobs, &
          tlobs,heightobs,fmeano,dfmeano,dummy,ieobs,wobs1,wobs2,wobs3,wobs4   ! By Xav
      ! Observed elevation
      hei(iobs)= real(wobs1*zsurf(iconsurf(1,ieobs)) &
          +wobs2*zsurf(iconsurf(2,ieobs)) &
          +wobs3*zsurf(iconsurf(3,ieobs)) &
          +wobs4*zsurf(iconsurf(npe,ieobs)), kind=sp)
      hei(iobs)= real(hei(iobs)*1000.+min(0.d0,heightobs), kind=sp)
      lonobs(iobs)= real(xlonobs, kind=sp)
      latobs(iobs)= real(xlatobs, kind=sp)
      if (ageheobs.gt.0.) then
        nhe=nhe+1
        aheoreg(nhe)=ageheobs
        hheoreg(nhe)=hei(iobs)
      endif
      if (ageftobs.gt.0.) then
        nft=nft+1
        aftoreg(nft)=ageftobs
        hftoreg(nft)=hei(iobs)
      endif
      if (ageheZobs.gt.0.) then
        nheZ=nheZ+1
        aheZoreg(nheZ)=ageheZobs
        hheZoreg(nheZ)=hei(iobs)
      endif
      if (ageftZobs.gt.0.) then
        nftZ=nftZ+1
        aftZoreg(nftZ)=ageftZobs
        hftZoreg(nftZ)=hei(iobs)
      endif
      if (agearKobs.gt.0.) then
        narK=narK+1
        aarKoreg(narK)=agearKobs
        harKoreg(narK)=hei(iobs)
      endif
      if (agearBobs.gt.0.) then
        narB=narB+1
        aarBoreg(narB)=agearBobs
        harBoreg(narB)=hei(iobs)
      endif
      if (agearMobs.gt.0.) then
        narM=narM+1
        aarMoreg(narM)=agearMobs
        harMoreg(narM)=hei(iobs)
      endif
      if (agearHobs.gt.0.) then
        narH=narH+1
        aarHoreg(narH)=agearHobs
        harHoreg(narH)=hei(iobs)
      endif
      ahepreg(iobs)=agehe(iobs)
      aftpreg(iobs)=ageft(iobs)
      aheZpreg(iobs)=ageheZ(iobs)
      aftZpreg(iobs)=ageftZ(iobs)
      aarKpreg(iobs)=agearK(iobs)
      aarBpreg(iobs)=agearB(iobs)
      aarMpreg(iobs)=agearM(iobs)
      aarHpreg(iobs)=agearH(iobs)
      hpreg(iobs)=hei(iobs)
      ! Write observed and predicted ages
      if (p%lon0.lt.0) xlonobs = xlonobs - 360 
      if (nd.eq.0.and.nobs1.gt.0) write (13,'(A,",",g12.6,63(",",g12.6))') trim(sampleID),xlonobs,xlatobs,heightobs,hei(iobs), &
            ageheobs,agehe(iobs),ageftobs,ageft(iobs),ageheZobs,ageheZ(iobs), &
            ageftZobs,ageftZ(iobs), &
            agearKobs,agearK(iobs),agearBobs,agearB(iobs),agearMobs,agearM(iobs),agearHobs,agearH(iobs),&
            fmeano,fmeanp(iobs),dfmeano,dfmeanp(iobs),(tlobs(i),ftdisto(i,iobs),i=1,20)
              
      !######### Compute misfit for each thermochronometer ##################
      datafit = 0
      misfitflag = p%misfit_function
      error_pred = real(p%error_predictions, kind=sp)
      ! AHE
      if (ageheobs.gt.0.) then
          call misfit_calculation (misfitflag,ageheobs,agehe(iobs),dageheobs,misfitres,error_pred,datafit)
          misfitAHE = misfitAHE+misfitres
          misfit1=misfit1+misfitres
          call LL_calculation (misfitflag,ageheobs,agehe(iobs),dageheobs,LL_results,error_pred)
          LL_AHE = LL_AHE + LL_results
          nmisfit1=nmisfit1+1
          nmisfitAHE = nmisfitAHE +1
      endif
          ! AFT
      if (ageftobs.gt.0.) then
          call misfit_calculation (misfitflag,ageftobs,ageft(iobs),dageftobs,misfitres,error_pred,datafit)
          misfitAFT=misfitAFT+misfitres
          misfit1=misfit1+misfitres
          call LL_calculation (misfitflag,ageftobs,ageft(iobs),dageftobs,LL_results,error_pred)
          LL_AFT = LL_AFT + LL_results
          nmisfitAFT = nmisfitAFT +1
          nmisfit1=nmisfit1+1
          ! on MFTL ?
          if (ageflag(9).eq.1.and.fmeano.gt.0.) then
        ! print *, 'MFTL obs = ', fmeano, dfmeano, fmeanp(iobs)+1e-6, dfmeanp(iobs)
            call misfit_calculation (misfitflag,real(fmeano),real(fmeanp(iobs))+1e-6,&
                                          real(p%MFTL_error),misfitres,error_pred,datafit)
            misfitMFTL=misfitMFTL+misfitres
            misfit1=misfit1+misfitres
           call misfit_calculation (misfitflag,real(dfmeano),real(dfmeanp(iobs))+1e-6,&
                                          real(p%MFTL_std_error),misfitres,error_pred,datafit)
            misfitDMFTL=misfitDMFTL+misfitres
            misfit1=misfit1+misfitres
            nmisfitMFTL = nmisfitMFTL + 1
                
                call LL_calculation (misfitflag,real(fmeano),real(fmeanp(iobs))+1e-6,&
                                       real(p%MFTL_error),LL_results,error_pred)
                LL_MFTL = LL_MFTL + LL_results
                call LL_calculation (misfitflag,real(dfmeano),real(dfmeanp(iobs))+1e-6,&
                                         real(p%MFTL_std_error),LL_results,error_pred)
                LL_DMFTL = LL_DMFTL + LL_results
                nmisfit1=nmisfit1+2
              endif
      endif
          !ZHE
      if (ageheZobs.gt.0.) then
        call misfit_calculation (misfitflag,ageheZobs,ageheZ(iobs),dageheZobs,misfitres,error_pred,datafit)
        misfitZHE=misfitZHE+misfitres
        misfit1=misfit1+misfitres
        call LL_calculation (misfitflag,ageheZobs,ageheZ(iobs),dageheZobs,LL_results,error_pred)
        LL_ZHE = LL_ZHE + LL_results
        nmisfit1=nmisfit1+1
        nmisfitZHE = nmisfitZHE +1
      endif
      !ZFT
      if (ageftZobs.gt.0.) then
        call misfit_calculation (misfitflag,ageftZobs,ageftZ(iobs),dageftZobs,misfitres,error_pred,datafit)
        misfitZFT=misfitZFT+misfitres
        misfit1=misfit1+misfitres
        call LL_calculation (misfitflag,ageftZobs,ageftZ(iobs),dageftZobs,LL_results,error_pred)
        LL_ZFT = LL_ZFT + LL_results
        nmisfit1=nmisfit1+1
        nmisfitZFT = nmisfitZFT +1
      endif
      ! KAR
      if (agearKobs.gt.0.) then
        call misfit_calculation (misfitflag,agearKobs,agearK(iobs),dagearKobs,misfitres,error_pred,datafit)
        misfitKAR=misfitKAR+misfitres
        misfit1=misfit1+misfitres
        call LL_calculation (misfitflag,agearKobs,agearK(iobs),dagearKobs,LL_results,error_pred)
        LL_KAR = LL_KAR + LL_results
        nmisfit1=nmisfit1+1
        nmisfitKAR = nmisfitKAR +1
      endif
      if (agearBobs.gt.0.) then
        call misfit_calculation (misfitflag,agearBobs,agearB(iobs),dagearBobs,misfitres,error_pred,datafit)
        misfitBAR=misfitBAR+misfitres
        misfit1=misfit1+misfitres
        call LL_calculation (misfitflag,agearBobs,agearB(iobs),dagearBobs,LL_results,error_pred)
        LL_BAR = LL_BAR + LL_results
        nmisfit1=nmisfit1+1
        nmisfitBAR = nmisfitBAR +1
      endif
      if (agearMobs.gt.0.) then
        call misfit_calculation (misfitflag,agearMobs,agearM(iobs),dagearMobs,misfitres,error_pred,datafit)
        misfitMAR=misfitMAR+misfitres
        misfit1=misfit1+misfitres
        call LL_calculation (misfitflag,agearMobs,agearM(iobs),dagearMobs,LL_results,error_pred)
        LL_MAR = LL_MAR + LL_results
        nmisfit1=nmisfit1+1
        nmisfitMAR = nmisfitMAR +1
      endif
      if (agearHobs.gt.0.) then
        call misfit_calculation (misfitflag,agearHobs,agearH(iobs),dagearHobs,misfitres,error_pred,datafit)
        misfitHAR=misfitHAR+misfitres
        misfit1=misfit1+misfitres
        call LL_calculation (misfitflag,agearHobs,agearH(iobs),dagearHobs,LL_results,error_pred)
        LL_HAR = LL_HAR + LL_results
        nmisfit1=nmisfit1+1
        nmisfitHAR = nmisfitHAR +1
      endif
        
      !############### Misfit for Track length distribution ##############################
      nmisfit1a=0
      misfit1a=0.
      if (tlobs(1).gt.-1.d-3) then
        do j=1,20
          misfit1a= real(misfit1a+(tlobs(j)-ftdisto(j,iobs))**2, kind=sp)
          nmisfit1a=nmisfit1a+1
        enddo
      endif
    enddo

    LL_total = LL_AHE + LL_AFT + LL_ZHE + LL_ZFT + LL_KAR + LL_MAR + LL_BAR + LL_HAR + LL_MFTL + LL_DMFTL

    ! if the flag mftflag is set, misfit1 is replaced by a misfit on the slope of the age-elevation relationship for each chronometer

    if (mftflag.eq.1) then
      nmisfit1=0
      misfit1=0.
      if (nhe.gt.1) call regression (aheoreg,hheoreg,nhe,aheom,hheom,heoi,heos)
      if (nft.gt.1) call regression (aftoreg,hftoreg,nft,aftom,hftom,ftoi,ftos)
      if (nheZ.gt.1) call regression (aheZoreg,hheZoreg,nheZ,aheZom,hheZom,heZoi,heZos)
      if (nftZ.gt.1) call regression (aftZoreg,hftZoreg,nftZ,aftZom,hftZom,ftZoi,ftZos)
      if (narK.gt.1) call regression (aarKoreg,harKoreg,narK,aarKom,harKom,arKoi,arKos)
      if (narB.gt.1) call regression (aarBoreg,harBoreg,narB,aarBom,harBom,arBoi,arBos)
      if (narM.gt.1) call regression (aarMoreg,harMoreg,narM,aarMom,harMom,arMoi,arMos)
      if (narH.gt.1) call regression (aarHoreg,harHoreg,narH,aarHom,harHom,arHoi,arHos)
      if (nobs.gt.1) call regression (ahepreg,hpreg,nobs,ahepm,hhepm,hepi,heps)
      if (nobs.gt.1) call regression (aftpreg,hpreg,nobs,aftpm,hftpm,ftpi,ftps)
      if (nobs.gt.1) call regression (aheZpreg,hpreg,nobs,aheZpm,hheZpm,heZpi,heZps)
      if (nobs.gt.1) call regression (aftZpreg,hpreg,nobs,aftZpm,hftZpm,ftZpi,ftZps)
      if (nobs.gt.1) call regression (aarKpreg,hpreg,nobs,aarKpm,harKpm,arKpi,arKps)
      if (nobs.gt.1) call regression (aarBpreg,hpreg,nobs,aarBpm,harBpm,arBpi,arBps)
      if (nobs.gt.1) call regression (aarMpreg,hpreg,nobs,aarMpm,harMpm,arMpi,arMps)
      if (nobs.gt.1) call regression (aarHpreg,hpreg,nobs,aarHpm,harHpm,arHpi,arHps)
      misfit=9999.d0
      if (nhe.gt.1) then
        misfitAHE = real(misfitAHE+(abs(heos-heps)**2/abs(heos)**2+abs(aheom-ahepm)**2/abs(aheom)**2), kind=sp)
        misfit1= real(misfit1+(abs(heos-heps)**2/abs(heos)**2+abs(aheom-ahepm)**2/abs(aheom)**2), kind=sp)
        nmisfit1=nmisfit1+2
      endif
      if (nft.gt.1) then
        misfitAFT = real(misfitAFT+(abs(ftos-ftps)**2/abs(ftos)**2+abs(aftom-aftpm)**2/abs(aftom)**2), kind=sp)
        misfit1=real(misfit1+(abs(ftos-ftps)**2/abs(ftos)**2+abs(aftom-aftpm)**2/abs(aftom)**2), kind=sp)
        nmisfit1=nmisfit1+2
      endif
      if (nheZ.gt.1) then
        misfitZHE = real(misfitZHE+(abs(heZos-heZps)**2/abs(heZos)**2+abs(aheZom-aheZpm)**2/abs(aheZom)**2), kind=sp)
        misfit1=real(misfit1+(abs(heZos-heZps)**2/abs(heZos)**2+abs(aheZom-aheZpm)**2/abs(aheZom)**2), kind=sp)
        nmisfit1=nmisfit1+2
      endif
      if (nftZ.gt.1) then
        misfitZFT = real(misfitZFT+(abs(ftZos-ftZps)**2/abs(ftZos)**2+abs(aftZom-aftZpm)**2/abs(aftZom)**2), kind=sp)
        misfit1=real(misfit1+(abs(ftZos-ftZps)**2/abs(ftZos)**2+abs(aftZom-aftZpm)**2/abs(aftZom)**2), kind=sp)
        nmisfit1=nmisfit1+2
      endif
      if (narK.gt.1) then
        misfitKAR = real(misfitKAR+(abs(arKos-arKps)**2/abs(arKos)**2+abs(aarKom-aarKpm)**2/abs(aarKom)**2), kind=sp)
        misfit1=real(misfit1+(abs(arKos-arKps)**2/abs(arKos)**2+abs(aarKom-aarKpm)**2/abs(aarKom)**2), kind=sp)
        nmisfit1=nmisfit1+2
      endif
      if (narB.gt.1) then
        misfitBAR = real(misfitBAR+(abs(arBos-arBps)**2/abs(arBos)**2+abs(aarBom-aarBpm)**2/abs(aarBom)**2), kind=sp)
        misfit1=real(misfit1+(abs(arBos-arBps)**2/abs(arBos)**2+abs(aarBom-aarBpm)**2/abs(aarBom)**2), kind=sp)
        nmisfit1=nmisfit1+2
      endif
      if (narM.gt.1) then
        misfitMAR = real(misfitMAR+(abs(arMos-arMps)**2/abs(arMos)**2+abs(aarMom-aarMpm)**2/abs(aarMom)**2), kind=sp)
        misfit1=real(misfit1+(abs(arMos-arMps)**2/abs(arMos)**2+abs(aarMom-aarMpm)**2/abs(aarMom)**2), kind=sp)
        nmisfit1=nmisfit1+2
      endif
      if (narH.gt.1) then
        misfitHAR = real(misfitHAR+(abs(arHos-arHps)**2/abs(arHos)**2+abs(aarHom-aarHpm)**2/abs(aarHom)**2), kind=sp)
        misfit1=real(misfit1+(abs(arHos-arHps)**2/abs(arHos)**2+abs(aarHom-aarHpm)**2/abs(aarHom)**2), kind=sp)
        nmisfit1=nmisfit1+2
      endif
    endif

    if (nd.eq.0.and.nobs1.gt.0) close (13)

    !############## Compute misfit for thermal histories ##########################

    if (nd.eq.0.and.nobs2.gt.0) then
      open (13,file=run//'/output/CompareTT.csv',status='unknown')
      write (13,'(a,4(",",a))') 'LON','LAT','TIME','TEMP','TEMPPRED'
    endif

    nhist=0
    do iobs=nobs1+1,nobs1+nobs2
      read (7,*) xlonobs,xlatobs,nhist(iobs),(thist(j,iobs),temphist(j,iobs),errortemphist(j,iobs),j=1,nhist(iobs)), &
                   heightobs,ieobs,wobs1,wobs2,wobs3,wobs4   ! By Xav
      hei(iobs)=real(wobs1*zsurf(iconsurf(1,ieobs)) &
                 +wobs2*zsurf(iconsurf(2,ieobs)) &
                 +wobs3*zsurf(iconsurf(3,ieobs)) &
                 +wobs4*zsurf(iconsurf(npe,ieobs)), kind=sp)
      hei(iobs)=real(hei(iobs)*1000.+min(0.d0,heightobs), kind=sp)
      lonobs(iobs)=real(xlonobs, kind=sp)
      latobs(iobs)=real(xlatobs, kind=sp)
    enddo

    call calculate_ttpath (jrec,nobs,nz,400,nhist,thist,temphistp,iproc,nd)

    nmisfit2 = 0
    misfit2 = 0.
    do iobs=nobs1+1,nobs1+nobs2
      if (nd.eq.0.and.nobs2.gt.0) write (13,'(g15.9,",",g15.9)') lonobs(iobs),latobs(iobs)
      do j=1,nhist(iobs)
        if (temphistp(j,iobs).gt.-9998.d0) then
          call misfit_calculation (misfitflag,temphist(j,iobs),temphistp(j,iobs),errortemphist(j,iobs),misfitres,&
                                    error_pred,datafit)
          misfit2=misfit2+misfitres
          nmisfit2=nmisfit2+1
        endif
        if (nd.eq.0.and.nobs2.gt.0) write (13,'(g15.9,2(",",g15.9))') thist(j,iobs),temphist(j,iobs),temphistp(j,iobs)
      enddo
    enddo

    if (nd.eq.0.and.nobs2.gt.0) close (13)

    !####################### compute misfit for 43He  #############################

    if (nd.eq.0.and.nobs3.gt.0) then
      open (133,file=run//'/output/Compare43HE.csv',status='unknown')
      write (133,'(128(a,","))') 'SAMPLE','AGE43OBS','AGE43PRED','RELEASED',&
      'RELEASEDPRED','AGERELEASED','AGERELEASEDPRED','ARELEJEC'
    endif

    nheating_array=0
    ! Loop over 4He/3He data
    do iobs=nobs1+nobs2+1,nobs1+nobs2+nobs3
      read (7,*) sampleID_array(iobs),xlonobs,xlatobs,size43He_array(iobs),age43He(iobs),dage43He(iobs),nheating_array(iobs), &
                  Uppm43_array(iobs),Thppm43_array(iobs),rmr043_array(iobs),(theating(j,iobs),duration(j,iobs),released(j,iobs), &
                  dreleased(j,iobs),agereleased(j,iobs),dagereleased(j,iobs),j=1,nheating_array(iobs)), &
                   heightobs,ieobs,wobs1,wobs2,wobs3,wobs4   ! By Xav and Maxime added Uppm43 and Thppm43
      
      hei(iobs)=real(wobs1*zsurf(iconsurf(1,ieobs)) &
                 +wobs2*zsurf(iconsurf(2,ieobs)) &
                 +wobs3*zsurf(iconsurf(3,ieobs)) &
                 +wobs4*zsurf(iconsurf(npe,ieobs)), kind=sp)
      hei(iobs)=real(hei(iobs)*1000.+min(0.d0,heightobs), kind=sp)
      lonobs(iobs)=real(xlonobs, kind=sp)
      latobs(iobs)=real(xlatobs, kind=sp)
    enddo
    allocate (time43He(jrec,nobs),temperature43He(jrec,nobs))
    ! Extract Tt path at sample location
    call extract_ttpath (jrec,nobs,nz,400,time43He,temperature43He,iproc,nd)

    nmisfit3 = 0
    misfit3 = 0.
    nmisfit3a = 0
    misfit3a = 0.
    ! Loop over 4He/3He data
    do iobs=nobs1+nobs2+1,nobs1+nobs2+nobs3
      nheating = nheating_array(iobs) 
      allocate (releasedp(nheating),agereleasedp(nheating),He43_Ejec(nheating))
      rmr043 = rmr043_array(iobs)
      size43He = size43He_array(iobs)
      Thppm43 = Thppm43_array(iobs)
      Uppm43 = Uppm43_array(iobs)
      kinFTL_AHe = p%Kinetic_FTL_Parameter_value_AHe
      if (ageflag(1).eq.1) then ! If AHe prediction
          He43_inv_flag = p%He43_inv_flag
          
          call Compute_rmr0(rmr043, kinFTLID)
          ! print *, 'print values'
          ! print *, age43He(iobs), dage43He(iobs)
          ! print *, rmr043
          ! print *, size43He
          ! print *, Thppm43
          ! print *, Uppm43
          ! print *, jrec, nheating
          ! print *, 'end print values'
          call Hediff(time43He(:,iobs), temperature43He(:,iobs), jrec, duration(1:nheating,iobs), &
                theating(1:nheating,iobs), nheating,age43Hep, size43He, Alpha_Ejec_Flag,&
                RDmodel,Uppm43,Thppm43,D0_spec,Ea_spec,rmr043,1,agereleasedp,releasedp,&
                He43_Ejec,agereleased(1:nheating,iobs),released(1:nheating,iobs),&
                He43_inv_flag,data_ADAM_array,1,0)
      else ! Farley et al. (2000)
          call Diffusion1D (time43He(1,iobs),temperature43He(1,iobs),jrec,theating(1,iobs),duration(1,iobs),nheating, &
                          age43Hep,releasedp,agereleasedp,size43He)
      endif !Maxime     
     
      ! If want to calculate misfit on Normalized step age, compute step age
      if (He43_inv_flag.eq.1) then 
        allocate (StepAgeObs(nheating),StepAgePred(nheating),relError(nheating))
        do i=1,nheating
          StepAgeObs(i) = agereleased(i,iobs)/He43_Ejec(i)*age43He(iobs)
          StepAgePred(i) = agereleasedp(i)/He43_Ejec(i)*age43Hep
          relError(i) = dagereleased(i,iobs)/agereleased(i,iobs)
        enddo
        maxStepAgeObs = maxval(StepAgeObs(:))
        maxStepAgePred = maxval(StepAgePred(:))
      endif
          
      ! Compute misfit for each heating step
      ! open (71,file=run//'/NA/He43.csv',status='old',position='append', action='write')
      do i=1,nheating
        if (He43_inv_flag.eq.0) then ! Compute misfit on Rstep/Rbulk spectrum
          call misfit_calculation_DP (misfitflag,agereleasedp(i),agereleased(i,iobs),dagereleased(i,iobs),&
                                            real(misfitres, kind=dp))
        elseif (He43_inv_flag.eq.1) then ! Compute misfit on normalized step age
          StepAgeObs(i) = StepAgeObs(i)/age43He(iobs)
          StepAgePred(i) = StepAgePred(i)/age43Hep
          relError(i) = relError(i)*StepAgeObs(i)
          call misfit_calculation_DP (misfitflag,StepAgePred(i),StepAgeObs(i),relError(i),real(misfitres, kind=dp))
        endif
        misfitres = real(misfitres, kind=sp)
        misfit3=misfit3+misfitres
        nmisfit3=nmisfit3+1
        if (nd.eq.0.and.nobs3.gt.0) write (133,'(a,7(",",g12.6))') trim(sampleID_array(iobs)),age43He(iobs),age43Hep,&
                    released(i,iobs),releasedp(i), agereleased(i,iobs),agereleasedp(i),He43_Ejec(i)
        
        ! write (71,'(a,7(",",g12.6))') trim(sampleID_array(iobs)),age43He(iobs),age43Hep,&
        !             released(i,iobs),releasedp(i), agereleased(i,iobs),agereleasedp(i),He43_Ejec(i)
        
      enddo
      ! close (71)
        
      if (He43_inv_flag.eq.1) then
        deallocate (StepAgeObs,StepAgePred,relError)
      endif

      deallocate (releasedp,agereleasedp,He43_Ejec)
    enddo

    deallocate (time43He,nheating_array,temperature43He,duration,theating)


    
    if (nd.eq.0.and.nobs3.gt.0) close (133)

    !################## compute misfit for Thermo luminescence ####################

    if (nd.eq.0.and.nobs4.gt.0) then
      open (134,file=run//'/output/CompareTL.csv',status='unknown')
      write (134,'(A,6(",",a))') 'SAMPLE','LON','LAT','HEIGHTOBS','HEIGHTPRED','NNOBS','NNPRED'
    endif

   
    ! Loop over TL data
    do iobs=nobs1+nobs2+nobs3+1,nobs1+nobs2+nobs3+nobs4
      read (7,*) sampleID_array(iobs),xlonobs,xlatobs, &
                doser(iobs),d0(iobs),radius(iobs),et(iobs), &
                logs(iobs),b(iobs),logrho(iobs),nn(iobs),dnn(iobs), &
                heighto(iobs),ieobs,wobs1,wobs2,wobs3,wobs4   ! By Xav
      hei(iobs)=real(wobs1*zsurf(iconsurf(1,ieobs)) &
                  +wobs2*zsurf(iconsurf(2,ieobs)) &
                  +wobs3*zsurf(iconsurf(3,ieobs)) &
                  +wobs4*zsurf(iconsurf(npe,ieobs)), kind=sp)
      hei(iobs)=real(hei(iobs)*1000.+min(0.d0,heighto(iobs)), kind=sp)
      lonobs(iobs)=real(xlonobs, kind=sp)
      latobs(iobs)=real(xlatobs, kind=sp)
    enddo

    allocate (timeTSL(jrec,nobs),temperatureTSL(jrec,nobs),nnf_array(jrec,nobs))
    call extract_ttpath (jrec,nobs,nz,400,timeTSL,temperatureTSL,iproc,nd)
      
    misfit4 = 0.
    nmisfit4 = 0
    ! Loop over TL data
    do iobs=nobs1+nobs2+nobs3+1,nobs1+nobs2+nobs3+nobs4
      ! Calculate n/N or age
          if (p%TL_Model.eq.0) then
               call GOK_FAD_model (timeTSL(:,iobs), temperatureTSL(:,iobs), jrec, doser(iobs), d0(iobs), &
                radius(iobs),et(iobs),logs(iobs),b(iobs),logrho(iobs),b(iobs), nnf_array, nnf_array, nobs, iobs)
          !!! To change b(iobs) to imax and nnf_array to age_array!!!!
          endif
          nnf = real(nnf_array(jrec,iobs), kind=sp)
          error_pred = real(p%error_predictions, kind=sp)
      ! Calculate misfit
      call misfit_calculation (misfitflag,nn(iobs),nnf,dnn(iobs),misfitres,error_pred,datafit)
      misfit4=misfit4+misfitres
      nmisfit4=nmisfit4+1
      if (nd.eq.0.and.nobs4.gt.0) write (134,'(a,6(",",g15.9))') & 
          trim(sampleID_array(iobs)),lonobs(iobs),latobs(iobs),heighto(iobs),hei(iobs),nn(iobs),nnf
    enddo

        ! Write time evolution of n/N
        line = ''
        if (nd.eq.0.and.nobs4.gt.0) then
          open (14,file=run//'/output/TLTime.csv',status='unknown')
          write (line,'(A,1023(",",a))') 'TIME',(trim(sampleID_array(k)),&
                    k=nobs1+nobs2+nobs3+1,nobs1+nobs2+nobs3+nobs4)
          write (14,'(a)') line(:len(trim(line))-1) 
          do i=1,jrec
              write (line,'(g12.6,1023(",",g12.6))') timeTSL(i,1), (nnf_array(i,iobs),&
                         iobs=nobs1+nobs2+nobs3+1,nobs1+nobs2+nobs3+nobs4)
              write (14,'(a)') line(:len(trim(line))-1)
          enddo
          close (14)
        endif
        
            
    deallocate (timeTSL,temperatureTSL,nnf_array)

    if (nd.eq.0.and.nobs4.gt.0) close (134)


    !################## compute misfit for OSL ####################

    if (nd.eq.0.and.nobs5.gt.0) then
      open (135,file=run//'/output/CompareOSL.csv',status='unknown')
      write (135,'(A,6(",",a))') 'SAMPLE','LON','LAT','HEIGHTOBS','HEIGHTPRED','OSLOBS','OSLPRED'
    endif
        
        allocate (ageOSLf(nobs))
        ageOSLf(:) = -9999
    
    ! Loop over OSL data
    do iobs=nobs1+nobs2+nobs3+nobs4+1,nobs1+nobs2+nobs3+nobs4+nobs5
      read (7,*) sampleID_array(iobs),xlonobs,xlatobs, &
                doser(iobs),d0(iobs),et(iobs), &
                logs(iobs),logrho(iobs),eu(iobs),nn(iobs),dnn(iobs),a_coef(iobs),imax(iobs), &
                heighto(iobs),ieobs,wobs1,wobs2,wobs3,wobs4
      hei(iobs)=real(wobs1*zsurf(iconsurf(1,ieobs)) &
                +wobs2*zsurf(iconsurf(2,ieobs)) &
                +wobs3*zsurf(iconsurf(3,ieobs)) &
                +wobs4*zsurf(iconsurf(npe,ieobs)), kind=sp)
      hei(iobs)=real(hei(iobs)*1000.+min(0.d0,heighto(iobs)), kind=sp)
      lonobs(iobs)=real(xlonobs, kind=sp)
      latobs(iobs)=real(xlatobs, kind=sp)
    enddo
  
    allocate (timeOSL(jrec,nobs),temperatureOSL(jrec,nobs),nnf_array(jrec,nobs),ageOSL_array(jrec,nobs))
    call extract_ttpath (jrec,nobs,nz,400,timeOSL,temperatureOSL,iproc,nd)
  
    misfit5 = 0.
    nmisfit5 = 0
        ageOSL_array = 0.
        counter = 0
    ! Lopp over OSL data
    do iobs=nobs1+nobs2+nobs3+nobs4+1,nobs1+nobs2+nobs3+nobs4+nobs5
          counter = counter +1
      ! Compute n/N or ages
          if (p%OSL_Model.eq.0) then

            call SSE_BTS_FAD_model (timeOSL(:,iobs), temperatureOSL(:,iobs), jrec,ageOSL_array,&
                                     doser(iobs), d0(iobs),et(iobs),logs(iobs),logrho(iobs),&
                                     eu(iobs),nnf_array,imax(iobs), nobs, iobs)

          elseif (p%OSL_Model.eq.1) then
            call GOK_Gauss_FAD_model (timeOSL(:,iobs), temperatureOSL(:,iobs), jrec,ageOSL_array,&
                                       doser(iobs), d0(iobs), logs(iobs),et(iobs), eu(iobs),&
                                       logrho(iobs),a_coef(iobs),imax(iobs),nnf_array, nobs, iobs)

      elseif (p%OSL_Model.eq.2) then ! GOK Fad model - eu is gok_b
            call GOK_FAD_model (timeOSL(:,iobs), temperatureOSL(:,iobs), jrec, doser(iobs), d0(iobs),&
                                a_coef(iobs), et(iobs),logs(iobs),  eu(iobs), logrho(iobs), imax(iobs),&
                                nnf_array, ageOSL_array, nobs, iobs)
      
      elseif (p%OSL_Model.eq.3) then ! Gauss FAD model (to test)
        call SSE_Gauss_FAD_model (timeOSL(:,iobs), temperatureOSL(:,iobs), jrec, ageOSL_array,&
                                  doser(iobs), d0(iobs), logs(iobs),et(iobs), eu(iobs), logrho(iobs),&
                                  imax(iobs),nnf_array,nobs, iobs)

          endif
          ageOSLf(counter) = real(ageOSL_array(jrec,iobs), kind=sp)
          nnf = real(nnf_array(jrec,iobs), kind=sp)
          error_pred = real(p%error_predictions, kind=sp)
          
          OSLmisfitTarget = p%OSL_misfit_Target
          if (OSLmisfitTarget.eq.0) then ! calculate misfit on Age
                    call misfit_calculation (misfitflag,nn(iobs),ageOSLf(counter),dnn(iobs),misfitres,error_pred,datafit) ! Use Ages
                else ! Calculate misfit on n/N values
                    call misfit_calculation (misfitflag,nn(iobs),nnf,dnn(iobs),misfitres,error_pred,datafit) ! use n/N value
                    ageOSLf(counter) = nnf
            endif
            misfit5=misfit5+misfitres
            nmisfit5=nmisfit5+1
                
      if (nd.eq.0.and.nobs5.gt.0) write (135,'(a,6(",",g15.9))') & 
        trim(sampleID_array(iobs)),lonobs(iobs),latobs(iobs),heighto(iobs),hei(iobs),nn(iobs),ageOSLf(counter)
    enddo
         
        ! Write time evolution of n/N
        line = ''
        if (nd.eq.0.and.nobs5.gt.0) then
          open (14,file=run//'/output/OSLTime.csv',status='unknown')
          write (line,'(A,1023(",",a))') 'TIME',(trim(sampleID_array(k)),k=nobs1+nobs2+nobs3+nobs4+1,&
                                            nobs1+nobs2+nobs3+nobs4+nobs5)
          write (14,'(a)') line(:len(trim(line))-1) 
          do i=1,jrec
            if (OSLmisfitTarget.eq.0) then ! calculate misfit on Age
                  write (line,'(g12.6,1023(",",g12.6))') timeOSL(i,1), (ageOSL_array(i,iobs),&
                                iobs=nobs1+nobs2+nobs3+nobs4+1,nobs1+nobs2+nobs3+nobs4+nobs5)
            else ! Calculate misfit on n/N values
                  write (line,'(g12.6,1023(",",g12.6))') timeOSL(i,1), (nnf_array(i,iobs),&
                                iobs=nobs1+nobs2+nobs3+nobs4+1,nobs1+nobs2+nobs3+nobs4+nobs5)
            endif       
            write (14,'(a)') line(:len(trim(line))-1)
          enddo
          close (14)
        endif
        
  
    deallocate (timeOSL,temperatureOSL,nnf_array)
  
    if (nd.eq.0.and.nobs5.gt.0) close (135)
        


    !################## compute misfit for ESR ####################

    if (nd.eq.0.and.nobs6.gt.0) then
      open (136,file=run//'/output/CompareESR.csv',status='unknown')
      write (136,'(A,6(",",a))') 'SAMPLE','LON','LAT','HEIGHTOBS','HEIGHTPRED','ESROBS','ESRPRED'
    endif
        allocate (ageESRf(nobs))
        ageESRf(:) = -9999

    do iobs=nobs1+nobs2+nobs3+nobs4+nobs5+1,nobs1+nobs2+nobs3+nobs4+nobs5+nobs6
        read (7,*) sampleID_array(iobs),xlonobs,xlatobs, &
                  doser(iobs),d0(iobs), &
                  logs(iobs),et(iobs),sigmaet(iobs),nn(iobs),dnn(iobs),GOK_a(iobs),GOK_b(iobs),imax(iobs), &
                  heighto(iobs),ieobs,wobs1,wobs2,wobs3,wobs4  
        hei(iobs)=real(wobs1*zsurf(iconsurf(1,ieobs)) &
                +wobs2*zsurf(iconsurf(2,ieobs)) &
                +wobs3*zsurf(iconsurf(3,ieobs)) &
                +wobs4*zsurf(iconsurf(npe,ieobs)), kind=sp)
        hei(iobs)=real(hei(iobs)*1000.+min(0.d0,heighto(iobs)), kind=sp)
        lonobs(iobs)=real(xlonobs, kind=sp)
        latobs(iobs)=real(xlatobs, kind=sp)
    enddo           
          
    allocate (timeESR(jrec,nobs),temperatureESR(jrec,nobs),nnf_array(jrec,nobs),ageESR_array(jrec,nobs))
    call extract_ttpath (jrec,nobs,nz,400,timeESR,temperatureESR,iproc,nd)
    
    misfit6 = 0.
    nmisfit6 = 0
    ageESR_array = 0.
    counter = 0
    ! loop over data
    do iobs=nobs1+nobs2+nobs3+nobs4+nobs5+1,nobs1+nobs2+nobs3+nobs4+nobs5+nobs6
      counter = counter +1

      ! Calculate ESR n/N or age
      if (p%ESR_Model.eq.0) then ! SSE_Gauss
        call SSE_Gauss_model (timeESR(:,iobs), temperatureESR(:,iobs), jrec, doser(iobs), d0(iobs),&
        logs(iobs),et(iobs),sigmaet(iobs),imax(iobs),nnf_array,ageESR_array, nobs, iobs)

      elseif (p%ESR_Model.eq.1) then ! GOK-GOK
        call GOK_model (timeESR(:,iobs), temperatureESR(:,iobs), jrec, doser(iobs), d0(iobs), GOK_a(iobs),&
               et(iobs), logs(iobs), GOK_b(iobs), imax(iobs),nnf_array,ageESR_array, nobs, iobs)
      endif
      nnf = real(nnf_array(jrec,iobs), kind=sp)
      ageESRf(counter) = real(ageESR_array(jrec,iobs), kind=sp)
            
      ESRmisfitTarget = p%ESR_misfit_Target
      error_pred = real(p%error_predictions, kind=sp)

      ! Calculate misfit for ESR
      if (nobs6.gt.0) then
        if (ESRmisfitTarget.eq.0) then ! calculate misfit on Age
          call misfit_calculation (misfitflag,nn(iobs),ageESRf(counter),dnn(iobs),misfitres,error_pred,datafit) ! Use Ages
        else ! Calculate misfit on n/N values
          call misfit_calculation (misfitflag,nn(iobs),nnf,dnn(iobs),misfitres,error_pred,datafit) ! use n/N value
          ageESRf(counter) = nnf
        endif
        misfit6=misfit6+misfitres+1e-3
        nmisfit6=nmisfit6+1
      endif

      if (nd.eq.0.and.nobs6.gt.0) write (136,'(a,6(",",g15.9))') & 
        trim(sampleID_array(iobs)),lonobs(iobs),latobs(iobs),heighto(iobs),hei(iobs),nn(iobs),ageESRf(counter)

    enddo

        ! Write time evolution of n/N or age
        line = ''
        if (nd.eq.0.and.nobs6.gt.0) then

            open (14,file=run//'/output/ESRTime.csv',status='unknown')
            write (line,'(A,1023(",",a))') 'TIME',(trim(sampleID_array(k)),k=nobs1+nobs2+nobs3+nobs4+nobs5+1,&
                                                        nobs1+nobs2+nobs3+nobs4+nobs5+nobs6)
            write (14,'(a)') line(:len(trim(line))-1)   
      ! loop over data
            do i=1,jrec
                if (ESRmisfitTarget.eq.0) then ! calculate misfit on ages
                    write (line,'(g12.6,1023(",",g12.6))') timeESR(i,1), (ageESR_array(i,iobs),&
                            iobs=nobs1+nobs2+nobs3+nobs4+nobs5+1,nobs1+nobs2+nobs3+nobs4+nobs5+nobs6)
                else ! Calculate misfit on n/N values
                    write (line,'(g12.6,1023(",",g12.6))') timeESR(i,1), (nnf_array(i,iobs),&
                            iobs=nobs1+nobs2+nobs3+nobs4+nobs5+1,nobs1+nobs2+nobs3+nobs4+nobs5+nobs6)
                endif       
                write (14,'(a)') line(:len(trim(line))-1)
            enddo

            close (14)
        endif
        
    
    deallocate (timeESR,temperatureESR,nnf_array)
          
    ! #################### End ESR ####################
    
    if (nd.eq.0.and.nobs6.gt.0) close (136)
    deallocate (lonobs,latobs)

    ! Calculate total misfit
        misfit = real((misfit4*p%misfit_weight_TL+ misfit5*p%misfit_weight_OSL + misfit6*p%misfit_weight_ESR +&
              misfitAHE*p%misfit_weight_AHE + MisfitZHE*p%misfit_weight_ZHE + misfitAFT*p%misfit_weight_AFT + &
              misfitZFT*p%misfit_weight_ZFT + misfitBAR*p%misfit_weight_BAR + misfitMAR*p%misfit_weight_MAR + &
              misfitHAR*p%misfit_weight_HAR + misfitKAR*p%misfit_weight_KAR) + misfitMFTL*p%misfit_weight_FTLD + &
              misfitDMFTL* p%misfit_weight_FTLD + misfit1a * p%misfit_weight_FTLD + misfit2*p%misfit_weight_TH + &
              (misfit3 + misfit3a)*p%misfit_weight_43HE , kind=sp)
        
    ! Sum number of data
        nmisfit = nmisfit1 + nmisfit1a + nmisfit2 + nmisfit3 + nmisfit3a + nmisfit4 + nmisfit5 + nmisfit6
          
        ! Apply L2-norm
        if (p%misfit_function.eq.3) then
            misfit = sqrt(misfit/(nmisfit1 + nmisfit1a + nmisfit2 + nmisfit3 + nmisfit3a + nmisfit4 + nmisfit5 + nmisfit6))
        endif
     
    ! If misfit chi-squared corrected of L1-norm corrected
    if (p%misfit_function.eq.2.or.p%misfit_function.eq.5) then
      ! check if the denominator is higher than the number of data
      if (nmisfit1 + nmisfit1a + nmisfit2 + nmisfit3 + nmisfit3a + nmisfit4 + nmisfit5 + nmisfit6 - nd -1 .gt. 0) then
        misfit = misfit/(nmisfit1 + nmisfit1a + nmisfit2 + nmisfit3 + nmisfit3a + nmisfit4 + nmisfit5 + nmisfit6 - nd - 1)
      else
        if (iproc.eq.0) then
          write (*,*) 'Warning - Pecube will not correct the misfit (misfit_corrected = 1) when the total number'
          write (*,*) 'of data points is smaller than the number of parameters being inverted  + 1'
          write (*,*) 'Currently it is:',nmisfit1 + nmisfit1a + nmisfit2 + nmisfit3 + nmisfit3a + nmisfit4 + nmisfit5 + nmisfit6
          write (*,*) 'and the number of parameters is ',nd
        endif
      endif
    endif
        
        ! Score model with data ?
        rand_num = 0
        call random_number(rand_num)
            datascore = (1 - (datafit / (nmisfit1 + nmisfit1a + nmisfit2 + nmisfit3 + nmisfit3a + nmisfit4 + nmisfit5 + nmisfit6) +&
                        rand_num/100))* (nmisfit1 + nmisfit1a + nmisfit2 + nmisfit3 + nmisfit3a + nmisfit4 + nmisfit5 + nmisfit6)
        if (p%score_with_data.eq.1) then
            datascore = misfit
            misfit = (1 - (datafit / (nmisfit1 + nmisfit1a + nmisfit2 + nmisfit3 + nmisfit3a + nmisfit4 + nmisfit5 + nmisfit6) +&
                        rand_num/100))* (nmisfit1 + nmisfit1a + nmisfit2 + nmisfit3 + nmisfit3a + nmisfit4 + nmisfit5 + nmisfit6)
        endif
        
    ! Write misfits in file
    if (nd.ne.0) then
        
      ! Open file according to the operating system
          if (is_unix) then
              open (71,file=run//'/NA/NA_int_res.csv',status='old',position='append',action='write')
          else
              open (71,file=run//'\NA\NA_int_res.csv',status='old',position='append',action='write')
          endif
      ! Write misfit values in file
      ! write (71,'(g15.9,99(a1,g15.9))') misfit,(",",param(i),i=1,nd),",",datascore
      write (71,'(g15.9,99(a1,g15.9))') misfit,(",",param(i),i=1,nd)
      close (71)
    endif


    if (nd.eq.0.and.p%echo_input_file.gt.0.and.agecomput.eq.2) then
        write (6,*) 'Misfit T.Lum.: ',misfit4
        write (6,*) 'Misfit OSL: ',misfit5
        write (6,*) 'Misfit ESR: ',misfit6
        write (6,*) 'Misfit AHE: ',misfitAHE
        write (6,*) 'Misfit AFT : ',misfitAFT
        write (6,*) 'Misfit ZHE : ',misfitZHE
        write (6,*) 'Misfit ZFT : ',misfitZFT
        write (6,*) 'Misfit KAR : ',misfitKAR
        write (6,*) 'Misfit BAR : ',misfitBAR
        write (6,*) 'Misfit MAR : ',misfitMAR
        write (6,*) 'Misfit HAR : ',misfitHAR
            write (6,*) 'Misfit MFTL : ',misfitMFTL
            write (6,*) 'Misfit DMFTL : ',misfitDMFTL
        write (6,*) 'Misfit 4He/3He : ',misfit3
        write (6,*) 'Total misfit : ',misfit
    endif

    ! write sumamry misfit by thermochronometer in file (forward mode)  
    if (nd.eq.0.and.agecomput.eq.2) then
          open (71,file=run//'/output/Misfit_forward.csv',status='unknown',position='append')
          write (71,'(a,14(",",a))') 'Parameter','Total Misfit','THL','OSL','ESR','AHE','AFT','ZHE','ZFT','KAR','BAR','MAR','HAR',&
                                        'MFTL','DMFTL'
          ! First write number of observation
          write (71,*) "N observation,",nmisfit,",",nmisfit4,",",nmisfit5,",",nmisfit6,",",nmisfitAHE,",",&
           nmisfitAFT,",",nmisfitZHE,",",nmisfitZFT,",",nmisfitKAR,",", nmisfitBAR, ",", nmisfitMAR,",",nmisfitHAR,&
           ",",nmisfitMFTL,",",nmisfitMFTL
           ! Second is chi squared
           write (71,*) "misfit,",misfit,",",misfit4,",",misfit5,",",misfit6,",",misfitAHE,",",misfitAFT,",",&
           misfitZHE,",",misfitZFT,",",misfitKAR,",", misfitBAR, ",", misfitMAR,",",misfitHAR, ",",&
           misfitMFTL,",",misfitDMFTL
           ! Third is Log likelihood
           write (71,*) "Log likelihood,",LL_total,",",LL_ThL,",",LL_OSL,",",LL_ESR,",",LL_AHE,",",LL_AFT,",",&
                LL_ZHE,",",LL_ZFT,",",LL_KAR,",", LL_BAR, ",", LL_MAR,",",LL_HAR,",",LL_MFTL,",",LL_DMFTL
           close (71)     
    endif

    ! If in batch mode 
    if (p%inversion_mode.eq.2) then
        ! ! write elevations
        ! open (71,file=run//'/NA/ElevationPredicted.csv',status='old',position='append', action='write')
        ! write (71,'(g15.9,99(a1,g15.9))') misfit,(",",hei(i),i=1,nhe+nmisfit6+nft+nheZ+nftZ+narK+narB+&
        !                                               narM+narH)
        ! close (71)

        ! write Ages
        if (p%age_ESR_flag.ne.0.) then
          open (71,file=run//'/NA/AgeESR.csv',status='old',position='append', action='write')
          write (71,'(g15.9,99(a1,g15.9))') misfit,(",",hei(i),i=1,nmisfit6)
          write (71,'(g15.9,99(a1,g15.9))') misfit,(",",ageESRf(i),i=1,nmisfit6)
          close (71)
        endif
        if (p%age_AHe_flag.ne.0.) then
          open (71,file=run//'/NA/AgeApatiteHelium.csv',status='old',position='append', action='write')
          write (71,'(g15.9,99(a1,g15.9))') misfit,(",",hei(i),i=1+nmisfit6,nmisfit6+nhe)
          write (71,'(g15.9,99(a1,g15.9))') misfit,(",",agehe(i),i=1+nmisfit6,nmisfit6+nhe)
          close (71)
        endif
        if (p%age_AFT_flag.ne.0.) then
          open (71,file=run//'/NA/AgeApatiteFT.csv',status='old',position='append', action='write')   
          write (71,'(g15.9,99(a1,g15.9))') misfit,(",",hei(i),i=1+nmisfit6+nhe,nmisfit6+nhe+nft) 
          write (71,'(g15.9,99(a1,g15.9))') misfit,(",",ageft(i),i=1+nmisfit6+nhe,nmisfit6+nhe+nft)
          close (71)
        endif
         if (p%age_ZHe_flag.ne.0.) then
          open (71,file=run//'/NA/AgeZirconHelium.csv',status='old',position='append', action='write')
          write (71,'(g15.9,99(a1,g15.9))') misfit,(",",hei(i),i=1+nmisfit6+nhe+nft,nmisfit6+nhe+nft+nheZ)
          write (71,'(g15.9,99(a1,g15.9))') misfit,(",",ageheZ(i),i=1+nmisfit6+nhe+nft,nmisfit6+nhe+nft+nheZ)
          close (71)
        endif
        if (p%age_ZFT_flag.ne.0.) then
          open (71,file=run//'/NA/AgeZirconFT.csv',status='old',position='append', action='write')
          write (71,'(g15.9,99(a1,g15.9))') misfit,(",",hei(i),i=1+nmisfit6+nhe+nft+nheZ,&
                                                     nmisfit6+nhe+nft+nheZ+nftZ)
          write (71,'(g15.9,99(a1,g15.9))') misfit,(",",ageftZ(i),i=1+nmisfit6+nhe+nft+nheZ,&
                                                     nmisfit6+nhe+nft+nheZ+nftZ)
          close (71)
        endif
        if (p%age_KAr_flag.ne.0.) then
          open (71,file=run//'/NA/AgeKSparArgon.csv',status='old',position='append', action='write')
          write (71,'(g15.9,99(a1,g15.9))') misfit,(",",hei(i),i=1+nmisfit6+nhe+nft+nheZ+nftZ,&
                                                    nmisfit6+nhe+nft+nheZ+nftZ+narK)
          write (71,'(g15.9,99(a1,g15.9))') misfit,(",",agearK(i),i=1+nmisfit6+nhe+nft+nheZ+nftZ,&
                                                    nmisfit6+nhe+nft+nheZ+nftZ+narK)
          close (71)
        endif
        if (p%age_BAr_flag.ne.0.) then
          open (71,file=run//'/NA/AgeBiotiteArgon.csv',status='old',position='append', action='write')
          write (71,'(g15.9,99(a1,g15.9))') misfit,(",",hei(i),i=1+nmisfit6+nhe+nft+nheZ+nftZ+narK,&
                                                      nmisfit6+nhe+nft+nheZ+nftZ+narK+narB)
          write (71,'(g15.9,99(a1,g15.9))') misfit,(",",agearB(i),i=1+nmisfit6+nhe+nft+nheZ+nftZ+narK,&
                                                      nmisfit6+nhe+nft+nheZ+nftZ+narK+narB)
          close (71)
        endif
        if (p%age_MAr_flag.ne.0.) then
          open (71,file=run//'/NA/AgeMuscoviteArgon.csv',status='old',position='append', action='write')
          write (71,'(g15.9,99(a1,g15.9))') misfit,(",",hei(i),i=1+nmisfit6+nhe+nft+nheZ+nftZ+narK+narB,&
                                                     nmisfit6+nhe+nft+nheZ+nftZ+narK+narB+narM)
          write (71,'(g15.9,99(a1,g15.9))') misfit,(",",agearM(i),i=1+nmisfit6+nhe+nft+nheZ+nftZ+narK+narB,&
                                                     nmisfit6+nhe+nft+nheZ+nftZ+narK+narB+narM)
          close (71)
        endif
        if (p%age_HAr_flag.ne.0.) then
          open (71,file=run//'/NA/AgeHornblendeArgon.csv',status='old',position='append', action='write')
          write (71,'(g15.9,99(a1,g15.9))') misfit,(",",hei(i),i=1+nmisfit6+nhe+nft+nheZ+nftZ+narK+&
                                                    narB+narM,nmisfit6+nhe+nft+nheZ+nftZ+narK+narB+narM+narH)
          write (71,'(g15.9,99(a1,g15.9))') misfit,(",",agearH(i),i=1+nmisfit6+nhe+nft+nheZ+nftZ+narK+&
                                                    narB+narM,nmisfit6+nhe+nft+nheZ+nftZ+narK+narB+narM+narH)
          close (71)
        endif                                 
    endif

    if (nd.ne.0.and.p%echo_input_file.gt.0) write (*,*) 'Misfit:',misfit,misfit1,misfit2,misfit3,&
                misfit4,misfit5,misfit6,' - Params',param(1:nd)
    !close (13)
    
   ! added by Jean to change misfit to slope rather than exact ages
   ! (4/6/2008)

    deallocate (rmr043_array)
    deallocate (age43He)
    deallocate (size43He_array)
    deallocate (Uppm43_array)
    deallocate (Thppm43_array)
    deallocate (dage43He)
    deallocate (dummy43)
    deallocate (aheoreg,ahepreg,hheoreg)
    deallocate (aftoreg,aftpreg,hftoreg)
    deallocate (aheZoreg,aheZpreg,hheZoreg)
    deallocate (aftZoreg,aftZpreg,hftZoreg)
    deallocate (aarKoreg,aarKpreg,harKoreg)
    deallocate (aarBoreg,aarBpreg,harBoreg)
    deallocate (aarMoreg,aarMpreg,harMoreg)
    deallocate (aarHoreg,aarHpreg,harHoreg)
    deallocate (hei,agehe,ageheZ,ageft,ageftZ,agearB,agearK,agearM,agearH,ftdisto,ageESRf,ageOSLf,fmeanp,dfmeanp)
    deallocate (hpreg,sampleID_array)
    deallocate (age1,age2,age3,age4,age5,age6,age7,age8,grainsize,AUppm,AThppm,kinFTL_AFT,kinFTL_AHe) !Maxime
    deallocate (ZSize,ZUppm,ZThppm)

    333   continue
    deallocate (iconsurf,ielsurf,neighbour)
    deallocate (xsurf,ysurf,zsurf,zsurfp)
    deallocate (topoa,topob,rsurf,rebound)
    deallocate (xdepth,ydepth,zdepth,tprev)
    deallocate (xexhumation,yexhumation,zexhumation)
    deallocate (x,y,z,t)
    deallocate (xp,yp,zp,tp)
    deallocate (icon,ael,bel)
    deallocate (kfix)
    deallocate (f)
    deallocate (proc,ice)

    call cpu_time (times(9))

      close (10)
    if (p%echo_input_file.gt.0) then
      if (nd.eq.0) write (*,*) '---------------------------------------------------------------------------------'
    endif

    if (iproc.eq.0.and.nd.eq.0) write (6,*) 'Total compute time : ',times(9)-times(1),' sec'

    close (7)

    if (p%echo_input_file.gt.0.and.nd.eq.0) then
      write (*,*) '---------------------------------------------------------------------------------'
      write (*,*) '-------------------------End of Pecube run---------------------------------------'
      write (*,*) '---------------------------------------------------------------------------------'
    endif

      close (500) ! Maxime

  !return

end subroutine forward

!--------------------------------------------------------------------------------------------

subroutine regression (x,y,n,xmean,ymean,intercept,slope)

! this routine is used to calculate the means, intercept and slope
! of the regression between two datasets of length n stored in x and y

  implicit none

  integer n
  double precision x(n),y(n),xmean,ymean,intercept,slope

  xmean=sum(x)/n
  ymean=sum(y)/n
  slope=sum((x-xmean)*(y-ymean))/sum((x-xmean)**2)
  intercept=ymean-xmean*slope

  return
end
 
 !--------------------------------------------------------------------------------------------

subroutine misfit_calculation (flag,obs,pred,error,results,error_pred,datafit)

 ! this routine computes the misfit function (phi):
 ! Three options are available:
 !    - L2-norm : flag = 1
 !    - L1-norm : flag = 2
 ! ! for real numbers

    integer flag
    real *4 results, pred,obs,error,error_pred, datafit, error_obs, error_pred2

        ! Check whether user wants to add error on predictions
    error_obs = error + 1e-10
    error_pred2 = 0.0
    pred = pred + 1e-10
        if (error_pred.gt.0) then
            error_pred2 = pred*(error_pred/100)
        endif

    if (flag.eq.1.or.flag.eq.2.or.flag.eq.3) then ! Chi-squared, Corrected chi-squared or L2-norm
        results = (obs - pred)**2 / (error**2 + error_pred2**2)

    elseif (flag.eq.4.or.flag.eq.5) then ! L1-norm or Corrected L1-norm
        results = abs((obs - pred) / sqrt(error**2 + error_pred2**2))

      elseif (flag.eq.6) then ! Log-scale misfit
        results = (0.5*(obs/error)*log(obs/pred))**2
    endif

      if (results.le.1) then
            datafit = datafit + 1
      endif

    return
       
end subroutine misfit_calculation
       
 !--------------------------------------------------------------------------------------------

subroutine LL_calculation (flag,obs,pred,error,results,error_pred)

 ! this routine computes the misfit function (phi):
 ! Three options are available:
 !    - L2-norm : flag = 1
 !    - L1-norm : flag = 2
 ! for real numbers

  integer flag,n
  real *4 results, pred,obs,error, error_obs, error_pred2

  real *4 error_pred
       
  ! Check whether user wants to add error on predictions
  results = 0.0
  error_pred2 = 0.0
  error_obs = error
  if (error_pred.gt.0) then
    error_pred2 = pred*(error_pred/100)
  endif

  if (flag.eq.4.or.flag.eq.5) then ! L1-norm or Corrected L1-norm
      results = -log(2*error) - (abs(obs-pred)/(sqrt(error**2+error_pred2**2)))

  elseif (flag.eq.1.or.flag.eq.2.or.flag.eq.3) then ! Chi-squared, Corrected chi-squared or L2-norm
      results = -(log(2*3.14159265359)/2 + (log(error)) + 0.5*((obs-pred)**2/(error**2+error_pred2**2)))

  elseif (flag.eq.6) then
      results = -(0.5*(obs/error)*log(pred/obs))**2
  endif

  return
       
end subroutine LL_calculation

 !--------------------------------------------------------------------------------------------

subroutine misfit_calculation_DP (flag,obs,pred,error,results)

 ! this routine computes the misfit function (phi):
 ! Three options are available:
 !    - L2-norm : flag = 1
 !    - L1-norm : flag = 2
 ! For double precision numbers

  integer flag
  double precision pred,obs,error
  real *8 results
  flag = 1
!        if (flag.eq.1.or.flag.eq.2) then
!            results = ((obs - pred) / error)**2
!        elseif (flag.eq.3.or.flag.eq.4) then
    results = abs((obs - pred) / error)
!        endif
  return
       
end subroutine misfit_calculation_DP
       
 !--------------------------------------------------------------------------------------------

subroutine LL_calculation_DP (flag,obs,pred,error,results)

 ! this routine computes the misfit function (phi):
 ! Three options are available:
 !    - L2-norm : flag = 1
 !    - L1-norm : flag = 2
  ! For double precision numbers

  integer flag
  double precision pred,obs,error
  real *8 results
  flag=1
!        if (flag.eq.3.or.flag.eq.4) then !L1-norm
           results = -log(2*error) - (abs(obs-pred)/error)
!        elseif (flag.eq.1.or.flag.eq.2) then !L2-norm
!            results = -(log(2*3.14159265359)/2 + (log(error)) + 0.5*((obs-pred)/error)**2)
!        endif
  return
       
end subroutine LL_calculation_DP
