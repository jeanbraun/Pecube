...
! This subroutines is called in 'create_pecube_in.f90'
...
logical french, is_unix
character*100 :: folder2
character*100, dimension(:,:), allocatable :: t
...
if (nd.eq.0 .and. echo.gt.0) print*,'---------------------------------------------------------------------------------'                       
if (nd.eq.0 .and. echo.gt.0) print*,'-----------------------------Reading data----------------------------------------'
if (nd.eq.0 .and. echo.gt.0) print*,'---------------------------------------------------------------------------------'
...
'SIZE  ','ASIZE ','AUPPM ', 'ATHPPM','KINAFT','KINAHE','ZUPPM ','ZTHPPM','ZSIZE ','MFTL  ','DMFTL '/) !Maxime
...
if (is_unix) then
    ...
    call RemoveDirectory(".\tmp")
    call system ('mkdir tmp\')
    !Change slash to backslash
    folder2 = folder
    do i = 1, len_trim(folder2)
        if (folder2(i:i) == '/') then
          ! Replace backslash with forward slash
          folder2(i:i) = '\'
        end if
    end do
    folder2 = trim(folder2)
    call system('dir /b ' // folder2 // ' > tmp\data_lst' // cproc // '.txt')
    if (nd.eq.0) then
      print*,'Reading data from folder: ',folder2
    endif
endif
...
if (is_unix) then
  ...
  do
  ! Read each csv files stored in data_lst
  read (8,'(a)', end=998) fnme
  if (is_unix) then
    call read_data_files (folder//'/'//fnme, t, nx, ny, f, s)
  else
    call read_data_files (trim(folder2)//'\'//fnme, t, nx, ny, f, s)
  endif
    do i = 1, ny
      do j = 1, nsample
...
if (is_unix) then
    ...
    !write (111,*) nsample
      ...
      enddo
    close (111)
    ! Maxime - Write obs sample name
    open (112, file = folder//cproc//'samples.txt')
    !write (111,*) nsample
      do i = 1, nsample
      write (112,*) sample(i)
      enddo
    ...
    if (nd.eq.0.and.iproc.eq.0 .and. echo.ge.0) print*,'Number of Samples:',nsample

    ! now scans folder for thermal histories
    call read_data_files_for_thermal_histories (folder,cproc, xlon1, xlon2, xlat1, xlat2, nd, nobs2, is_unix)
    ! now scans folder for thermal histories
    call read_data_files_for_43He (folder,cproc, xlon1, xlon2, xlat1, xlat2, nd, nobs3, is_unix)
    ! now scans for TSL data
    call read_data_files_for_TSL (folder,cproc, xlon1, xlon2, xlat1, xlat2, nd, nobs4, is_unix)
    ! now scans for OSL data
    call read_data_files_for_OSL (folder,cproc, xlon1, xlon2, xlat1, xlat2, nd, nobs5, is_unix)
    ! now scans for ESR data
    call read_data_files_for_ESR (folder,cproc, xlon1, xlon2, xlat1, xlat2, nd, nobs6, is_unix) 
    
else
    call CheckFileExistence(trim(folder2)//cproc//'.txt')
    open (111, file = trim(folder2)//cproc//'.txt')
    !write (111,*) nsample
      do i = 1, nsample
      write (111,*) (obs(j,i), j = 1, nfield)
      enddo
    close (111)
    ! Maxime - Write obs sample name
    call CheckFileExistence(trim(folder2)//cproc//'samples.txt')
    open (112, file = trim(folder2)//cproc//'samples.txt')
    !write (111,*) nsample
      do i = 1, nsample
      write (112,*) sample(i)
      enddo
    ...
    ! now scans folder for thermal histories
    call read_data_files_for_thermal_histories (trim(folder2),cproc, xlon1, xlon2, xlat1, xlat2, nd, nobs2, is_unix)
    ! now scans folder for thermal histories
    call read_data_files_for_43He (trim(folder2),cproc, xlon1, xlon2, xlat1, xlat2, nd, nobs3, is_unix)
    ! now scans for TSL data
    call read_data_files_for_TSL (trim(folder2),cproc, xlon1, xlon2, xlat1, xlat2, nd, nobs4, is_unix)
    ! now scans for OSL data
    call read_data_files_for_OSL (trim(folder2),cproc, xlon1, xlon2, xlat1, xlat2, nd, nobs5, is_unix)
    ! now scans for ESR data
    call read_data_files_for_ESR (trim(folder2),cproc, xlon1, xlon2, xlat1, xlat2, nd, nobs6, is_unix)  
endif
                                                                          
deallocate (sample, field_name)

if (nd.eq.0 .and. echo.gt.0) then
print*,'---------------------------------------------------------------------------------'
endif
        ...
        do i=1, nx
          do it = 1, nfield
            if (trim(ucase(f(i))).eq.trim(field_name(it))) then
              ...
              obs(it,jsample) = val
              nobs(it,jsample) = 1
              ! if we want several observations in same location (ex: AHe + 43He)
              ! this has to be commented...
!               else
!               print *, val
!               print *, obs(it,jsample)
...
subroutine read_data_files_for_thermal_histories (folder, cproc, xlon1, xlon2, xlat1, xlat2, nd, nobs, is_unix)
...
open (8, file='tmp/data_lst'//cproc//'.txt', status='old', err=996)
  ...
  ! Check Os system
  if (is_unix) then
    folderOS = trim(folder//'/'//trim(fnme))
  else
    folderOS = trim(folder//'\'//trim(fnme))
  endif
  
  open (9, file = folderOS, status = 'old')
...
!  if (lon.lt.0.d0) lon = lon + 360.d0
    ...
    call find_string (9, 'SAMPLE', isample, jsample)
      ...
      if (itemph.eq.0) then
      print*,'Found TIMEH field but not TEMPH field in file ',trim(folderOS)
      stop
      endif
...
subroutine read_data_files_for_43He (folder, cproc, xlon1, xlon2, xlat1, xlat2, nd, nobs, is_unix)
...
integer :: nobs, nhist, iuppm, juppm, ithppm, jthppm, irmr0, jrmr0
character*100 :: sample, field,sample_prev
double precision :: lat, lon, height,size43,age,dage
double precision :: temph(10000),durh(10000),rel(10000),drel(10000),arel(10000),darel(10000)
real UPPM, THPPM, RMR0 ! Maxime
integer :: i,ksample
character*4 :: cproc
character(len=1024) :: folderOS
logical is_unix
...
! then reads the lat, lon, height and 43He data of all samples in all files
...
open (8, file='tmp/data_lst'//cproc//'.txt', status='old', err=994)
  ...
  do
    ...
    if (idurh.ne.0) then
      ...
      if (itemph.eq.0) then
        print*,'Found DUR43 field but not TEMP43 field in file ',trim(folderOS)
        stop
      endif
      ...
      if (isample.ne.0) then
        ...
        do while (trim(sample).ne.'NotFound')
          jsample = jsample + 1
          sample = read_string(9, isample, jsample)
          if (trim(sample).ne.trim(sample_prev).or.sample.eq.'NotFound') then
              ! Write data in temporary file
              if (nhist.ne.0) then
                if ((lon - xlon1)*(lon - xlon2).le.0.d0 .and. (lat - xlat1)*(lat - xlat2).le.0.d0) then
                  write (111,*) sample_prev,lon,lat,height,size43,age,dage,nhist,UPPM,THPPM,RMR0,&
                  (temph(i),durh(i),rel(i),drel(i),arel(i),darel(i),i=1,nhist) ! Maxime added UPPM and THPPM
                endif
              endif
              nhist = 0 
          endif
...
subroutine read_data_files_for_TSL (folder, cproc, xlon1, xlon2, xlat1, xlat2, nd, nobs, is_unix)
...
do
  ...
  call read_table(folderOS, tag0, 12, sample_names, sample_values, nsamples, 10000,tagSample)
    ...
    if (abs(sample_values(6,isample)+9999).gt.tiny(0.d0)) then
      do j = 1,12
        if (abs(sample_values(j,isample)+9999).lt.tiny(0.d0) .and. j.ne.3) then
          if (nd.eq.0) print*,'TLSSample ',trim(sample_names(isample)),' needs the ',trim(tag0(j)),' field'
          stop
        endif
      enddo
...
do
  ...
  do isample = 1, nsamples
   if (abs(sample_values(6,isample)+9999).gt.tiny(0.d0)) then                                                         
    ...
    if ((lon - xlon1)*(lon - xlon2).le.0.d0 .and. (lat - xlat1)*(lat - xlat2).le.0.d0) &
      write (111,*) trim(sample_names(isample)),lon,lat,height,doser,d0,a,et,logs,b,logrho,nn,dnn
    endif
  enddo
enddo
...
subroutine read_data_files_for_OSL (folder, cproc, xlon1, xlon2, xlat1, xlat2, nd, nobs, is_unix)
  ...
  integer :: isample
  integer :: nobs
  double precision :: lat, lon, height, doser, d0, et, logs, logrho, eu, nn, a, imax
  double precision :: dnn
  integer :: i,j,ntag
  character*4 cproc
  character*128 tag0(13)
  character*128 sample_names(10000)
  double precision sample_values(13, 10000)
  integer nsamples
  character(len=1024) :: folderOS
  logical is_unix
  character*128 tagSample
  ...
  nobs = 0
  ntag = 13 ! to update according to number of parameters below
  tagSample = 'SAMPLEOSL'
  tag0(1)='LON'
  tag0(2)='LAT'
  tag0(3)='HEIGHT'
...
if (is_unix) then
  ...
  do
    ...
    call read_table(folderOS, tag0, ntag, sample_names, sample_values, nsamples, 10000,tagSample)
    do isample = 1, nsamples
  ...
  do
    ...
    do isample = 1, nsamples
      if (abs(sample_values(9,isample)+9999).gt.tiny(0.d0)) then
        ...
        a = sample_values(12,isample)
        imax = sample_values(13,isample)
        if ((lon - xlon1)*(lon - xlon2).le.0.d0 .and. (lat - xlat1)*(lat - xlat2).le.0.d0) &
          write (111,*) trim(sample_names(isample)),lon,lat,height,doser,d0,et,logs,logrho,eu,nn,dnn,a,imax
      endif
    enddo
  enddo
  ...
  subroutine read_data_files_for_ESR (folder, cproc, xlon1, xlon2, xlat1, xlat2, nd, nobs, is_unix)
    ...
    integer :: isample
    integer :: nobs
    double precision :: lat, lon, height, doser, d0, logs, et, sigmaet, nn, dnn,imax
    double precision :: GOK_a, GOK_b
    integer :: i,j, ntag
    character*4 cproc
    character*128 tag0(14)
    character*128 sample_names(10000)
    double precision sample_values(13, 10000)
    integer nsamples
    character(len=1024) :: folderOS
  ...
  tag0(12) = 'b'
    ...
    do
      ...
      do isample = 1, nsamples
        if (abs(sample_values(8,isample)+9999).gt.tiny(0.d0)) then ! Check for ESR, Name of sample Header is SAMPLEESR
          do j = 1,ntag
            if (abs(sample_values(j,isample)+9999).lt.tiny(0.d0) .and. j.ne.3) then
          ...
          lon = sample_values(1,isample)
          lat = sample_values(2,isample)
          ! print *, 'lat = ', lat, xlat1, xlat2
          ! print *, 'lon = ', lon, xlon1, xlon2
          ! print *, 'Sample Name = ', sample_names(isample)
          ! print *, 'sample_values = ',sample_values(1:10,isample)
          if ((lon - xlon1)*(lon - xlon2).le.0.d0 .and. (lat - xlat1)*(lat - xlat2).le.0.d0) &
            nobs = nobs + 1
        endif
      enddo
    ...
    do
      ...
      GOK_a = sample_values(11,isample)
      GOK_b = sample_values(12, isample)
          imax = sample_values(13,isample)
          if ((lon - xlon1)*(lon - xlon2).le.0.d0 .and. (lat - xlat1)*(lat - xlat2).le.0.d0) &
            write (111,*) trim(sample_names(isample)),lon,lat,height,doser,d0,logs,et,sigmaet,nn,dnn,&
                  GOK_a,GOK_b,imax
        endif
      enddo
    enddo
